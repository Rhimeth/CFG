...
namespace GraphGenerator {
    ...
    void extractStatementsFromBlock(const clang::CFGBlock* block, CFGGraph* graph, const QString& filename) {
        ...
        // Get the ASTContext from the CFG's context member
        const clang::CFG* cfg = block->getParent();
        if (!cfg) return;
        
        // CFG::getDecl() doesn't exist, we need to use getParentDecl() instead
        const clang::Decl* D = cfg->getParentDecl();
        if (!D) return;
        
        const clang::ASTContext& astContext = D->getASTContext();
