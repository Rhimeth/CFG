...
int main(int argc, char *argv[])
{
    ...
    try {
        ...
        if (parser.isSet(inputFileOption)) {
            QString inputFile = parser.value(inputFileOption);
            window.openFile(inputFile); // Use the new public method
        }method instead of private loadFile()
           window.initialize();
        return app.exec();       // You'll need to add code to pass the file path or handle it another way
    } catch (const std::exception& e) {        // For example, assuming you have a public method to open files:
        qCritical() << "Fatal error:" << e.what();t::invokeMethod(&window, "on_browseButton_clicked", 
        QMessageBox::critical(nullptr, "Fatal Error", Q_ARG(QString, inputFile));
