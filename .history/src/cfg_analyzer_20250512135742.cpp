...
AnalysisResult CFGAnalyzer::analyzeFile(const QString& filePath) {
    AnalysisResult result;
    result.success = false; // Default to failure state
    ...
    try {
        ...
        // Use a separate autorelease scope for the analysis
        {
            auto analysisResult = analyze(filename);
            
            // Exit early if analysis failed
            if (!analysisResult.success) {
                result.report = "Analysis failed: " + analysisResult.report;
                return result;
            }
            
            // Safely copy successful results
            result = analysisResult;
        }
        
        // Only generate JSON if the analysis was successful
        if (result.success) {
            try {
                json j = {
                    {"filename", filename},
                ...
                for (const auto& funcPair : result.functions) {
                    ...
                    if (depIt != result.functionDependencies.end()) {
                        // Use a temporary vector for the calls
