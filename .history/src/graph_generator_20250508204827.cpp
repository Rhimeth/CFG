...
namespace GraphGenerator {
    ...
    void extractStatementsFromBlock(const clang::CFGBlock* block, CFGGraph* graph, 
        ...
        for (const auto& element : *block) {
            if (element.getKind() == clang::CFGElement::Statement) {
                ...
                // Extract precise source location information
                ...
                if (range.isValid()) {
                    ...
                    // Store the full source location in the node
                    std::string sourceFile = SM.getFilename(range.getBegin()).str();
                    if (!sourceFile.empty()) {
                        graph->setNodeSourceLocation(block->getBlockID(), 
        ...
        // Set the source range for the block if not already set
        if (startLine != -1 && endLine != -1 && !filename.isEmpty()) {
    ...
    std::unique_ptr<CFGGraph> generateCFG(const clang::FunctionDecl* FD) {
        ...
        // Store ASTContext for use in block processing
