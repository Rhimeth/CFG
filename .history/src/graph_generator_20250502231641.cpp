...
namespace GraphGenerator {
    ...
    void extractStatementsFromBlock(const clang::CFGBlock* block, CFGGraph* graph, const QString& filename) {
        if (!block || !graph) return;
    
        // Get the ASTContext from the first statement in the block
        const clang::Stmt* firstStmt = nullptr;
        for (const auto& element : *block) {
            if (element.getKind() == clang::CFGElement::Statement) {
                firstStmt = element.castAs<clang::CFGStmt>().getStmt();
                break;
            }
        }
        ...
        // Get the ASTContext through the statement's parent DeclContext
        const clang::DeclContext* DC = firstStmt->getParentFunctionOrMethod();
        if (!DC) return;
        
        const clang::ASTContext& astContext = DC->getParentASTContext();
    ...
    std::unique_ptr<CFGGraph> generateCFG(const clang::FunctionDecl* FD) {
        ...
        for (const auto* block : *cfg) {
            ...
            extractStatementsFromBlock(block, graph.get(), filename);
