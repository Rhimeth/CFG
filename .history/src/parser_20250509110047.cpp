...
std::vector<Parser::FunctionCFG> Parser::extractAllCFGs(const std::string& filePath) {
    ...
    // 2. Parse with proper lifetime management
    auto astUnit = parseFileWithAST(filePath);
    if (!astUnit) return cfgs;
    ...
    try {
        ...
        // 3. Process each function with scope-limited CFGs
        for (const auto& funcInfo : visitor.getFunctions()) {
            ...
            // 4. Build and process CFG in a limited scope
            {
                ...
                processCFGBlocks(*cfgPtr, cfg, context);
                processCFGEdges(*cfgPtr, cfg);
            } // cfgPtr freed here
            
            cfgs.push_back(std::move(cfg));
        }
    } catch (...) {
        qCritical() << "Exception during CFG extraction";
    }
    
    return cfgs;
}

// Add these helper functions for extractAllCFGs
void Parser::processCFGBlocks(const clang::CFG& cfg, FunctionCFG& functionCFG, clang::ASTContext* context) {
    for (const auto* block : cfg) {
        ...
        if (block->empty()) {
            ...
            for (const auto& elem : *block) {
                if (elem.getKind() == clang::CFGElement::Statement) {
                    ...
                    stmt->printPretty(os, nullptr, clang::PrintingPolicy(context->getLangOpts()));
                    os.flush();
        ...
        if (!block->empty() && block->front().getKind() == clang::CFGElement::Statement) {
            const clang::Stmt* stmt = block->front().castAs<clang::CFGStmt>().getStmt();
        ...
        functionCFG.nodes.push_back(node);
    }
}

void Parser::processCFGEdges(const clang::CFG& cfg, FunctionCFG& functionCFG) {
    for (const auto* block : cfg) {
        if (!block) continue;
        ...
        for (auto it = block->succ_begin(); it != block->succ_end(); ++it) {
            if (const clang::CFGBlock* succ = *it) {
                CFGEdge edge;
                edge.sourceId = sourceId;
                edge.targetId = succ->getBlockID();
                
                // Label edges from conditional blocks
                ...
                functionCFG.edges.push_back(edge);
                edgeIdx++;
            }
        }
    }
...
clang::ASTContext* Parser::ThreadLocalState::parse(const std::string& filePath) {
    ...
    try {
        consumer.reset();
        compiler.reset(new clang::CompilerInstance());
        setupCompiler();
        
        // Create a fresh consumer
        consumer = std::make_unique<ASTStoringConsumer>();
        
        // Don't access Context yet - it's not initialized
        ...
        cArgs.reserve(args.size()); // Preallocate to prevent reallocation
        ...
        // Use a separate ignoring diagnostic consumer that we create and own
        auto diagConsumer = new clang::IgnoringDiagConsumer();
        DiagnosticsEngine& diags = compiler->getDiagnostics();
        diags.setClient(diagConsumer, true); // Transfer ownership
        ...
        // Transfer ownership of the consumer to the compiler BEFORE execution
        ...
        // Now we can safely get the context AFTER the action is executed
        if (compiler && compiler->hasASTContext()) {
            return &compiler->getASTContext();
        } else {
            qCritical() << "AST Context not available after parsing";
            return nullptr;
        }
    } catch (const std::exception& e) {
...
void Parser::ThreadLocalState::setupCompiler() {
    try {
        if (!compiler) {
            qCritical() << "Compiler instance is null during setup";
            throw std::runtime_error("Null compiler instance");
        }
        
        compiler->createDiagnostics();
        if (!compiler->hasDiagnostics()) {
            throw std::runtime_error("Failed to create diagnostics");
        }
        ...
        if (!compiler->hasSourceManager()) {
            throw std::runtime_error("Failed to create source manager");
        }
        
        compiler->createPreprocessor(TU_Complete);
        if (!compiler->hasPreprocessor()) {
            throw std::runtime_error("Failed to create preprocessor");
        }
        
        compiler->createASTContext();
        if (!compiler->hasASTContext()) {
            throw std::runtime_error("Failed to create AST context");
        }
...
Parser::ThreadLocalState::~ThreadLocalState() {
    try {
        // We no longer need to detach the AST context from our consumer
        // since we're getting it directly from the compiler
        
        // Reset our consumer pointer since we no longer need it
        consumer.reset();
        
        if (compiler) {
            // Clean up the diagnostic client first
            if (compiler->hasDiagnostics()) {
                auto& diags = compiler->getDiagnostics();
                if (diags.hasSourceManager() && diags.ownsClient()) {
                    // Replace with dummy consumer to prevent access to deleted memory
            ...
            // Release all resources in reverse order of creation - use separate try blocks
            // so that one failure doesn't prevent cleaning up other resources
            try {
                if (compiler->hasASTContext())
...
// Replace the global ASTContext vector with a vector of ThreadLocalState objects
static std::vector<std::unique_ptr<Parser::ThreadLocalState>> g_parserStates;
static std::mutex g_parserMutex;
...
clang::ASTContext* Parser::parseFile(const std::string& filePath) {
    ...
    try {
        // Create a persistent state for parsing and store it
        std::lock_guard<std::mutex> lock(g_parserMutex);
        g_parserStates.push_back(std::make_unique<ThreadLocalState>());
        
        // Parse file using our persistent state
        auto& state = g_parserStates.back();
        auto contextPtr = state->parse(filePath);
        
        if (!contextPtr) {
            qCritical() << "Failed to parse file:" << filePath.c_str();
            g_parserStates.pop_back();  // Remove failed state
            return nullptr;
        }
        
        // Return the context - it stays alive because we're keeping the state
        return contextPtr;
    } catch (const std::exception& e) {
...
// Fix the AST consumer to properly initialize the context
void Parser::ASTStoringConsumer::Initialize(clang::ASTContext &Ctx) {
    Context = &Ctx;
}