...
CFGAnalyzer::AnalysisResult CFGAnalyzer::analyzeFiles(const std::vector<std::string>& filePaths) {
    ...
    if (filePaths.empty()) {
        result.success = false;
        result.report = "No files provided for analysis";
        return result;
    }
    ...
    for (const auto& filePath : filePaths) {
        if (!std::filesystem::exists(filePath)) {
            result.success = false;
            result.report = "File not found: " + filePath;
            return result;
        }
    }
    
    try {
        // Generate a combined CFG for all files
        auto graph = GraphGenerator::generateCFG(filePaths);
        if (!graph || graph->getNodeCount() == 0) {
            result.success = false;
            result.report = "Failed to generate control flow graph for the files";
            return result;
        }
        
        // Generate DOT representation
        std::string dotOutput = GraphGenerator::generateValidDot(graph.get());
        
        // Build report
        std::stringstream report;
        report << "Multi-file Analysis Results\n";
        report << "==========================\n";
        report << "Files: " << filePaths.size() << "\n";
        report << "Nodes: " << graph->getNodeCount() << "\n";
        report << "Edges: " << graph->getEdgeCount() << "\n\n";
        
        // Add file-specific information
        for (size_t i = 0; i < filePaths.size(); i++) {
            report << "File " << (i+1) << ": " << filePaths[i] << "\n";
        }
        
        result.success = true;
        result.report = report.str();
        result.dotOutput = dotOutput;
