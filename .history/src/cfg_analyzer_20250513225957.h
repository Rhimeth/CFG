...
private:
    ...
    std::unordered_map<std::string, std::set<std::string>> FunctionDependencies;
    std::unique_ptr<FunctionCallVisitor> m_functionCallVisitor;
...
private:
    ...
    int countFunctionCalls(const std::unordered_map<std::string, std::set<std::string>>& dependencies) const; // Added 'const' qualifier
};
