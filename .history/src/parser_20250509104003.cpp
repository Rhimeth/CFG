...
void Parser::ThreadLocalState::setupCompiler() {
    try {
        // Using more defensive initialization with null checks
        if (!compiler) {
...
// Keep ASTContext objects alive through program execution
static std::vector<std::unique_ptr<clang::ASTContext>> g_astContexts;
static std::mutex g_astContextMutex;
...
clang::ASTContext* Parser::parseFile(const std::string& filePath) {
    ...
    try {
        // Create a new state for parsing
        ...
        // Create a new ASTContext object that will stay alive
        std::lock_guard<std::mutex> lock(g_astContextMutex);
        g_astContexts.push_back(std::unique_ptr<clang::ASTContext>(new clang::ASTContext(
            contextPtr->getLangOpts(),
            contextPtr->getSourceManager(),
            contextPtr->getIdentifierTable(),
            contextPtr->getSelectorTable(),
            contextPtr->getBuiltinTypes()
        )));
        
        // Clone the relevant parts of the AST
        g_astContexts.back()->getTranslationUnitDecl()->addDecl(
            contextPtr->getTranslationUnitDecl());
        
        // Return the permanent ASTContext
        return g_astContexts.back().get();
    } catch (const std::exception& e) {
