...
AnalysisResult CFGAnalyzer::analyzeFile(const QString& filePath) {
    ...
    try {
        if (filePath.isEmpty()) {
            result.report = "Empty file path provided";
            return result;
        }
        
        QFileInfo fileInfo(filePath);
        if (!fileInfo.exists() || !fileInfo.isReadable()) {
            result.report = "File does not exist or cannot be read: " + filePath.toStdString();
            return result;
        }
        
        if (fileInfo.size() > 10 * 1024 * 1024) {
            result.report = "File too large (over 10MB), limit exceeded";
            return result;
        }
        ...
        // Analyze the file once and store the result directly
        ...
        try {
            ...
            for (const auto& funcPair : result.functions) {
                const auto& func = funcPair.first;
                const auto& funcInfo = funcPair.second;
                
                // Skip invalid functions
                if (func.empty() || funcInfo.name.empty()) continue;
                ...
                json function = {
                    ...
                    {"isConstructor", funcInfo.isConstructor},
                    {"isDestructor", funcInfo.isDestructor},
                    {"calls", json::array()}
                };
                
                auto depIt = result.functionDependencies.find(func);
                if (depIt != result.functionDependencies.end()) {
                    std::vector<std::string> calleeList(depIt->second.begin(), depIt->second.end());
                    function["calls"] = calleeList;
                }
    ...
    // Safety checks on output sizes
    assert(result.dotOutput.size() < 1000000);
    assert(result.jsonOutput.size() < 1000000);
...
AnalysisResult CFGAnalyzer::analyze(const std::string& filePath) {
    ...
    try {
        ...
        // Setup command-line arguments for clang tool
        std::vector<std::string> args = {"-fsyntax-only", filePath};
        ...
        llvm::cl::OptionCategory category("CFG Analysis");
        auto expectedParser = clang::tooling::CommonOptionsParser::create(argc, argv, category);
        if (!expectedParser) {
            auto err = expectedParser.takeError();
            result.report = "Failed to create CommonOptionsParser: " + llvm::toString(std::move(err));
            delete[] argv;
            return result;
        }
        ...
        // Create our action directly and run it instead of using newFrontendActionFactory
        CFGAction action(outputDir, result);
        int toolResult = tool.run(new clang::tooling::FrontendActionFactory([&action]() -> clang::FrontendAction* {
            return &action;
        }));
