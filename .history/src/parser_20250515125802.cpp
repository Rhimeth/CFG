...
void Parser::ASTStoringConsumer::Initialize(clang::ASTContext& Ctx) {
    Context = &Ctx;  // Store the AST context for later use
    // This method initializes the consumer with the provided AST context
    // The context will be used during HandleTranslationUnit
}

// FunctionVisitor class methods (lines ~21-60)
bool Parser::FunctionVisitor::VisitFunctionDecl(FunctionDecl* decl) {
    // Line ~27
    // Extracts function declarations from AST
    // Captures function names, source locations, and stores function declarations
    // ...existing code...
}

// Core function extraction (lines ~62-87)
std::vector<Parser::FunctionInfo> Parser::extractFunctions(const std::string& filePath) {
    // Line ~62
    // Entry point for function extraction
    // Parses file and traverses AST to collect function metadata
    // ...existing code...
}

// CFG extraction (lines ~89-194)
std::vector<Parser::FunctionCFG> Parser::extractAllCFGs(const std::string& filePath) {
    // Line ~89
    // Main CFG extraction function
    // Builds control flow graphs for each function in the file
    // ...existing code...
}

// DOT generation (lines ~196-250)
std::string Parser::generateDOT(const FunctionCFG& cfg) {
    // Line ~196
    // Transforms CFG to DOT graph format for visualization
    // Formats nodes, edges, and handles special styling
    // ...existing code...
}

// AST parsing utilities (lines ~252-327)
std::unique_ptr<clang::ASTUnit> Parser::parseFileWithAST(const std::string& filename) {
    // Line ~252
    // Alternative parsing method using Clang's buildASTFromCodeWithArgs
    // Configures compilation parameters and diagnostics
    // ...existing code...
}

// Thread-local parsing state (lines ~329-388)
clang::ASTContext* Parser::ThreadLocalState::parse(const std::string& filePath) {
    // Line ~329
    // Thread-safe parsing implementation
    // Sets up compiler instance and executes parsing action
    // ...existing code...
}

// Compiler setup (lines ~390-404)
void Parser::ThreadLocalState::setupCompiler() {
    // Line ~390
    // Initializes Clang compiler components
    // Creates diagnostics, file manager, source manager, etc.
    // ...existing code...
}

// Parser lifecycle methods (lines ~406-415)
Parser::Parser() {
    // Line ~406
...
// Primary parsing method (lines ~417-425)
clang::ASTContext* Parser::parseFile(const std::string& filePath) {
    // Line ~417
    // Main entry point for file parsing
    // Uses thread-local state for efficient parsing
    // ...existing code...
}

// AST consumer methods (lines ~427-431)
void Parser::ASTStoringConsumer::Initialize(clang::ASTContext& Ctx) {
    // Line ~427
    // Stores AST context for later use in processing
    // ...existing code...
}