...
CFGAnalyzer::AnalysisResult CFGAnalyzer::analyze(const std::string& filename) {
    ...
    public:
        CFGActionFactory(CFGAnalyzer::AnalysisResult& results) : m_results(results) {}
        
        std::unique_ptr<clang::FrontendAction> create() override {
            // Convert string literal to std::string to match constructor parameter
            std::string outputDir = "cfg_output";
            return std::make_unique<CFGAction>(outputDir, m_results);
        }
        
    private:
        // Fix: Ensure the member variable has the correct type
...
CFGAnalyzer::AnalysisResult CFGAnalyzer::analyzeMultipleFiles(
...
CFGAnalyzer::AnalysisResult CFGAnalyzer::analyzeFile(const QString& filePath) {
    AnalysisResult result;
    try {
...
CFGAnalyzer::AnalysisResult CFGAnalyzer::analyzeFiles(const std::vector<std::string>& filePaths) {
    if (filePaths.empty()) {
        return AnalysisResult(false, "", "No files to analyze");
    }
    
    // If only one file, use the single-file analysis
    if (filePaths.size() == 1) {
    ...
    for (const auto& filePath : filePaths) {
        ...
        if (result.success) {
            // Extract the content from the DOT output
...
// Helper function to filter DOT content for inclusion in a combined graph
