...
// Remove the namespace and properly scope ASTStoringConsumer to Parser class
Parser::ASTStoringConsumer::ASTStoringConsumer()
    : m_ast(nullptr)
{
}

void Parser::ASTStoringConsumer::HandleTranslationUnit(clang::ASTContext& Context)
{
    ...
    if (m_ast) {
        clang::TranslationUnitDecl* tuDecl = m_ast->getTranslationUnitDecl();
        
        // Collect all function declarations
        for (auto it = tuDecl->decls_begin(); it != tuDecl->decls_end(); ++it) {
            if (auto* funcDecl = llvm::dyn_cast<clang::FunctionDecl>(*it)) {
                FunctionInfo info;
                info.name = funcDecl->getNameAsString();
                
                // Get source location information
                clang::SourceManager& SM = m_ast->getSourceManager();
                ...
                if (loc.isValid()) {
                    ...
                    if (presumedLoc.isValid()) {
                        ...
                        info.column = presumedLoc.getColumn();
                    }
                }
                
                // Add function definition status
                info.hasBody = funcDecl->hasBody();
                
                // Store the function declaration for later reference
                m_functions.push_back(info);
                m_functionDecls[info.name] = funcDecl;
            }
        }
    }
}