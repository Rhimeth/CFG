...
// Improved visitor to capture function calls with more detailed AST information
...
public:
    ...
    bool VisitFunctionDecl(clang::FunctionDecl* FD) {
        ...
        // Store function declaration details
        FunctionInfo info;
        info.name = funcName;
        info.isMethod = llvm::isa<clang::CXXMethodDecl>(FD);
        info.isConstructor = FD->isConstructor();
        info.isDestructor = FD->isDestructor();
        ...
        if (!m_currentFunction.empty() && CE) {
            if (auto* CalledFunc = CE->getDirectCallee()) {
                ...
                if (m_functions.find(calleeName) == m_functions.end()) {
                    ...
                    calleeInfo.isConstructor = CalledFunc->isConstructor();
                    calleeInfo.isDestructor = CalledFunc->isDestructor();
