...
public:
    ...
    bool VisitFunctionDecl(clang::FunctionDecl* FD) {
        ...
        // Use the FunctionInfo from CFGAnalyzer namespace
        CFGAnalyzer::FunctionInfo info;
        info.name = funcName;
    ...
    bool VisitCallExpr(clang::CallExpr* CE) {
        if (!m_currentFunction.empty() && CE) {
            if (auto* CalledFunc = CE->getDirectCallee()) {
                ...
                // Also store the callee in our function list if not already present
                if (m_functions.find(calleeName) == m_functions.end()) {
                    CFGAnalyzer::FunctionInfo calleeInfo;
    ...
    std::map<std::string, CFGAnalyzer::FunctionInfo> getFunctions() const {
        return m_functions;
    }
...
private:
    ...
    std::map<std::string, CFGAnalyzer::FunctionInfo> m_functions;
};
...
void CFGVisitor::FinalizeCombinedFile() {
    ...
    // Debug check
    if (FunctionDependencies.empty()) {
        llvm::errs() << "Warning: Empty FunctionDependencies\n";
    }
    
    m_results.functionDependencies = std::move(FunctionDependencies);
    
    // Post-move check
    assert(FunctionDependencies.empty() && "Move should leave source empty");
}
