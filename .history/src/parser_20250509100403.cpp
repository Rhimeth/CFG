...
clang::ASTContext* Parser::ThreadLocalState::parse(const std::string& filePath) {
    ...
    try {
        // Create a completely new consumer and compiler for each parse operation
        // to avoid memory issues between different parsing operations
        consumer.reset();
        ...
        // Store a raw pointer to Context before we transfer ownership of the consumer
        ASTContext* contextPtr = nullptr;
        ...
        // Get the context pointer before transferring ownership
        contextPtr = consumer->Context;
        
        // Transfer ownership of the consumer to the compiler
        compiler->setASTConsumer(std::move(consumer));
        
        // Keep a local strong reference to prevent early destruction
        auto localCompiler = compiler;
        
        try {
            SyntaxOnlyAction action;
            if (!localCompiler->ExecuteAction(action)) {
        ...
        // Return context but don't alter its ownership
        return contextPtr;
    } catch (const std::exception& e) {
...
Parser::ThreadLocalState::~ThreadLocalState() {
    try {
        // First detach the AST context from our consumer to prevent double-free
        if (consumer) {
            consumer->Context = nullptr;
        }
        
        // Then reset our consumer pointer since we no longer need it
        consumer.reset();
