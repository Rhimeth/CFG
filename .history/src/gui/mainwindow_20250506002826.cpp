...
void MainWindow::displayGraph(const QString& dotContent, bool isProgressive, int rootNode) 
{
    ...
    m_currentDotContent = dotContent.toStdString();
    
    // For progressive display, generate modified DOT
    std::string processedDot = isProgressive ? 
        generateProgressiveDot(m_currentDotContent, rootNode) : 
        m_currentDotContent;

    // Escape for JavaScript (convert to QString first)
    std::string escapedDot = escapeDotLabel(QString::fromStdString(processedDot));
...
std::string MainWindow::generateProgressiveDot(const std::string& fullDot, int rootNode) 
{
    ...
    QString qFullDot = QString::fromStdString(fullDot);
    auto matches = edgeRegex.globalMatch(qFullDot);
    ...
    matches = nodeRegex.globalMatch(qFullDot);
    ...
    for (auto it = adjacencyList.begin(); it != adjacencyList.end(); ++it) {
        ...
        for (int to : it.value()) {
            if (m_visibleNodes[to]) {
                ...
                output << "  node" << to << " [label=\"+\", shape=ellipse, fillcolor=\"#9E9E9E\"];\n";
                output << "  node" << from << " -> node" << to << " [style=dashed, color=gray];\n";
            }
        }
    }
...
Q_INVOKABLE void MainWindow::expandNode(const QString& nodeIdStr) {
    ...
    displayGraph(QString::fromStdString(m_currentDotContent), true, m_currentRootNode);};
...
void MainWindow::onNodeClicked(const QString& nodeId) {
    ...
    // First convert graph to DOT string
    std::string dotContent = generateValidDot(m_currentGraph);
    
    // Then generate progressive DOT
    std::string progressiveDot = generateProgressiveDot(dotContent, id);
    
    // Display it
    displayGraph(QString::fromStdString(progressiveDot));
};
