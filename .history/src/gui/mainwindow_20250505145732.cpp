...
std::shared_ptr<GraphGenerator::CFGGraph> MainWindow::parseDotToCFG(const QString& dotContent) {
    // Create a graph on the heap for safer memory handling
    auto graph = std::make_shared<GraphGenerator::CFGGraph>();
    
    if (dotContent.isEmpty()) {
        qWarning() << "Empty DOT content provided to parseDotToCFG";
        return graph;
    }
    ...
    try {
        ...
        if (!checkRegex(nodeRegex, "node") || !checkRegex(edgeRegex, "edge") ||
            ...
            qWarning() << "Invalid regex patterns in parseDotToCFG";
            return graph;
        }
        ...
        if (lines.isEmpty()) {
            qWarning() << "No content in DOT file after splitting";
            return graph;
        }
        ...
        for (const QString& line : lines) {
            ...
            if (nodeMatch.hasMatch()) {
                ...
                // Skip reserved keywords and invalid node IDs
                QStringList reservedWords = {"node", "edge", "graph", "digraph", "subgraph"};
                if (reservedWords.contains(nodeIdStr, Qt::CaseInsensitive)) {
                    qDebug() << "Skipping reserved word as node ID:" << nodeIdStr;
                    continue;
                }
                
                // Skip node IDs that already start with "node" prefix (likely from DOT visualization output)
                if (nodeIdStr.startsWith("node", Qt::CaseInsensitive)) {
                    qDebug() << "Skipping node ID with 'node' prefix:" << nodeIdStr;
                    continue;
                }
                
                // Try to extract numeric ID - handle different formats
                bool ok = false;
                int id = 0;
                
                if (nodeIdStr.startsWith("B") || nodeIdStr.startsWith("b")) {
                    // Handle "Bxx" format (possible block number)
                    id = nodeIdStr.mid(1).toInt(&ok);
                } else if (nodeIdStr.startsWith("N") || nodeIdStr.startsWith("n")) {
                    // Handle "Nxx" format (possible node number)
                    id = nodeIdStr.mid(1).toInt(&ok);
                } else {
                    // Try direct conversion
                    id = nodeIdStr.toInt(&ok);
                }
                
                if (!ok) {
                    qDebug() << "Could not convert node ID to integer, skipping:" << nodeIdStr;
                    continue;
                }
        ...
        // Check if any nodes were actually found
        if (!nodesFound || graph->getNodes().empty()) {
            qWarning() << "No valid nodes found in DOT content - content may be invalid";
            return std::make_shared<GraphGenerator::CFGGraph>();  // Return a fresh, empty graph
        }
        ...
        for (const QString& line : lines) {   
            ...
            if (edgeMatch.hasMatch()) {
                ...
                // Skip edges that refer to reserved words
                QStringList reservedWords = {"node", "edge", "graph", "digraph", "subgraph"};
                if (reservedWords.contains(fromStr, Qt::CaseInsensitive) || 
                    reservedWords.contains(toStr, Qt::CaseInsensitive)) {
                    qDebug() << "Skipping edge with reserved word as endpoint:" << fromStr << "->" << toStr;
                    continue;
                }
                
                // Skip edges that use node prefixes
                if (fromStr.startsWith("node", Qt::CaseInsensitive) || 
                    toStr.startsWith("node", Qt::CaseInsensitive)) {
                    qDebug() << "Skipping edge with 'node' prefix:" << fromStr << "->" << toStr;
                    continue;
                }
                ...
                // Handle different ID formats for source node
                if (fromStr.startsWith("B") || fromStr.startsWith("b")) {
                    fromId = fromStr.mid(1).toInt(&ok1);
                } else if (fromStr.startsWith("N") || fromStr.startsWith("n")) {
                    fromId = fromStr.mid(1).toInt(&ok1);
                } else {
                    fromId = fromStr.toInt(&ok1);
                }
                
                // Handle different ID formats for target node
                if (toStr.startsWith("B") || toStr.startsWith("b")) {
                    toId = toStr.mid(1).toInt(&ok2);
                } else if (toStr.startsWith("N") || toStr.startsWith("n")) {
                    toId = toStr.mid(1).toInt(&ok2);
                } else {
                    toId = toStr.toInt(&ok2);
                }
                
                if (!ok1 || !ok2) {
                    qDebug() << "Invalid edge references, skipping:" << fromStr << "->" << toStr;
                    continue;
                }
                
                QString edgeAttrs = edgeMatch.captured(3);
                
                // Make sure nodes exist before adding edges
                const auto& nodes = graph->getNodes();
                if (nodes.find(fromId) == nodes.end()) {
                    graph->addNode(fromId);
                    qDebug() << "Added missing source node:" << fromId;
                }
                
                if (nodes.find(toId) == nodes.end()) {
                    graph->addNode(toId);
                    qDebug() << "Added missing target node:" << toId;
                }
                
                // Add the edge and parse its attributes
                graph->addEdge(fromId, toId);
                qDebug() << "Added edge:" << fromId << "->" << toId;
        ...
        if (graph->getNodes().empty()) {
            qWarning() << "Graph has no nodes after processing - data may be invalid";
            return std::make_shared<GraphGenerator::CFGGraph>();
        }

        // Third pass: build node information map (only if we have a valid graph)
        const auto& nodes = graph->getNodes();
        for (const auto& pair : nodes) {
            ...
            // Process node statements and other attributes
            // ...existing code...
        }
        
        qDebug() << "Successfully parsed CFG with" << nodes.size() << "nodes";
    }
    catch (const std::exception& e) {
        qCritical() << "Exception in parseDotToCFG:" << e.what();
        return std::make_shared<GraphGenerator::CFGGraph>();  // Return a clean graph on error
    }
    catch (...) {
        qCritical() << "Unknown exception in parseDotToCFG";
        return std::make_shared<GraphGenerator::CFGGraph>();  // Return a clean graph on error
    }
    
    return graph; // Return the successfully parsed graph
};
