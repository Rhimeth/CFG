...
void MainWindow::highlightNodeInCodeEditor(int nodeId) {
    // Verify we're in the GUI thread
    Q_ASSERT(QThread::currentThread() == QApplication::instance()->thread());
    
    if (!m_nodeInfoMap.contains(nodeId)) {
        qDebug() << "No info for node" << nodeId;
        return;
    }
    
    const NodeInfo& info = m_nodeInfoMap[nodeId];
    
    // Verify valid line numbers
    int totalLines = ui->codeEditor->document()->blockCount();
    if (info.startLine <= 0 || info.endLine > totalLines || info.startLine > info.endLine) {
        qWarning() << "Invalid line numbers for node" << nodeId << ":" << info.startLine << "-" << info.endLine;
        return;
    }
    ...
    // Main block highlight (yellow)
    QTextCursor blockCursor(doc);
    blockCursor.setPosition(doc->findBlockByNumber(info.startLine - 1).position());
    blockCursor.movePosition(QTextCursor::Down, QTextCursor::KeepAnchor, 
                           info.endLine - info.startLine + 1);
    
    QTextEdit::ExtraSelection blockSelection;
    blockSelection.format.setBackground(QColor(255, 255, 150, 100)); // Semi-transparent
    blockSelection.format.setProperty(QTextFormat::FullWidthSelection, true);
    blockSelection.cursor = blockCursor;
    extraSelections.append(blockSelection);
    
    // Start line marker (green)
    QTextCursor startCursor(doc->findBlockByNumber(info.startLine - 1));
    QTextEdit::ExtraSelection startSelection;
    ...
    // End line marker (red, if different)
    if (info.startLine != info.endLine) {
        QTextCursor endCursor(doc->findBlockByNumber(info.endLine - 1));
        QTextEdit::ExtraSelection endSelection;
    ...
    // Scroll to show the node with some context
    int contextLines = 3;
    int scrollToLine = qMax(1, info.startLine - contextLines);
    ...
    // Visual feedback
    statusBar()->showMessage(
        QString("Node %1: Lines %2-%3").arg(nodeId).arg(info.startLine).arg(info.endLine), 
        3000
    );
};
