    ...
    m_recentFiles()
{
    ...
    // Add file list widget and buttons
    ...
    m_fileListWidget->setVisible(false); // Initially hidden
    
    QHBoxLayout* fileButtonsLayout = new QHBoxLayout();
    m_addFileButton = new QPushButton("Add File", this);
    m_removeFileButton = new QPushButton("Remove File", this);
    m_toggleFileListButton = new QPushButton("Multiple Files", this);
    m_toggleFileListButton->setCheckable(true);
    
    fileButtonsLayout->addWidget(m_toggleFileListButton);
    fileButtonsLayout->addWidget(m_addFileButton);
    fileButtonsLayout->addWidget(m_removeFileButton);
    fileButtonsLayout->addStretch();
    
    // Insert widgets into layout
    QLayout* centralLayout = ui->centralwidget->layout();
    if (QVBoxLayout* vLayout = qobject_cast<QVBoxLayout*>(centralLayout)) {
        vLayout->insertWidget(2, m_fileListWidget);
        vLayout->insertLayout(2, fileButtonsLayout);
    }
    
    // Connect file list buttons
    connect(m_toggleFileListButton, &QPushButton::toggled, [this](bool checked) {
        m_fileListWidget->setVisible(checked);
        m_addFileButton->setVisible(checked);
        m_removeFileButton->setVisible(checked);
        ui->analyzeButton->setText(checked ? "Analyze All" : "Analyze");
    });
    
    connect(m_addFileButton, &QPushButton::clicked, this, &MainWindow::onAddFileClicked);
    connect(m_removeFileButton, &QPushButton::clicked, this, &MainWindow::onRemoveFileClicked);
