...
std::shared_ptr<GraphGenerator::CFGGraph> MainWindow::parseDotToCFG(const QString& dotContent) {
    // Create a new graph with safety checks
    ...
    try {
        // Regular expressions for parsing DOT file with improved patterns
        QRegularExpression nodeRegex(R"(^\s*(\w+)\s*\[([^\]]*)\]\s*;?\s*$)");
        QRegularExpression edgeRegex(R"(^\s*(\w+)\s*(->|--)\s*(\w+)\s*(\[[^\]]*\])?\s*;?\s*$)");
        QRegularExpression labelRegex(R"(label\s*=\s*(?:"([^"]*)"|([^][\s;,]+)))");
        QRegularExpression locRegex(R"(location\s*=\s*"([^:]+):(\d+)-(\d+))");
        QRegularExpression colorRegex(R"(color\s*=\s*"?(red|blue|green|black|white|gray|#[\da-fA-F]{6})"?)");
        QRegularExpression shapeRegex(R"(shape\s*=\s*"?(box|ellipse|diamond|circle|record|plaintext|Mrecord)"?)");
        QRegularExpression fillcolorRegex(R"(fillcolor\s*=\s*"?(lightblue|lightgray|lightgreen|lightpink|#[\da-fA-F]{6})"?)");
        QRegularExpression functionRegex(R"(function\s*=\s*(?:"([^"]*)"|([^][\s;,]+)))");
        QRegularExpression styleRegex(R"(style\s*=\s*(?:"([^"]*)"|([^][\s;,]+)))");
        ...
        for (const QString& line : lines) {
            ...
            if (nodeMatch.hasMatch()) {
                ...
                // Handle node IDs - support both numeric and alphanumeric
                int id = -1;
                ...
                // Try to parse "B<number>" format
                else if (nodeIdStr.startsWith("B")) {
                    id = nodeIdStr.mid(1).toInt(&ok);
                    if (ok) isNumericId = true;
                }
                // Fallback to string hash if non-numeric
                if (!isNumericId) {
                    id = qHash(nodeIdStr);
                    qDebug() << "Non-numeric node ID converted to hash:" << nodeIdStr << "->" << id;
                }
                
                // Skip if we couldn't get any valid ID
                if (id == -1) {
                    qDebug() << "Could not determine node ID, skipping:" << nodeIdStr;
                    continue;
                }
                ...
                // Parse label (support both quoted and unquoted)
                auto labelMatch = labelRegex.match(attributes);
                if (labelMatch.hasMatch()) {
                    QString label = labelMatch.captured(1);
                    if (label.isEmpty()) label = labelMatch.captured(2);
                    ...
                    if (functionMatch.hasMatch()) {
                        ...
                        if (functionName.isEmpty()) functionName = functionMatch.captured(2);
                        graph->setNodeFunctionName(id, functionName);
                    }
                }
                ...
                auto styleMatch = styleRegex.match(attributes);
                
                // Check for try block (ellipse shape or specific style)
                if ((shapeMatch.hasMatch() && shapeMatch.captured(1) == "ellipse") ||
                    (styleMatch.hasMatch() && styleMatch.captured(1).contains("rounded"))) {
                    graph->markNodeAsTryBlock(id);
                }
                
                // Check for exception node (red color or specific fill)
                if ((colorMatch.hasMatch() && colorMatch.captured(1) == "red") ||
                    (fillMatch.hasMatch() && (fillMatch.captured(1) == "lightpink" || 
                                              fillMatch.captured(1).startsWith("#ff")))) {
        ...
        for (const QString& line : lines) {   
            ...
            if (edgeMatch.hasMatch()) {
                ...
                QString toStr = edgeMatch.captured(3);
                QString edgeAttrs = edgeMatch.captured(4);
                
                if (fromStr.isEmpty() || toStr.isEmpty()) {
                    qDebug() << "Empty node reference in edge, skipping";
                    continue;
                }
                
                // Convert node IDs using same logic as node parsing
                auto convertNodeId = [](const QString& idStr) -> int {
                    bool ok;
                    int id = idStr.toInt(&ok);
                    if (ok) return id;
                    if (idStr.startsWith("B")) {
                        return idStr.mid(1).toInt(&ok);
                    }
                    return qHash(idStr);
                };
                
                int fromId = convertNodeId(fromStr);
                int toId = convertNodeId(toStr);
                
                // Make sure both nodes exist - add if not found
                const auto& nodes = graph->getNodes();
                if (nodes.find(fromId) == nodes.end()) {
                    qDebug() << "Adding missing source node:" << fromId;
                    graph->addNode(fromId);
                }
                if (nodes.find(toId) == nodes.end()) {
                    qDebug() << "Adding missing target node:" << toId;
                    graph->addNode(toId);
                }
                ...
                if (graph->getNodes().find(fromId) != graph->getNodes().end() && 
                    ...
                    if (!edgeAttrs.isEmpty()) {
                        ...
                        auto styleMatch = styleRegex.match(edgeAttrs);
                        
                        // Exception edges are typically red and/or dashed
                        if ((colorMatch.hasMatch() && colorMatch.captured(1) == "red") ||
                            (styleMatch.hasMatch() && styleMatch.captured(1) == "dashed")) {
