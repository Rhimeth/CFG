...
void MainWindow::initializeComponents()
{
    ...
    // Load empty state with proper dimensions
    loadEmptyVisualization();
    
    // Set up graph view fallback
    if (!initializeGraphView()) {
        qWarning() << "Failed to initialize graph view, using text-only mode";
        startTextOnlyMode();
    }
...
void MainWindow::displayGraph(const QString& dotContent) {
    if (dotContent.isEmpty()) {
        ...
        webView->setHtml("<h1 style='color:red'>Error: Empty graph content</h1>");
        return;
    }
                ...
                viz.renderSVGElement(dot)
                    ...
                    .catch(err => {
                        ...
                        document.getElementById('graph-container').innerHTML = 
                            '<div class="error">Graph rendering failed: ' + err + '</div>';
                    });
            } catch(e) {
                document.getElementById('graph-container').innerHTML = 
                    '<div class="error">Initialization error: ' + e + '</div>';
                console.error(e);
            }
        }

        // Start rendering when resources are loaded
        if (typeof Viz !== 'undefined') {
            renderGraph();
        } else {
            document.getElementById('graph-container').innerHTML = 
                '<div class="error">Failed to load visualization library</div>';
        }
    </script>
</body>
</html>
    )").arg(dotContent.replace("`", "\\`").replace("$", "\\$"));

    webView->setHtml(html);
    
    // Debug output
    qDebug() << "Displaying graph with content size:" << dotContent.size();
}
...
void MainWindow::on_analyzeButton_clicked()
{
    ...
    try {
        ...
        if (!result.dotOutput.empty()) {
            ...
            qWarning() << "Empty DOT output from analyzer";
        }

    } catch (const std::exception& e) {
        QString error = QString("Failed to generate graph:\n%1\n"
                              "DOT content was: %2")
                      .arg(e.what())
                      .arg(QString::fromStdString(result.dotOutput));
        qCritical() << error;
        QMessageBox::critical(this, "Graph Error", error);
    }
