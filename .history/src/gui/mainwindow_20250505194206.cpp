...
std::shared_ptr<GraphGenerator::CFGGraph> MainWindow::parseDotToCFG(const QString& dotContent) {
    ...
    // Updated regex patterns for your DOT format
    static const QRegularExpression nodeRegex(R"("([^"]+)"\s*(?:\[[^\]]*\])?\s*;)");
    static const QRegularExpression edgeRegex(R"("([^"]+)"\s*->\s*"([^"]+)"\s*)");
    
    if (!nodeRegex.isValid()) {
        qCritical() << "Invalid node regex:" << nodeRegex.errorString();
        return graph;
    }
    if (!edgeRegex.isValid()) {
        qCritical() << "Invalid edge regex:" << edgeRegex.errorString();
        return graph;
    }
    ...
    try {
        ...
        for (const QString& line : lines) {
            ...
            // Skip graph declarations and style lines
            ...
            if (edgeMatch.hasMatch()) {
                ...
                if (!nodeNameToId.contains(fromName)) {
                    nodeNameToId[fromName] = nextId++;
                    graph->addNode(nodeNameToId[fromName], fromName);
                }
                if (!nodeNameToId.contains(toName)) {
                    nodeNameToId[toName] = nextId++;
                    graph->addNode(nodeNameToId[toName], toName);
                }
                
                graph->addEdge(nodeNameToId[fromName], nodeNameToId[toName]);
            }
        }

        if (graph->getNodes().empty()) {
            qWarning() << "No valid nodes found in:\n" << dotContent.left(500);
        } else {
            qDebug() << "Parsed CFG with" << graph->getNodes().size() << "nodes";
        }
    }
    catch (const std::exception& e) {
        qCritical() << "Parsing error:" << e.what();
    }
