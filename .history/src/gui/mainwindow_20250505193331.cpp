...
std::shared_ptr<GraphGenerator::CFGGraph> MainWindow::parseDotToCFG(const QString& dotContent) {
    ...
    // Initialize and validate regex patterns
    ...
    if (!nodeRegex.isValid() || !edgeRegex.isValid()) {
        qCritical() << "Invalid regex pattern";
        return graph;
    }
    ...
    try {
        ...
        for (const QString& line : lines) {
            ...
            if (nodeMatch.hasMatch()) {
                ...
                if (nodeIdStr.isEmpty()) continue;
                
                bool ok;
                int id = nodeIdStr.toInt(&ok);
                if (!ok) {
                    qWarning() << "Invalid node ID:" << nodeIdStr;
                    continue;
                }

                graph->addNode(id);
                m_nodeDetails[id] = parseNodeAttributes(nodeMatch.captured(2));
                continue;
            }
        ...
        if (!nodesFound) {
            ...
            qDebug() << "Parsed CFG with" << graph->getNodes().size() << "nodes and" 
