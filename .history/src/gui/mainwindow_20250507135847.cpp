    ...
    m_recentFiles()
{
    ...
    connect(this, &MainWindow::nodeClicked, this, &MainWindow::handleProgressiveNodeClick);
                ...
                new QWebChannel(qt.webChannelTransport, function(channel) {
                    ...
                    document.addEventListener('click', function(e) {
                        ...
                        if (node) {
                            ...
                            if (node.querySelector('ellipse') && node.textContent === '+') {
                                // This is an expandable node
                                window.mainWindow.handleProgressiveNodeClick(nodeId);
                            } else {
                                // Regular node click
                                window.mainWindow.onNodeClicked(nodeId);
                            }
                        }
                    });
                });
...
void MainWindow::displayGraph(const QString& dotContent, bool isProgressive, int rootNode) 
{
    ...
    // Check if web channel is ready
    if (!m_webChannelReady) {
        qDebug() << "Web channel not ready, storing content for later display";
        m_pendingDotContent = dotContent;
        m_pendingProgressive = isProgressive;
        m_pendingRootNode = rootNode;
        
        // Try to ensure the web channel gets initialized
            ...
            webView->page()->runJavaScript(
                ...
                try {
                    if (typeof QWebChannel !== "undefined") {
                        new QWebChannel(qt.webChannelTransport, function(channel) {
                            window.bridge = channel.objects.bridge;
                            console.log("Explicit WebChannel initialization");
                            if (window.bridge && typeof window.bridge.webChannelInitialized === "function") {
                                window.bridge.webChannelInitialized();
                            } else {
                                console.error("Bridge or init method not available");
                            }
                        });
                    } else {
                        console.error("QWebChannel not available yet");
                    }
                } catch(e) {
                    console.error("WebChannel init error:", e);
                }
                )"
            );
        });
...
<head>
    ...
    <style>
        ...
        .expandable-node { cursor:pointer; fill:#9E9E9E; }
    </style>
</head>
<body>
    <div id="graph-container"></div>
        ...
        function renderGraph(dot) {
            ...
            viz.renderSVGElement(dot)
                ...
                .catch(error => {
                    ...
                    document.getElementById("graph-container").innerHTML = 
                        "<p style='color:red'>Error rendering graph</p>";
                });
        }
    ...
    // Connect to loadFinished if not already connected
    static bool loadFinishedConnected = false;
    if (!loadFinishedConnected) {
        connect(webView, &QWebEngineView::loadFinished, [this](bool success) {
            if (success) {
...
{
    QMutexLocker locker(&m_graphvizMutex);  // Proper mutex locking
    ...
    try {
        ...
        QString html = generateInteractiveGraphHtml(dotContent);
...
QString MainWindow::generateInteractiveDot(std::shared_ptr<GraphGenerator::CFGGraph> graph)
{
    ...
    if (!graph) {
        stream << "digraph G {\n";
        stream << "    label=\"Empty Graph\";\n";
        stream << "    empty [shape=plaintext, label=\"No graph available\"];\n";
        stream << "}\n";
        return dot;
    }
    ...
    stream << "  size=\"12,12\";\n";
    stream << "  dpi=150;\n";
    stream << "  node [fontname=\"Arial\", fontsize=10];\n";
    stream << "  edge [fontname=\"Arial\", fontsize=8];\n\n";

    // Add nodes - ensure consistent format
    const auto& nodes = graph->getNodes();
    if (nodes.empty()) {
        stream << "  empty [shape=plaintext, label=\"Empty graph - no nodes found\"];\n";
    } else {
        for (const auto& [id, node] : nodes) {
            // Ensure we have a valid node ID and properly escaped label
            QString nodeLabel = !node.label.isEmpty() ? escapeDotLabel(node.label) : QString("Node %1").arg(id);
            stream << "  node" << id << " [label=\"" << nodeLabel << "\"";
            
            // Add styling attributes
            if (graph->isNodeTryBlock(id)) {
                stream << ", shape=ellipse, color=blue, style=filled, fillcolor=lightblue";
            } else if (graph->isNodeThrowingException(id)) {
                stream << ", color=red, style=filled, fillcolor=pink";
            } else {
                stream << ", shape=box, style=filled, fillcolor=white";
            }
            
            stream << "];\n";
        }

        // Add edges with consistent format
        for (const auto& [id, node] : nodes) {
            for (int successor : node.successors) {
                stream << "  node" << id << " -> node" << successor;
                
                // Style exception edges differently
    ...
    // Debug output to help diagnose issues
    qDebug() << "Generated DOT with" << nodes.size() << "nodes";
    qDebug() << "DOT preview:" << dot.left(200) << "...";
...
std::shared_ptr<GraphGenerator::CFGGraph> MainWindow::parseDotToCFG(const QString& dotContent) {
    ...
    if (!dotContent.contains("digraph") && !dotContent.contains("graph")) {
        qWarning("Invalid DOT format - missing graph declaration");
        return graph;
    }

    // Modified regex patterns with better node/edge detection
    static const QRegularExpression nodeRegex("\\s*(node)?(\\d+)\\s*\\[([^\\]]*)]");
    static const QRegularExpression edgeRegex("\\s*(node)?(\\d+)\\s*->\\s*(node)?(\\d+)(?:\\s*\\[([^\\]]*)])?");
    static const QRegularExpression attrRegex("(\\w+)\\s*=\\s*\"([^\"]*)\"");
    ...
    try {
        ...
        // Validate regex patterns
        if (!nodeRegex.isValid() || !edgeRegex.isValid() || !attrRegex.isValid()) {
            qCritical() << "Invalid regex pattern in parseDotToCFG";
            return graph;
        }
        ...
        for (const QString& line : lines) {
            ...
            if (trimmed.isEmpty() || trimmed.startsWith("//") || 
                trimmed.startsWith("#") || trimmed.startsWith("digraph") ||
                trimmed.startsWith("graph") || trimmed.startsWith("size") ||
                trimmed.startsWith("dpi") || trimmed.startsWith("rankdir") ||
                trimmed == "{" || trimmed == "}" || trimmed.startsWith("edge [") || 
            ...
            if (nodeMatch.hasMatch()) {
                QString nodePrefix = nodeMatch.captured(1).trimmed();  // "node" or empty
                QString nodeNumber = nodeMatch.captured(2).trimmed();  // Number part
                QString attributes = nodeMatch.captured(3).trimmed();  // Attributes
                
                qDebug() << "Found node:" << (nodePrefix.isEmpty() ? "" : nodePrefix) << nodeNumber;
                
                QString nodeName = (nodePrefix.isEmpty() ? "" : nodePrefix) + nodeNumber;
                
                // Generate node ID
                int nodeId = nodeNumber.toInt();
                if (nodeId == 0) nodeId = nextId++;  // Fallback if not a valid number
                nodeNameToId[nodeName] = nodeId;
                
                // Add node to the graph
                graph->addNode(nodeId, nodeName);
                graphHasNodes = true;
                nodeCount++;
                
                // Parse node attributes
                QRegularExpressionMatchIterator attrMatches = attrRegex.globalMatch(attributes);
                while (attrMatches.hasNext()) {
                    ...
                    if (key == "label") {
                        graph->addStatement(nodeId, value);
                    } else if (key == "shape" && value == "ellipse") {
                        graph->markNodeAsTryBlock(nodeId);
                    } else if ((key == "color" && value == "red") || 
                              (key == "fillcolor" && value.contains("pink"))) {
                        graph->markNodeAsThrowingException(nodeId);
                    }
                }
            } 
            else if (trimmed.contains("[") && trimmed.contains("]") && !trimmed.contains("->")) {
                // This might be a node in a different format - log for debugging
                qDebug() << "Potential node didn't match regex:" << trimmed;
            }
        }
        
        qDebug() << "First pass complete. Found" << nodeCount << "nodes";
        
        // Verify we found some nodes before proceeding
        if (!graphHasNodes) {
            qWarning() << "No valid nodes found in DOT content";
            // Sample and log some lines to help diagnose the format issue
        ...
        for (const QString& line : lines) {
            ...
            // Skip lines that definitely aren't edges
            ...
            if (edgeMatch.hasMatch()) {
                QString fromPrefix = edgeMatch.captured(1).trimmed();  // "node" or empty
                QString fromNumber = edgeMatch.captured(2).trimmed();  // Number part
                QString toPrefix = edgeMatch.captured(3).trimmed();    // "node" or empty
                QString toNumber = edgeMatch.captured(4).trimmed();    // Number part
                QString attributes = edgeMatch.hasMatch() && edgeMatch.lastCapturedIndex() >= 5 ? 
                    edgeMatch.captured(5).trimmed() : "";
                
                int fromId = fromNumber.toInt();
                int toId = toNumber.toInt();
                
                // Add edge if both nodes exist
                if (graph->getNodes().find(fromId) != graph->getNodes().end() && 
                    graph->getNodes().find(toId) != graph->getNodes().end()) {
                    graph->addEdge(fromId, toId);
                    edgeCount++;
                    
                    // Check for exception edges
                    if (!attributes.isEmpty()) {
                        if (attributes.contains("color=\"red\"") || attributes.contains("style=\"dashed\"")) {
                ...
                qDebug() << "Potential edge didn't match regex:" << trimmed;
            }
        }

        qDebug() << "Second pass complete. Found" << edgeCount << "edges";
        qDebug() << "Final graph has" << graph->getNodes().size() << "nodes";
    }
    catch (const std::exception& e) {
        qCritical() << "Error in parseDotToCFG:" << e.what();
        return std::make_shared<GraphGenerator::CFGGraph>(); // Return empty graph on error
    }
...
void MainWindow::onNodeClicked(const QString& nodeId) {
    bool ok;
    int id = nodeId.toInt(&ok);
    if (!ok || !m_currentGraph || !m_nodeInfoMap.contains(id)) return;

    // Get node information
    const NodeInfo& nodeInfo = m_nodeInfoMap[id];

    // 1. Display connection information
    ui->reportTextEdit->setPlainText(nodeInfo.getConnectionInfo());

    // 2. Highlight the node in the graph
    highlightNode(id, QColor(Qt::yellow));

    // 3. Load and highlight the corresponding code
    if (!nodeInfo.filePath.isEmpty()) {
        loadAndHighlightCode(nodeInfo.filePath, nodeInfo.startLine, nodeInfo.endLine);
    }

    // 4. Center on the node in the graph view
    centerOnNode(id);
};
...
void MainWindow::handleNodeClick(const QString& nodeId) {
    ...
    m_expandedNodes[id] = !m_expandedNodes[id];
    
    // Update the visualization
    webView->page()->runJavaScript(
        QString("toggleNodeExpansion(%1, %2);")
        .arg(id).arg(m_expandedNodes[id] ? "true" : "false")
    );
};
...
void MainWindow::highlightCodeSection(int startLine, int endLine) {
    ...
    cursor.movePosition(QTextCursor::EndOfBlock, QTextCursor::KeepAnchor, endLine - startLine + 1);

    QTextEdit::ExtraSelection selection;
    selection.format.setBackground(QColor(255, 255, 150)); // Light yellow
    selection.format.setProperty(QTextFormat::FullWidthSelection, true);
    selection.cursor = cursor;
    extraSelections.append(selection);

    // Highlight individual lines for better visibility
    for (int line = startLine; line <= endLine; ++line) {
        QTextEdit::ExtraSelection lineSelection;
        lineSelection.format.setBackground(QColor(255, 255, 180)); // Slightly lighter yellow
        lineSelection.format.setProperty(QTextFormat::FullWidthSelection, true);
        lineSelection.cursor = QTextCursor(ui->codeEditor->document()->findBlockByNumber(line - 1));
...
void MainWindow::loadFile(const QString& filePath)
{
    ...
    if (!fileInfo.isReadable()) {
        QMessageBox::critical(this, "Error", 
                            QString("No read permission for file:\n%1").arg(filePath));
        return;
    }
    ...
    if (fileInfo.size() > maxFileSize) {
        int ret = QMessageBox::question(this, "Large File",
                                      QString("File is large (%1 MB). Continue loading?")
    ...
    // Read content with progress feedback
    QTextStream in(&file);
    QString content;
    ...
    if (fileInfo.size() < 1024 * 1024) { // <1MB
    ...
    else { // For large files, read in chunks with progress
        ...
        while (!in.atEnd()) {
            ...
            progress.setValue((bytesRead * 100) / totalBytes);
            QCoreApplication::processEvents();
    ...
    if (content.isEmpty() && fileInfo.size() > 0) {
        QMessageBox::warning(this, "Warning", 
                           "File appears to be empty or could not be read properly");
    }
    ...
    if (!m_fileWatcher->files().contains(filePath)) {
        if (!m_fileWatcher->addPath(filePath)) {
            qWarning() << "Failed to add file to watcher:" << filePath;
        }
    }
    ...
    QString statusMessage = QString("Loaded: %1 (%2 KB)")
                          .arg(fileInfo.fileName())
                          .arg(fileInfo.size() / 1024);
    statusBar()->showMessage(statusMessage, 5000);

    // Auto-save the path in settings
    QSettings settings;
    settings.setValue("lastOpenedFile", filePath);

    // Emit signal for other parts of the application
