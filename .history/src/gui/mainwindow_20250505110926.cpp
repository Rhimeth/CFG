...
void MainWindow::setupWebView() {
    ...
    // Create web channel if it doesn't exist
    if (!m_webChannel) {
        m_webChannel = new QWebChannel(this);
    }

    // Register this object with all necessary methods
    m_webChannel->registerObject("mainWindow", this);
...
<head>
    ...
    <style>
        body { margin: 0; padding: 0; background-color: %1; }
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            new QWebChannel(qt.webChannelTransport, function(channel) {
                ...
                // Setup click handlers after channel is ready
                document.addEventListener('click', function(e) {
                    const node = e.target.closest('[id^="node"]');
                    if (node && mainWindow) {
                        const nodeId = node.id.replace('node', '');
                        node.classList.add('highlighted');
                        mainWindow.onNodeClicked(nodeId);
                    }
                    ...
                    if (edge && mainWindow) {
                        ...
                        if (parts.length === 2) {
                            mainWindow.onEdgeClicked(parts[0], parts[1]);
                        }
                    }
                });
                ...
                document.addEventListener('mousemove', function(e) {
                    ...
                    if (edge && mainWindow) {
                        const parts = edge.id.replace('edge', '').split('_');
                        if (parts.length === 2) {
                            mainWindow.onEdgeHovered(parts[0], parts[1]);
                        }
                    }
                });
            });
...
<body>
    ...
    connect(ui->webView, &QWebEngineView::loadFinished, [this](bool success) {
        if (!success) {
            ...
            ui->webView->setHtml("<div style='padding:20px;color:red'>Failed to load visualization</div>");
        }
    });
};
...
void MainWindow::onNodeClicked(const QString& nodeId)
{
    ...
    if (m_nodeInfoMap.contains(id)) {
        ...
        if (!info.filePath.isEmpty() && info.startLine > 0) {
...
void MainWindow::handleEdgeClick(const QString& fromId, const QString& toId) {
    qDebug() << "Edge clicked from" << fromId << "to" << toId;
