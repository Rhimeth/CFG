...
void MainWindow::onNodeClicked(const QString& nodeId) {
    ...
    auto it = nodes.find(id);
    if (it != nodes.end()) {
        // Display node details and highlight in code
        displayDetailedNodeInfo(id, it->second);
        highlightNodeInCodeEditor(id);
    }
}

void MainWindow::displayDetailedNodeInfo(int nodeId, const GraphGenerator::CFGNode& node) {
    // Create a rich HTML report
    QString html = "<html><body style='font-family: Arial; font-size: 10pt;'>";
    
    // Header with node ID
    html += QString("<h2>Node %1</h2>").arg(nodeId);
    ...
    if (!node.label.isEmpty()) {
        html += QString("<tr><td><b>Label:</b></td><td>%1</td></tr>")
                .arg(node.label.toHtmlEscaped());
    }
    
    // Function name
    if (!node.functionName.isEmpty()) {
        html += QString("<tr><td><b>Function:</b></td><td>%1</td></tr>")
                .arg(node.functionName.toHtmlEscaped());
    }
    
    // Source file and location
    if (!node.filename.isEmpty()) {
        html += QString("<tr><td><b>File:</b></td><td>%1</td></tr>")
                .arg(node.filename.toHtmlEscaped());
                
        if (node.startLine > 0) {
            html += QString("<tr><td><b>Lines:</b></td><td>%1").arg(node.startLine);
            
            if (node.endLine > node.startLine) {
                html += QString(" - %1").arg(node.endLine);
            }
    ...
    if (m_currentGraph->isNodeTryBlock(nodeId)) {
        html += "<tr><td><b>Type:</b></td><td><span style='color: blue;'>Try Block</span></td></tr>";
    }
    if (m_currentGraph->isNodeThrowingException(nodeId)) {
        html += "<tr><td><b>Type:</b></td><td><span style='color: red;'>Exception Throw</span></td></tr>";
    }
    ...
    // Incoming edges
    html += "<h4>Incoming Edges</h4><ul>";
    bool hasIncoming = false;
    
    for (const auto& [otherNodeId, otherNode] : nodes) {
        if (otherNode.successors.find(nodeId) != otherNode.successors.end()) {
            hasIncoming = true;
            QString type = m_currentGraph->isExceptionEdge(otherNodeId, nodeId) ? 
                " <span style='color: red;'>(exception)</span>" : "";
            html += QString("<li>From Node %1%2</li>").arg(otherNodeId).arg(type);
        }
    }
    
    if (!hasIncoming) {
        html += "<li><i>None (Entry Node)</i></li>";
    }
    
    html += "</ul>";
    
    // Outgoing edges
    html += "<h4>Outgoing Edges</h4><ul>";
    
    if (node.successors.empty()) {
        html += "<li><i>None (Exit Node)</i></li>";
    } else {
        for (int successorId : node.successors) {
            QString type = m_currentGraph->isExceptionEdge(nodeId, successorId) ? 
                " <span style='color: red;'>(exception)</span>" : "";
            html += QString("<li>To Node %1%2</li>").arg(successorId).arg(type);
        }
    }
    ...
    // Code content
    html += "<h3>Code Content</h3>";
    
    if (node.statements.empty()) {
        html += "<p><i>No code content available</i></p>";
    } else {
        html += "<pre style='background-color: #f5f5f5; padding: 10px; border-radius: 5px;'>";
        
        for (const QString& stmt : node.statements) {
            html += stmt.toHtmlEscaped() + "\n";
        }
    ...
    ui->reportTextEdit->setHtml(html);
};

void MainWindow::highlightNodeInCodeEditor(int nodeId) {
    if (!m_currentGraph) return;
    
    auto nodeIt = m_currentGraph->getNodes().find(nodeId);
    if (nodeIt == m_currentGraph->getNodes().end()) return;
    ...
    if (node.startLine > 0) {
        ...
        for (int i = 1; i < node.startLine; ++i) {
            ui->codeEditor->moveCursor(QTextCursor::Down);
        }
        
        // Start selection
        ui->codeEditor->moveCursor(QTextCursor::StartOfLine);
        ui->codeEditor->moveCursor(QTextCursor::EndOfLine, QTextCursor::KeepAnchor);
        
        // Expand selection to end line if different
        if (node.endLine > node.startLine) {
            for (int i = node.startLine; i < node.endLine; ++i) {
                ui->codeEditor->moveCursor(QTextCursor::Down, QTextCursor::KeepAnchor);
                ui->codeEditor->moveCursor(QTextCursor::EndOfLine, QTextCursor::KeepAnchor);
            }
        }
