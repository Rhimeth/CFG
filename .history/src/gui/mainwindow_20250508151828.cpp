...
QString MainWindow::parseNodeAttributes(const QString& attributes) {
    QString details;parseNodeAttributes(const QString& attributes) {
...
{oid MainWindow::loadAndProcessJson(const QString& filePath) 
    if (!QFile::exists(filePath)) {
        qWarning() << "JSON file does not exist:" << filePath;
        QMessageBox::warning(this, "Error", "JSON file not found: " + filePath);
        return;eBox::warning(this, "Error", "JSON file not found: " + filePath);
    }   return;
    }
    QFile file(filePath);
    if (!file.open(QIODevice::ReadOnly)) {
        ...
        return;eBox::warning(this, "Error", "Could not open JSON file: " + file.errorString());
    }   return;
    }
    // Read and parse JSON
    ...
    if (parseError.error != QJsonParseError::NoError) {
        ...
        QMessageBox::warning(this, "JSON Error", " << parseError.offset << ":" << parseError.errorString();
                           QString("Parse error at position %1: %2")
                           .arg(parseError.offset) position %1: %2")
                           .arg(parseError.errorString()));
        return;            .arg(parseError.errorString()));
    }   return;
    }
    if (doc.isNull()) {
        qWarning() << "Invalid JSON document";
        QMessageBox::warning(this, "Error", "Invalid JSON document");
        return;eBox::warning(this, "Error", "Invalid JSON document");
    }   return;
    }
    try {
        ...
        if (jsonObj.contains("nodes") && jsonObj["nodes"].isArray()) {
            QJsonArray nodes = jsonObj["nodes"].toArray();isArray()) {
            for (const QJsonValue& node : nodes) {Array();
                if (node.isObject()) {e : nodes) {
                    QJsonObject nodeObj = node.toObject();
                    // Process each node= node.toObject();
                }   // Process each node
            }   }
        }   }
        }
...
{oid MainWindow::initializeGraphviz()
    QString dotPath = QStandardPaths::findExecutable("dot");
    if (dotPath.isEmpty()) {ardPaths::findExecutable("dot");
        qCritical() << "Graphviz 'dot' not found in PATH";
        QMessageBox::critical(this, "Error", und in PATH";
                            "Graphviz 'dot' executable not found.\n"
                            "Please install Graphviz and ensure it's in your PATH.");
        startTextOnlyMode();"Please install Graphviz and ensure it's in your PATH.");
        return;xtOnlyMode();
    }   return;
    }
    qDebug() << "Found Graphviz dot at:" << dotPath;
    setupGraphView();d Graphviz dot at:" << dotPath;
};  setupGraphView();
};
void MainWindow::analyzeDotFile(const QString& filePath) {
    if (!verifyDotFile(filePath)) return;ring& filePath) {
    ...
    if (renderDotToImage(filePath, pngPath)) {
        displayImage(pngPath);ath, pngPath)) {
        return;Image(pngPath);
    }   return;
    }
    // Fallback to SVG
    if (renderDotToImage(filePath, svgPath)) {
        displaySvgInWebView(svgPath);gPath)) {
        return;SvgInWebView(svgPath);
    }   return;
    }
...
{ool MainWindow::renderDotToImage(const QString& dotPath, const QString& outputPath, const QString& format)
    // 1. Enhanced DOT file validation
    QFile dotFile(dotPath); validation
    if (!dotFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        QString error = QString("Cannot open DOT file:\n%1\nError: %2")
                      .arg(dotPath)nnot open DOT file:\n%1\nError: %2")
                      .arg(dotFile.errorString());
        qWarning() << error;otFile.errorString());
        QMessageBox::critical(this, "DOT File Error", error);
        return false;critical(this, "DOT File Error", error);
    }   return false;
    }
    QString dotContent = dotFile.readAll();
    dotFile.close();nt = dotFile.readAll();
    if (dotContent.trimmed().isEmpty()) {
        QString error = "DOT file is empty or contains only whitespace";
        qWarning() << error; file is empty or contains only whitespace";
        QMessageBox::critical(this, "DOT File Error", error);
        return false;critical(this, "DOT File Error", error);
    }   return false;
    if (!dotContent.startsWith("digraph") && !dotContent.startsWith("graph")) {
        QString error = QString("Invalid DOT file format. Must start with 'digraph' or 'graph'.\n"
                              "First line: %1").arg(dotContent.left(100));'digraph' or 'graph'.\n"
    ...
    if (outputFormat.isEmpty()) {.toLower();
        ...
        else {f (outputPath.endsWith(".pdf", Qt::CaseInsensitive)) outputFormat = "pdf";
            QString error = QString("Unsupported output format for file: %1").arg(outputPath);
            qWarning() << error;ing("Unsupported output format for file: %1").arg(outputPath);
            QMessageBox::critical(this, "Export Error", error);
            return false;critical(this, "Export Error", error);
        }   return false;
    }   }
    }
    // 3. Graphviz executable handling
    QString dotExecutablePath;handling
    QStringList potentialPaths = {
        "dot",t potentialPaths = {
        "/usr/local/bin/dot",
        "/usr/bin/dot",/dot",
        "C:/Program Files/Graphviz/bin/dot.exe"
    };  "C:/Program Files/Graphviz/bin/dot.exe"
    for (const QString &path : potentialPaths) {
        if (QFile::exists(path)) {entialPaths) {
            dotExecutablePath = path;
            break;cutablePath = path;
        }   break;
    }   }
    }
    ...
    dotProcess.start(dotExecutablePath, arguments);edChannels);
    ...
    if (!dotProcess.waitForStarted(3000)) {
        QString error = QString("Failed to start Graphviz:\n%1\nCommand: %2 %3")
                       .arg(dotProcess.errorString())hviz:\n%1\nCommand: %2 %3")
                       .arg(dotExecutablePath)ring())
                       .arg(arguments.join(" "));
        qWarning() << error;arguments.join(" "));
        QMessageBox::critical(this, "Process Error", error);
        return false;critical(this, "Process Error", error);
    }   return false;
    }
    // Wait with timeout and process output
    QByteArray processOutput;process output
    QElapsedTimer timer;tput;
    timer.start();timer;
    timer.start();
    while (!dotProcess.waitForFinished(500)) {
        processOutput += dotProcess.readAll();
        QCoreApplication::processEvents();l();
        if (timer.hasExpired(15000)) { // 15 second timeout
            dotProcess.kill();5000)) { // 15 second timeout
            QString error = QString("Graphviz timed out after 15 seconds\nPartial output:\n%1")
                          .arg(QString(processOutput)); after 15 seconds\nPartial output:\n%1")
            qWarning() << error;String(processOutput));
            QMessageBox::critical(this, "Timeout Error", error);
            return false;critical(this, "Timeout Error", error);
        }   return false;
    }   }
    }
    processOutput += dotProcess.readAll();
    ...
    if (dotProcess.exitCode() != 0 || !QFile::exists(outputPath)) {
        QString error = QString("Graphviz failed (exit code %1)\nError output:\n%2")
                      .arg(dotProcess.exitCode())(exit code %1)\nError output:\n%2")
                      .arg(QString(processOutput));
        qWarning() << error;String(processOutput));
        QMessageBox::critical(this, "Rendering Error", error);
        if (QFile::exists(outputPath)) {dering Error", error);
    ...
    if (outputInfo.size() < 100) { // Minimum expected file size
        QString error = QString("Output file too small (%1 bytes)\nGraphviz output:\n%2")
                      .arg(outputInfo.size())too small (%1 bytes)\nGraphviz output:\n%2")
                      .arg(QString(processOutput));
        qWarning() << error;String(processOutput));
        ...
        return false;critical(this, "Output Error", error);
    }   return false;
    }
    // 7. Format-specific validation
    if (outputFormat == "png") {tion
        QFile file(outputPath);{
        if (file.open(QIODevice::ReadOnly)) {
            QByteArray header = file.read(8);
            file.close();ader = file.read(8);
            if (!header.startsWith("\x89PNG")) {
                QFile::remove(outputPath);G")) {
                QString error = "Invalid PNG file header - corrupted output";
                qWarning() << error;alid PNG file header - corrupted output";
                QMessageBox::critical(this, "PNG Error", error);
                return false;critical(this, "PNG Error", error);
            }   return false;
        }   }
    }   }
    else if (outputFormat == "svg") {
        QFile file(outputPath);vg") {
        if (file.open(QIODevice::ReadOnly)) {
            QString content = file.read(1024);
            file.close();nt = file.read(1024);
            if (!content.contains("<svg")) {
                QFile::remove(outputPath); {
                QString error = "Invalid SVG content - missing SVG tag";
                qWarning() << error;alid SVG content - missing SVG tag";
                QMessageBox::critical(this, "SVG Error", error);
                return false;critical(this, "SVG Error", error);
            }   return false;
        }   }
    }   }
    }
    qDebug() << "Successfully exported graph to:" << outputPath;
    return true;"Successfully exported graph to:" << outputPath;
};  return true;
};
void MainWindow::showRawDotContent(const QString& dotPath) {
    QFile file(dotPath);DotContent(const QString& dotPath) {
    if (file.open(QIODevice::ReadOnly)) {
        ui->reportTextEdit->setPlainText(file.readAll());
        file.close();tEdit->setPlainText(file.readAll());
    }   file.close();
};  }
};
void MainWindow::visualizeCurrentGraph() {
    if (!m_currentGraph) return;tGraph() {
    if (!m_currentGraph) return;
    std::string dot = Visualizer::generateDotRepresentation(m_currentGraph.get());
    std::string dot = Visualizer::generateDotRepresentation(m_currentGraph.get());
    // Load into web view
    QString html = QString(R"(
<!DOCTYPE html>l = QString(R"(
<html>YPE html>
<head>
    <title>CFG Visualization</title>
    <script src="qrc:/qtwebchannel/qwebchannel.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/viz.js/2.1.2/viz.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/viz.js/2.1.2/full.render.js"></script>
    <style> src="https://cdnjs.cloudflare.com/ajax/libs/viz.js/2.1.2/full.render.js"></script>
        body { margin:0; background:#2D2D2D; }
        #graph-container { width:100%; height:100%; }
        .node:hover { stroke-width:2px; cursor:pointer; }
        .expanded-node { fill: #ffffcc; stroke-width:2px; }
        .error-message { color: red; padding: 20px; text-align: center; }
        .highlighted { { color: red; padding: 20px; text-align: center; }
            stroke: #FFA500 !important;
            stroke-width: 3px !important;
            filter: drop-shadow(0 0 5px rgba(255, 165, 0, 0.7));
        }   filter: drop-shadow(0 0 5px rgba(255, 165, 0, 0.7));
...
<body>>
    <div id="graph-container"></div>
    <script>"graph-container"></div>
        new QWebChannel(qt.webChannelTransport, function(channel) {
            window.bridge = channel.objects.bridge;ction(channel) {
        }); window.bridge = channel.objects.bridge;
        const viz = new Viz();
        viz.renderSVGElement(`%1`)
            .then(element => {%1`)
                ...
                element.addEventListener('click', (e) => {
                    const node = e.target.closest('[id^="node"]');
                    if (node && window.bridge) {t('[id^="node"]');
                ...
                element.addEventListener('mousemove', (e) => {
                    const edge = e.target.closest('[id^="edge"]');
                    if (edge && window.bridge) {t('[id^="edge"]');
                        const [from, to] = edge.id.replace('edge', '').split('_');
                        window.bridge.onEdgeHovered(from, to);ge', '').split('_');
            ...
            .catch(error => {
                console.error(error);
                const container = document.getElementById('graph-container');
                if (container) {= document.getElementById('graph-container');
                    container.innerHTML = '<div class="error-message">Graph rendering failed: ' + 
                                          error.message + '</div>';e">Graph rendering failed: ' + 
                }                         error.message + '</div>';
            }); }
    </script>);
</body>cript>
    ...
    webView->setHtml(html);
    connect(webView, &QWebEngineView::loadFinished, [this](bool success) {
        if (success) {QWebEngineView::loadFinished, [this](bool success) {
            qDebug() << "Web view loaded successfully, initializing web channel...";
            webView->page()->runJavaScript(ccessfully, initializing web channel...";
                R"(->page()->runJavaScript(
                if (typeof QWebChannel === "undefined") {
                    console.error("QWebChannel not loaded");
                } else {ole.error("QWebChannel not loaded");
                    new QWebChannel(qt.webChannelTransport, function(channel) {
                        window.bridge = channel.objects.bridge;ction(channel) {
                        console.log("Web channel established");
                        if (window.bridge && typeof window.bridge.webChannelInitialized === "function") {
                            window.bridge.webChannelInitialized();webChannelInitialized === "function") {
                        } else {ow.bridge.webChannelInitialized();
...
{oid MainWindow::highlightNode(int nodeId, const QColor& color)
    if (!m_graphView || !m_graphView->scene()) return;
    if (!m_graphView || !m_graphView->scene()) return;
    // Reset previous highlighting
    resetHighlighting();ghlighting
    resetHighlighting();
    ...
    foreach (QGraphicsItem* item, m_graphView->scene()->items()) {
        if (item->data(MainWindow::NodeItemType).toInt() == 1) { {
            if (auto ellipse = qgraphicsitem_cast<QGraphicsEllipseItem*>(item)) {
                if (item->data(MainWindow::NodeIdKey).toInt() == nodeId) {tem)) {
                    QPen pen = ellipse->pen();eIdKey).toInt() == nodeId) {
                    pen.setWidth(3);se->pen();
                    pen.setColor(Qt::darkBlue);
                    ellipse->setPen(pen);Blue);
                    QBrush brush = ellipse->brush();
                    brush.setColor(color);->brush();
                    ellipse->setBrush(brush);
                    m_highlightNode = item;);
                    m_graphView->centerOn(item);
                    break;hView->centerOn(item);
                }   break;
            }   }
        }   }
    }   }
...
{oid MainWindow::highlightEdge(int fromId, int toId, const QColor& color)
    // Reset previous highlighting
    if (m_highlightEdge) {lighting
        if (auto line = qgraphicsitem_cast<QGraphicsLineItem*>(m_highlightEdge)) {
            QPen pen = line->pen();em_cast<QGraphicsLineItem*>(m_highlightEdge)) {
            pen.setWidth(1);>pen();
            pen.setColor(Qt::black);
            line->setPen(pen);lack);
        }   line->setPen(pen);
        m_highlightEdge = nullptr;
    }   m_highlightEdge = nullptr;
    }
        ...
        for (QGraphicsItem* item : m_scene->items()) {
            if (item->data(EdgeItemType).toInt() == 1) {
                if (item->data(EdgeFromKey).toInt() == fromId &&
                    item->data(EdgeToKey).toInt() == toId) {d &&
                    if (auto line = qgraphicsitem_cast<QGraphicsLineItem*>(item)) {
                        QPen pen = line->pen();em_cast<QGraphicsLineItem*>(item)) {
                        pen.setWidth(3);>pen();
                        pen.setColor(color);
                        line->setPen(pen););
                        m_highlightEdge = item;
                        break;lightEdge = item;
                    }   break;
                }   }
            }   }
        }   }
...
{oid MainWindow::resetHighlighting()
    if (m_highlightNode) {
        if (auto ellipse = qgraphicsitem_cast<QGraphicsEllipseItem*>(m_highlightNode)) {
            QPen pen = ellipse->pen();em_cast<QGraphicsEllipseItem*>(m_highlightNode)) {
            pen.setWidth(1);se->pen();
            pen.setColor(Qt::black);
            ellipse->setPen(pen);k);
...
{oid MainWindow::expandNode(const QString& nodeIdStr)
    bool ok;
    ...
    const auto& nodes = m_currentGraph->getNodes();
    auto it = nodes.find(nodeId);Graph->getNodes();
    if (it != nodes.end()) {eId);
        for (int succ : it->second.successors) {
            m_visibleNodes[succ] = true;ssors) {
        }   m_visibleNodes[succ] = true;
    }   }
...
int MainWindow::findEntryNode() {
    ...
    if (!m_currentGraph) return -1;
    const auto& nodes = m_currentGraph->getNodes();
    if (nodes.empty()) return -1;Graph->getNodes();
    ...
    for (const auto& [id, node] : nodes) {
        if (hasIncomingEdges.find(id) == hasIncomingEdges.end()) {
            return id;gEdges.find(id) == hasIncomingEdges.end()) {
        }   return id;
    }   }
    }
...
void MainWindow::selectNode(int nodeId) {
    ...
    m_currentlySelectedNodeId = nodeId;e
    ...
    if (!m_nodeInfoMap.contains(nodeId) || !m_currentGraph) {
        ...
        return;g() << "No node info available for node:" << nodeId;
    }   return;
    }
    // Highlight in graph
    ...
    highlightNode(nodeId, QColor(Qt::yellow));
    if (!m_nodeInfoMap.contains(nodeId) || !m_currentGraph) {
        return;InfoMap.contains(nodeId) || !m_currentGraph) {
    }   return;
    }
    ...
    QString report;ummary report
    report += QString("=== Node %1 Summary ===\n").arg(nodeId);
    report += QString("File: %1\n").arg(nodeInfo.filePath);Id);
    ...
    for (const auto& [otherId, otherNode] : m_currentGraph->getNodes()) {
        if (std::find(otherNode.successors.begin(), otherNode.successors.end(), nodeId) != otherNode.successors.end()) {
            hasIncoming = true;.successors.begin(), otherNode.successors.end(), nodeId) != otherNode.successors.end()) {
            QString edgeType = m_currentGraph->isExceptionEdge(otherId, nodeId) ? 
                " (exception path)" : " (normal flow)";ionEdge(otherId, nodeId) ? 
                " (exception path)" : " (normal flow)";
            // Get caller node info if available
            QString callerInfo;info if available
    ...
    ui->reportTextEdit->setPlainText(report);el
...
void MainWindow::centerOnNode(const QString& nodeId) {
    bool ok;dow::centerOnNode(const QString& nodeId) {
    int id = nodeId.toInt(&ok);
    if (ok) {nodeId.toInt(&ok);
        centerOnNode(id);
    } else {erOnNode(id);
...
Q_INVOKABLE void MainWindow::handleNodeClick(const QString& nodeId) {
    qDebug() << "Node clicked from web view:" << nodeId;ng& nodeId) {
    qDebug() << "Node clicked from web view:" << nodeId;
    // Convert to integer
    bool ok;rt to integer
    int id = nodeId.toInt(&ok);
    if (!ok || !m_currentGraph) return;
    if (!ok || !m_currentGraph) return;
    selectNode(id);
...
void MainWindow::handleEdgeClick(const QString& fromId, const QString& toId) {
    qDebug() << "Edge clicked:" << fromId << "->" << toId;nst QString& toId) {
    emit edgeClicked(fromId, toId);fromId << "->" << toId;
    emit edgeClicked(fromId, toId);
    bool ok1, ok2;
    int from = fromId.toInt(&ok1);
    int to = toId.toInt(&ok2);k1);
    ...
    if (ok1 && ok2 && m_currentGraph) {
        ...
        QString edgeType = m_currentGraph->isExceptionEdge(from, to) ? 
            "Exception Edge" : "Control Flow Edge";ionEdge(from, to) ? 
        ...
        if (m_nodeInfoMap.contains(from) && m_nodeInfoMap.contains(to)) {
            const NodeInfo& fromInfo = m_nodeInfoMap[from];ontains(to)) {
            // Highlight the source node (from) code[from];
            if (m_nodeCodePositions.contains(from)) {
                QTextCursor cursor = m_nodeCodePositions[from];
                codeEditor->setTextCursor(cursor);itions[from];
                codeEditor->ensureCursorVisible();
                highlightCodeSection(fromInfo.startLine, fromInfo.endLine);
                m_lastClickedEdgeTarget = to;.startLine, fromInfo.endLine);
                statusBar()->showMessage( to;
                    QString("Edge: %1 → %2 | Click again to see destination code").arg(from).arg(to), 
                    3000);g("Edge: %1 → %2 | Click again to see destination code").arg(from).arg(to), 
            }       3000);
        }   }
    }   }
};  }
...
{oid MainWindow::onEdgeClicked(const QString& fromId, const QString& toId)
    ...
    if (ok1 && ok2 && m_currentGraph) {
        ...
        static bool showDestination = false;e and destination nodes
        if (m_nodeInfoMap.contains(from) && m_nodeInfoMap.contains(to)) {
            int nodeToHighlight = showDestination ? to : from;ains(to)) {
            if (m_nodeCodePositions.contains(nodeToHighlight)) {
                ...
                codeEditor->setTextCursor(cursor);itions[nodeToHighlight];
                codeEditor->ensureCursorVisible();
                highlightCodeSection(info.startLine, info.endLine);
                QString message = showDestination ?, info.endLine);
                    QString("Showing destination node %1 code").arg(to) :
                    QString("Showing source node %1 code").arg(from);o) :
                statusBar()->showMessage(message, 3000);").arg(from);
...
void MainWindow::highlightCodeSection(int startLine, int endLine) {
    QList<QTextEdit::ExtraSelection> extraSelections;int endLine) {
    QTextDocument* doc = ui->codeEditor->document();;
    QTextDocument* doc = ui->codeEditor->document();
    // Highlight the entire block with a clear background
    QTextCursor blockCursor(doc); with a clear background
    ...
    blockSelection.format.setBackground(QColor(255, 255, 150)); // Light yellow background
    blockSelection.format.setProperty(QTextFormat::FullWidthSelection, true);ow background
    blockSelection.cursor = blockCursor;extFormat::FullWidthSelection, true);
    ...
    QTextEdit::ExtraSelection startSelection;umber(startLine - 1));
    startSelection.format.setBackground(QColor(200, 255, 200)); // Light green for start
    startSelection.format.setProperty(QTextFormat::FullWidthSelection, true);n for start
    startSelection.cursor = startCursor;extFormat::FullWidthSelection, true);
    extraSelections.append(startSelection);
    extraSelections.append(startSelection);
    // End line boundary (red)
    if (startLine != endLine) { // Only if different from start line
        QTextCursor endCursor(doc->findBlockByNumber(endLine - 1));e
        QTextEdit::ExtraSelection endSelection;umber(endLine - 1));
        endSelection.format.setBackground(QColor(255, 200, 200)); // Light red for end
        endSelection.format.setProperty(QTextFormat::FullWidthSelection, true);for end
        endSelection.cursor = endCursor;QTextFormat::FullWidthSelection, true);
    ...
    ui->statusbar->showMessage(headerText, 5000);ON - LINES %1-%2 */").arg(startLine).arg(endLine);
    ui->statusbar->showMessage(headerText, 5000);
    // Scroll more context - show a few lines before the selection
    int contextLines = 3;t - show a few lines before the selection
    ...
    ui->codeEditor->setTextCursor(scrollCursor);ument()->findBlockByNumber(scrollToLine - 1));
    ui->codeEditor->ensureCursorVisible();rsor);
};  ui->codeEditor->ensureCursorVisible();
};
void MainWindow::loadAndHighlightCode(const QString& filePath, int startLine, int endLine) {
    QFile file(filePath);ighlightCode(const QString& filePath, int startLine, int endLine) {
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qWarning() << "Could not open file:" << filePath;) {
        return;g() << "Could not open file:" << filePath;
    }   return;
    }
    // Read file content
    QTextStream in(&file);
    QString content = in.readAll();
    file.close();nt = in.readAll();
    file.close();
    // Set text in editor
    ...
    ui->codeEditor->setTextCursor(cursor);ument()->findBlockByNumber(startLine - 1));
    ui->codeEditor->ensureCursorVisible();
};  ui->codeEditor->ensureCursorVisible();
};
void MainWindow::clearCodeHighlights() {
    if (codeEditor) {rCodeHighlights() {
...
void MainWindow::onNodeExpanded(const QString& nodeId) {
    if (!m_currentGraph) return;const QString& nodeId) {
    if (!m_currentGraph) return;
    bool ok;
    int id = nodeId.toInt(&ok);
...
{oid MainWindow::onEdgeHovered(const QString& from, const QString& to)
    bool ok1, ok2;
    int fromId = from.toInt(&ok1);
    int toId = to.toInt(&ok2);k1);
    int toId = to.toInt(&ok2);
    if (ok1 && ok2) {
        ui->statusbar->showMessage(QString("Edge %1 → %2").arg(fromId).arg(toId), 2000);
    } else {statusbar->showMessage(QString("Edge %1 → %2").arg(fromId).arg(toId), 2000);
...
void MainWindow::updateExpandedNode(int nodeId, const QString& content) {
    // Execute JavaScript to update the nodeId, const QString& content) {
    webView->page()->runJavaScript( the node
        QString("var node = document.getElementById('node%1');"
                "if (node) {"ocument.getElementById('node%1');"
                "  var text = node.querySelector('text');"
                "  if (text) text.textContent = '%2';"');"
                "}").arg(nodeId).arg(content)); '%2';"
};              "}").arg(nodeId).arg(content));
};
void MainWindow::updateCollapsedNode(int nodeId) {
    // Execute JavaScript to collapse the noded) {
    webView->page()->runJavaScript(se the node
        QString("var node = document.getElementById('node%1');"
                "if (node) {"ocument.getElementById('node%1');"
                "  var text = node.querySelector('text');"
                "  if (text) text.textContent = 'Node %2';"
                "}").arg(nodeId).arg(nodeId));= 'Node %2';"
};              "}").arg(nodeId).arg(nodeId));
};
void MainWindow::showNodeContextMenu(const QPoint& pos) {
    QMenu menu;::showNodeContextMenu(const QPoint& pos) {
    QMenu menu;
    ...
    menu.exec(webView->mapToGlobal(pos));&MainWindow::handleExport);
};  menu.exec(webView->mapToGlobal(pos));
};
QString MainWindow::generateExportHtml() const {
    return QString(R"(nerateExportHtml() const {
<!DOCTYPE html>ing(R"(
<html>YPE html>
head>>
    <title>CFG Export</title>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/viz.js/2.1.2/viz.js"></script>
    <style> src="https://cdnjs.cloudflare.com/ajax/libs/viz.js/2.1.2/viz.js"></script>
        body { margin: 0; padding: 0; }
...
<body>>
    <script>
        ...
        document.body.innerHTML = svg; 'svg', engine: 'dot' });
    </script>ent.body.innerHTML = svg;
</body>cript>
</html>
...
{oid MainWindow::onParseButtonClicked()
    QString filePath = ui->filePathEdit->text();
    if (filePath.isEmpty()) {lePathEdit->text();
        QMessageBox::warning(this, "Error", "Please select a file first");
        return;eBox::warning(this, "Error", "Please select a file first");
    }   return;
    }
    setUiEnabled(false);
    ...
    QFuture<void> future = QtConcurrent::run([this, filePath]() {
        try {oid> future = QtConcurrent::run([this, filePath]() {
            // Read file content
            QFile file(filePath);
            if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
            ...
            file.close();ntent = file.readAll();
            file.close();
            // Parse DOT content
            auto graph = parseDotToCFG(dotContent);
            auto graph = parseDotToCFG(dotContent);
            // Count nodes and edges
            int nodeCount = 0; edges
            int edgeCount = 0;
            for (const auto& [id, node] : graph->getNodes()) {
                nodeCount++; [id, node] : graph->getNodes()) {
...
void MainWindow::onParsingFinished(bool success) {
    if (success) {nParsingFinished(bool success) {
        qDebug() << "Parsing completed successfully";
    } else {ug() << "Parsing completed successfully";
        qDebug() << "Parsing failed";
    }   qDebug() << "Parsing failed";
};  }
};
void MainWindow::applyGraphTheme() {
    // Define colorslyGraphTheme() {
...
void MainWindow::setupGraphLayout() {
    if (!m_graphView) return;yout() {
    if (!m_graphView) return;
    switch (m_currentLayoutAlgorithm) {
        case Hierarchical:tAlgorithm) {
            m_graphView->applyHierarchicalLayout();
            break;hView->applyHierarchicalLayout();
        case ForceDirected:
            m_graphView->applyForceDirectedLayout();
            break;hView->applyForceDirectedLayout();
        case Circular:
            m_graphView->applyCircularLayout();
            break;hView->applyCircularLayout();
    }       break;
};  }
};
...
void MainWindow::highlightFunction(const QString& functionName) {
    if (!m_graphView) return;ction(const QString& functionName) {
    if (!m_graphView) return;
    foreach (QGraphicsItem* item, m_graphView->scene()->items()) {
        if (item->data(MainWindow::NodeItemType).toInt() == 1) { {
            bool highlight = false;NodeItemType).toInt() == 1) {
            foreach (QGraphicsItem* child, item->childItems()) {
                if (auto text = dynamic_cast<QGraphicsTextItem*>(child)) {
                    if (text->toPlainText().contains(functionName, Qt::CaseInsensitive)) {
                        highlight = true;().contains(functionName, Qt::CaseInsensitive)) {
                        break;ght = true;
                    }   break;
                }   }
            }   }
            if (auto ellipse = dynamic_cast<QGraphicsEllipseItem*>(item)) {
                QBrush brush = ellipse->brush();phicsEllipseItem*>(item)) {
                brush.setColor(highlight ? Qt::yellow : m_currentTheme.nodeColor);
                ellipse->setBrush(brush);? Qt::yellow : m_currentTheme.nodeColor);
            }   ellipse->setBrush(brush);
        }   }
    }   }
};  }
...
{oid MainWindow::on_analyzeButton_clicked()
    ...
    if (filePath.isEmpty()) {lePathEdit->text().trimmed();
        QMessageBox::warning(this, "Error", "Please select a file first");
        return;eBox::warning(this, "Error", "Please select a file first");
    }   return;
    }
    QApplication::setOverrideCursor(Qt::WaitCursor);
    try {ication::setOverrideCursor(Qt::WaitCursor);
        QFileInfo fileInfo(filePath);
        ...
        bool validExtension = std::any_of(validExtensions.begin(), validExtensions.end(),
            [&filePath](const QString& ext) {idExtensions.begin(), validExtensions.end(),
        ...
        m_currentGraph = parseDotToCFG(QString::fromStdString(result.dotOutput));
        displayGraph(QString::fromStdString(result.dotOutput));esult.dotOutput));
        ui->reportTextEdit->setPlainText(QString::fromStdString(result.report));
        statusBar()->showMessage("Analysis completed", 3000);ng(result.report));
    } catch (const std::exception& e) {sis completed", 3000);
        QString errorMsg = QString("Analysis failed:\n%1\n"
                                 "Please verify:\n":\n%1\n"
                                 "1. File contains valid C++ code\n"
                                 "2. Graphviz is installed").arg(e.what());
        QMessageBox::critical(this, "Error", errorMsg);led").arg(e.what());
...
void MainWindow::handleAnalysisResult(const CFGAnalyzer::AnalysisResult& result) {
    if (QThread::currentThread() != this->thread()) {er::AnalysisResult& result) {
        QMetaObject::invokeMethod(this, "handleAnalysisResult", 
                                 Qt::QueuedConnection,sResult", 
                                 Q_ARG(CFGAnalyzer::AnalysisResult, result));
        return;                  Q_ARG(CFGAnalyzer::AnalysisResult, result));
    }   return;
    }
    if (!result.success) {
        ui->reportTextEdit->setPlainText(QString::fromStdString(result.report));
        QMessageBox::critical(this, "Analysis Error", StdString(result.report));
                            QString::fromStdString(result.report));
        return;             QString::fromStdString(result.report));
    }   return;
    }
    if (!result.dotOutput.empty()) {
        try {lt.dotOutput.empty()) {
            auto graph = parseDotToCFG(QString::fromStdString(result.dotOutput));
            m_currentGraph = graph;CFG(QString::fromStdString(result.dotOutput));
            visualizeCFG(graph);ph;
        } catch (...) {G(graph);
...
{oid MainWindow::displayFunctionInfo(const QString& input)
    if (!m_currentGraph) {
        ui->reportTextEdit->append("No CFG loaded");
        return;ortTextEdit->append("No CFG loaded");
    }   return;
    }
    bool found = false;
    const auto& nodes = m_currentGraph->getNodes();
    ...
    for (const auto& [id, node] : nodes) {
        if (node.functionName.contains(input, Qt::CaseInsensitive)) {
            found = true;Name.contains(input, Qt::CaseInsensitive)) {
            found = true;
            // Use QString directly without conversion
            ui->reportTextEdit->append(QString("Function: %1").arg(node.functionName));
            ui->reportTextEdit->append(QString("Node ID: %1").arg(id));.functionName));
            ...
            if (!node.statements.empty()) {
                ...
                for (const QString& stmt : node.statements) {
                    ui->reportTextEdit->append(stmt);ments) {
            ...
            if (!node.successors.empty()) {
                ui->reportTextEdit->append("\nConnects to:");
                for (int successor : node.successors) {to:");
                    QString edgeType = m_currentGraph->isExceptionEdge(id, successor) 
                        ? " (exception edge)" ntGraph->isExceptionEdge(id, successor) 
                        : "";exception edge)" 
                    ui->reportTextEdit->append(QString("  -> Node %1%2")
                                               .arg(successor)ode %1%2")
                                               .arg(edgeType));
                }                              .arg(edgeType));
            }   }
            ui->reportTextEdit->append("------------------");
...
QString MainWindow::getNodeAtPosition(const QPoint& pos) const {
    // Convert the QPoint to viewport coordinatest& pos) const {
    QPoint viewportPos = webView->mapFromGlobal(webView->mapToGlobal(pos));
    QPoint viewportPos = webView->mapFromGlobal(webView->mapToGlobal(pos));
    // JavaScript to find the node at given coordinates
    QString js = QString(R"(e node at given coordinates
        (function() {ing(R"(
            // Get element at point
            const element = document.elementFromPoint(%1, %2);
            if (!element) return '';.elementFromPoint(%1, %2);
            if (!element) return '';
            // Find the closest node element (either the node itself or a child element)
            const nodeElement = element.closest('[id^="node"]');self or a child element)
            if (!nodeElement) return '';closest('[id^="node"]');
            if (!nodeElement) return '';
            // Extract the node ID
            const nodeId = nodeElement.id.replace('node', '');
            return nodeId; nodeElement.id.replace('node', '');
        })()return nodeId;
    )").arg(viewportPos.x()).arg(viewportPos.y());
    )").arg(viewportPos.x()).arg(viewportPos.y());
    // Execute JavaScript synchronously and get the result
    QString nodeId;Script synchronously and get the result
    QEventLoop loop;
    webView->page()->runJavaScript(js, [&](const QVariant& result) {
        nodeId = result.toString();js, [&](const QVariant& result) {
        loop.quit();ult.toString();
    }); loop.quit();
    loop.exec();
    loop.exec();
    return nodeId;
...
{oid MainWindow::displayNodeInfo(int nodeId)
...
{oid MainWindow::onSearchButtonClicked()
    QString searchText = ui->search->text().trimmed();
    if (searchText.isEmpty()) return;text().trimmed();
    ...
    if (!m_currentGraph) {
        ...
        return;eBox::information(this, "Search", "No graph loaded");
    }   return;
    }
    // Search in different aspects
    const auto& nodes = m_currentGraph->getNodes();
    for (const auto& [id, node] : nodes) {tNodes();
        if (QString::number(id).contains(searchText)) {
            m_searchResults.insert(id);s(searchText)) {
            continue;esults.insert(id);
        }   continue;
        }
        if (node.label.contains(searchText, Qt::CaseInsensitive)) {
            m_searchResults.insert(id);ext, Qt::CaseInsensitive)) {
            continue;esults.insert(id);
        }   continue;
        }
        for (const auto& stmt : node.statements) {
            if (stmt.contains(searchText, Qt::CaseInsensitive)) {
                m_searchResults.insert(id);t::CaseInsensitive)) {
                break;chResults.insert(id);
            }   break;
        }   }
    }   }
    }
    if (m_searchResults.isEmpty()) {
        ...
        return;eBox::information(this, "Search", "No matching nodes found");
    }   return;
    }
    // Highlight first result
    showNextSearchResult();lt
};  showNextSearchResult();
};
void MainWindow::onSearchTextChanged(const QString& text)
{oid MainWindow::onSearchTextChanged(const QString& text)
    if (text.isEmpty() && !m_searchResults.isEmpty()) {
        m_searchResults.clear();rchResults.isEmpty()) {
        resetHighlighting();r();
        clearCodeHighlights();
    }   clearCodeHighlights();
}   }
...
void MainWindow::saveNodeInformation(const QString& filePath) {
    ...
    QFile file(filePath);esArray);
    if (file.open(QIODevice::WriteOnly)) {
        file.write(doc.toJson());eOnly)) {
        file.close();c.toJson());
    }   file.close();
};  }
};
void MainWindow::loadNodeInformation(const QString& filePath) {
    QFile file(filePath);Information(const QString& filePath) {
    if (!file.open(QIODevice::ReadOnly)) return;
    if (!file.open(QIODevice::ReadOnly)) return;
    QJsonDocument doc = QJsonDocument::fromJson(file.readAll());
    if (doc.isArray()) {QJsonDocument::fromJson(file.readAll());
        m_nodeInfoMap.clear();
        for (const QJsonValue& val : doc.array()) {
            QJsonObject obj = val.toObject();y()) {
            NodeInfo info;j = val.toObject();
            info.id = obj["id"].toInt();
            info.label = obj["label"].toString();
            info.filePath = obj["filePath"].toString();
            info.startLine = obj["startLine"].toInt();;
            info.endLine = obj["endLine"].toInt();t();
            ...
            for (const QJsonValue& stmt : obj["statements"].toArray()) {
                info.statements.append(stmt.toString());s"].toArray()) {
            }   info.statements.append(stmt.toString());
            }
            for (const QJsonValue& succ : obj["successors"].toArray()) {
                info.successors.append(succ.toInt());sors"].toArray()) {
    ...
    foreach (QGraphicsItem* item, m_graphView->scene()->items()) {
        if (item->data(MainWindow::NodeItemType).toInt() == 1) { {
            if (item->data(MainWindow::NodeIdKey).toInt() == nodeId) {
                m_graphView->centerOn(item);dKey).toInt() == nodeId) {
                break;hView->centerOn(item);
            }   break;
        }   }
    }   }
...
{oid MainWindow::on_toggleFunctionGraph_clicked()
    static bool showFullGraph = true;
    static bool showFullGraph = true;
    try {
        m_graphView->toggleGraphDisplay(!showFullGraph);
        showFullGraph = !showFullGraph;(!showFullGraph);
...
void MainWindow::setGraphTheme(const VisualizationTheme& theme) {
    ...
    if (webView) { = theme;
        ...
        ).arg(theme.nodeColor.name(),.style.setProperty('--bg-color', '%4');"
        ...
        if (item->data(MainWindow::NodeItemType).toInt() == 1) { {
            foreach (QGraphicsItem* child, item->childItems()) {
                if (dynamic_cast<QGraphicsTextItem*>(child)) { {
                    child->setVisible(visible);tem*>(child)) {
...
{oid MainWindow::switchLayoutAlgorithm(int index)
    if (!m_graphView) return;
    if (!m_graphView) return;
    switch(index) {
    case 0: m_graphView->applyHierarchicalLayout(); break;
    case 1: m_graphView->applyForceDirectedLayout(); break;
    case 2: m_graphView->applyCircularLayout(); break;reak;
    default: break;View->applyCircularLayout(); break;
    }efault: break;
    m_graphView->fitInView(m_graphView->scene()->itemsBoundingRect(), Qt::KeepAspectRatio);
};  m_graphView->fitInView(m_graphView->scene()->itemsBoundingRect(), Qt::KeepAspectRatio);
};
void MainWindow::visualizeFunction(const QString& functionName) 
{oid MainWindow::visualizeFunction(const QString& functionName) 
    QString filePath = ui->filePathEdit->text();
    if (filePath.isEmpty()) {lePathEdit->text();
        QMessageBox::warning(this, "Error", "Please select a file first");
        return;eBox::warning(this, "Error", "Please select a file first");
    ...
    QtConcurrent::run([this, filePath, functionName]() {
        try {ent::run([this, filePath, functionName]() {
            auto cfgGraph = generateFunctionCFG(filePath, functionName);
            QMetaObject::invokeMethod(this, [this, cfgGraph]() {onName);
                handleVisualizationResult(cfgGraph);fgGraph]() {
...
std::shared_ptr<GraphGenerator::CFGGraph> MainWindow::generateFunctionCFG(
    const QString& filePath, const QString& functionName)erateFunctionCFG(
{   const QString& filePath, const QString& functionName)
    try {
        ...
        throw;cal() << "Error generating function CFG:" << e.what();
    }   throw;
};  }
};
void MainWindow::connectSignals() {
    connect(ui->analyzeButton, &QPushButton::clicked, this, [this](){
        QString filePath = ui->filePathEdit->text();, this, [this](){
        if (!filePath.isEmpty()) {ePathEdit->text();
            std::vector<std::string> sourceFiles = { filePath.toStdString() };
            auto graph = GraphGenerator::generateCFG(sourceFiles);dString() };
            m_currentGraph = std::shared_ptr<GraphGenerator::CFGGraph>(graph.release());
            visualizeCurrentGraph();ared_ptr<GraphGenerator::CFGGraph>(graph.release());
        }   visualizeCurrentGraph();
    }); }
    connect(ui->toggleFunctionGraph, &QPushButton::clicked, this, &MainWindow::toggleVisualizationMode);
    connect(ui->searchButton, &QPushButton::clicked, this, &MainWindow::highlightSearchResult);ionMode);
    connect(ui->searchButton, &QPushButton::clicked, this, &MainWindow::highlightSearchResult);
    webView->setContextMenuPolicy(Qt::CustomContextMenu);
    connect(webView, &QWebEngineView::customContextMenuRequested,
            this, &MainWindow::showNodeContextMenu);enuRequested,
};          this, &MainWindow::showNodeContextMenu);
};
void MainWindow::toggleVisualizationMode() {
    static bool showFullGraph = true;ode() {
    if (m_graphView) {llGraph = true;
...
{oid MainWindow::handleExport()
    qDebug() << "Export button clicked";
    QString format = "png";ton clicked";
    if (m_currentGraph) {";
        exportGraph(format);
    } else {rtGraph(format);
...
{oid MainWindow::handleFileSelected(QListWidgetItem* item)
    if (!item) {
        qWarning() << "Null item selected";
        return;g() << "Null item selected";
    }   return;
    }
    QString filePath = item->data(Qt::UserRole).toString();
    qDebug() << "Loading file:" << filePath;le).toString();
    if (QFile::exists(filePath)) { filePath;
        loadFile(filePath);ath)) {
        ui->filePathEdit->setText(filePath);
    } else {filePathEdit->setText(filePath);
        QMessageBox::critical(this, "Error", "File not found: " + filePath);
    }   QMessageBox::critical(this, "Error", "File not found: " + filePath);
};  }
};
void MainWindow::loadFile(const QString& filePath)
{oid MainWindow::loadFile(const QString& filePath)
    QFile file(filePath);
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        QMessageBox::critical(this, "Error", evice::Text)) {
                            QString("Could not open file:\n%1\n%2")
                            .arg(filePath) not open file:\n%1\n%2")
                            .arg(file.errorString()));
        return;             .arg(file.errorString()));
    }   return;
    }
    // Read file content
    QTextStream in(&file);
    ...
    ui->codeEditor->setPlainText(content);
    ui->filePathEdit->setText(filePath););
    m_currentFile = filePath;(filePath);
...
{oid MainWindow::openFile(const QString& filePath)
    if (QFile::exists(filePath)) {
        loadFile(filePath); // This calls the private method
    } else {File(filePath); // This calls the private method
...
{oid MainWindow::fileChanged(const QString& path)
    if (QFileInfo::exists(path)) {
        int ret = QMessageBox::question(this, "File Changed",
                                      "The file has been modified externally. Reload?",
                                      QMessageBox::Yes | QMessageBox::No);ly. Reload?",
        if (ret == QMessageBox::Yes) {QMessageBox::Yes | QMessageBox::No);
            loadFile(path);Box::Yes) {
        }   loadFile(path);
    } else {
        QMessageBox::warning(this, "File Removed", 
                           "The file has been removed or renamed.");
        m_fileWatcher->removePath(path); been removed or renamed.");
    }   m_fileWatcher->removePath(path);
};  }
};
void MainWindow::updateRecentFiles(const QString& filePath)
{oid MainWindow::updateRecentFiles(const QString& filePath)
    // Remove duplicates and maintain order
    m_recentFiles.removeAll(filePath);order
    m_recentFiles.prepend(filePath););
    m_recentFiles.prepend(filePath);
    // Trim to max count
    while (m_recentFiles.size() > MAX_RECENT_FILES) {
        m_recentFiles.removeLast();AX_RECENT_FILES) {
    }   m_recentFiles.removeLast();
    }
    // Save to settings
    QSettings settings;
    settings.setValue("recentFiles", m_recentFiles);
    updateRecentFilesMenu();tFiles", m_recentFiles);
};  updateRecentFilesMenu();
};
void MainWindow::updateRecentFilesMenu()
{oid MainWindow::updateRecentFilesMenu()
    m_recentFilesMenu->clear();
    m_recentFilesMenu->clear();
    foreach (const QString& file, m_recentFiles) {
        QAction* action = m_recentFilesMenu->addAction(
            QFileInfo(file).fileName());enu->addAction(
        action->setData(file);leName());
        connect(action, &QAction::triggered, [this, file]() {
            loadFile(file);ction::triggered, [this, file]() {
        }); loadFile(file);
    }   });
    m_recentFilesMenu->addSeparator();
    m_recentFilesMenu->addAction("Clear History", [this]() {
        m_recentFiles.clear();on("Clear History", [this]() {
        QSettings().remove("recentFiles");
        updateRecentFilesMenu();ntFiles");
    }); updateRecentFilesMenu();
};  });
};
void MainWindow::highlightInCodeEditor(int nodeId) {
    qDebug() << "Highlighting node" << nodeId << "in code editor";
    if (m_nodeCodePositions.contains(nodeId)) {< "in code editor";
        const NodeInfo& info = m_nodeInfoMap[nodeId];
        highlightCodeSection(info.startLine, info.endLine);
        highlightCodeSection(info.startLine, info.endLine);
        // Center in editor
        QTextCursor cursor = m_nodeCodePositions[nodeId];
        codeEditor->setTextCursor(cursor);itions[nodeId];
        codeEditor->ensureCursorVisible();
    }   codeEditor->ensureCursorVisible();
};  }
};
void MainWindow::handleVisualizationResult(std::shared_ptr<GraphGenerator::CFGGraph> graph)
{oid MainWindow::handleVisualizationResult(std::shared_ptr<GraphGenerator::CFGGraph> graph)
    if (graph) {
        m_currentGraph = graph;
        visualizeCFG(graph);ph;
    }   visualizeCFG(graph);
...
{oid MainWindow::handleVisualizationError(const QString& error)
    QMessageBox::warning(this, "Visualization Error", error);
    setUiEnabled(true);g(this, "Visualization Error", error);
    statusBar()->showMessage("Visualization failed", 3000);
};  statusBar()->showMessage("Visualization failed", 3000);
};
void MainWindow::onErrorOccurred(const QString& message) {
    ui->reportTextEdit->setPlainText("Error: " + message);
    setUiEnabled(true);>setPlainText("Error: " + message);
    QMessageBox::critical(this, "Error", message);
    qDebug() << "Error occurred: " << message;ge);
};  qDebug() << "Error occurred: " << message;
};
void MainWindow::setUiEnabled(bool enabled) {
    QList<QWidget*> widgets = {ool enabled) {
        ui->browseButton, s = {
        ui->analyzeButton, 
        ui->searchButton,  
...
void MainWindow::dumpSceneInfo() {
    ...
    qDebug() << "Scene rect:" << m_scene->sceneRect();e();
...
void MainWindow::exportGraph(const QString& format) {
    if (!m_currentGraph) {ph(const QString& format) {
        QMessageBox::warning(this, "Export Error", "No graph to export");
        return;eBox::warning(this, "Export Error", "No graph to export");
    }   return;
    }
    QString fileName = getExportFileName(format);
    if (fileName.isEmpty()) {ortFileName(format);
    ...
    if (format.toLower() == "dot") {
        ...
        QFile file(fileName);t = generateValidDot(m_currentGraph);
        if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
            QTextStream stream(&file);Only | QIODevice::Text)) {
            stream << QString::fromStdString(dotContent);
            file.close();ring::fromStdString(dotContent);
            success = true;
        }   success = true;
    } else if (format.toLower() == "svg" || format.toLower() == "png" || format.toLower() == "pdf") {
        // Generate DOT, then use Graphviz to create the image= "png" || format.toLower() == "pdf") {
        QString tempDotFile = QDir::temp().filePath("temp_export.dot");
        std::string dotContent = generateValidDot(m_currentGraph);ot");
        std::string dotContent = generateValidDot(m_currentGraph);
        ...
        if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
            ...
            QFile::remove(tempDotFile); // Clean up temp file format);
...
{oid MainWindow::webChannelInitialized()
    ...
    m_webChannelReady = true;annel is ready for communication
    m_webChannelReady = true;
    // If we have pending visualization, display it now
    if (!m_pendingDotContent.isEmpty()) {display it now
        displayGraph(m_pendingDotContent, m_pendingProgressive, m_pendingRootNode);
        m_pendingDotContent.clear();tent, m_pendingProgressive, m_pendingRootNode);
    }   m_pendingDotContent.clear();
}   }
}
void MainWindow::graphRenderingComplete()
{oid MainWindow::graphRenderingComplete()
    ...
    if (ui->toggleFunctionGraph) { if needed
...
void MainWindow::onNodeClicked(const QString& nodeId) {
    bool ok;dow::onNodeClicked(const QString& nodeId) {
    int id = nodeId.toInt(&ok);
    if (!ok || !m_currentGraph) return;
    if (!ok || !m_currentGraph) return;
    ...
    selectNode(id);ode (this handles highlighting and content display)
    selectNode(id);
    // Emit the nodeClicked signal
    emit nodeClicked(nodeId);ignal
