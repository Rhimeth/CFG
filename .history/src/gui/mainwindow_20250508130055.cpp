...
void MainWindow::onNodeClicked(const QString& nodeId) {
    ...
    // Select the node (this will handle highlighting)
    selectNode(id);

    // Build enhanced node information display
    QString infoText;
    infoText += QString("=== Node %1 Information ===\n").arg(id);
    infoText += QString("Location: %1\n").arg(m_nodeInfoMap[id].filePath);
    infoText += QString("Code Boundaries: Lines %1-%2\n\n").arg(m_nodeInfoMap[id].startLine).arg(m_nodeInfoMap[id].endLine);
    
    // Show incoming connections
    infoText += "=== Incoming Connections ===\n";
    bool hasIncoming = false;
    for (const auto& [otherId, otherNode] : m_currentGraph->getNodes()) {
        if (std::find(otherNode.successors.begin(), otherNode.successors.end(), id) != otherNode.successors.end()) {
            hasIncoming = true;
            QString edgeType = m_currentGraph->isExceptionEdge(otherId, id) ? 
                " (exception edge)" : " (normal flow)";
            infoText += QString("From Node %1%2\n").arg(otherId).arg(edgeType);
        }
    }
    if (!hasIncoming) {
        infoText += "No incoming connections (entry point)\n";
    }
    ...
    if (nodeIter != nodes.end()) {
        if (nodeIter->second.successors.empty()) {
            infoText += "No outgoing connections (exit point)\n";
        } else {
            for (int successor : nodeIter->second.successors) {
                QString edgeType = m_currentGraph->isExceptionEdge(id, successor) ? 
                    " (exception edge)" : " (normal flow)";
                infoText += QString("To Node %1%2\n").arg(successor).arg(edgeType);
            }
        }
    }
    ...
    // Show node contents
    infoText += "\n=== Node Contents ===\n";
    for (const QString& stmt : m_nodeInfoMap[id].statements) {
        infoText += stmt + "\n";
    }

    // Display the enhanced information
    ui->reportTextEdit->setPlainText(infoText);
};
...
void MainWindow::centerOnNode(const QString& nodeId) {
    ...
    int id = nodeId.toInt(&ok);
    if (ok) {
        // Call your existing centerOnNode method that takes an integer
        centerOnNode(id);
    } else {
...
void MainWindow::highlightCodeSection(int startLine, int endLine) {
    ...
    // Add a status message to indicate the boundaries clearly
...
void MainWindow::selectNode(int nodeId) {
    ...
    // Highlight in code editor if we have mapping info
    ...
    // Center on the node in the graph view
    centerOnNode(nodeId);
}
