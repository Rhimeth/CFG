    ...
    try {
        ...
        for (const QString& line : lines) {
            ...
            if (nodeMatch.hasMatch()) {
                ...
                // Add a node name validation
                if (nodeIdStr.isEmpty() || nodeIdStr.startsWith("node")) {
                    qDebug() << "Invalid node ID format, skipping:" << nodeIdStr;
                    continue;
                }
        ...
        for (const QString& line : lines) {   
            ...
            if (edgeMatch.hasMatch()) {
                ...
                if (!ok1 || !ok2) {
                    qDebug() << "Invalid edge references:" << fromStr << "->" << toStr;
                    continue;
                }
                
                // Make sure both nodes exist - use a safe node lookup
                const auto& nodes = graph->getNodes();
                if (nodes.find(fromId) == nodes.end()) {
                    // Node doesn't exist, add it more safely
                ...
                // Only add edge if both nodes are now available
                if (graph->getNodes().find(fromId) != graph->getNodes().end() && 
                    ...
                    qWarning() << "Failed to create edge" << fromId << "->" << toId;
                }
            }
        }

        // Check if the graph still has nodes after edge processing
        if (graph->getNodes().empty()) {
            qWarning() << "Graph has no nodes after edge processing";
