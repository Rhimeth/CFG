...
void MainWindow::displayGraph(const QString& dotContent, bool isProgressive, int rootNode) 
{
    ...
    // Generate HTML with visualization - includes the WebChannel setup function definition
...
<head>
    ...
    <script>
        // Define all function declarations at the top level
        let currentRoot = %2;
        let bridge = null;
        ...
        function setupWebChannel() {
            if (typeof QWebChannel !== 'undefined' && qt && qt.webChannelTransport) {
                new QWebChannel(qt.webChannelTransport, function(channel) {
                    ...
                    // Only render after bridge is connected
                    if (bridge) {
                        renderGraph("%1");
                        
                        // Notify Qt that initialization is complete
                        if (typeof bridge.webChannelInitialized === 'function') {
                            bridge.webChannelInitialized();
                        }
                    } else {
                        console.error("Bridge object not available in channel");
                        document.getElementById("graph-container").innerHTML = 
                            '<div class="error-message">WebChannel bridge initialization failed</div>';
                    }
                });
            } else {
                console.error("QWebChannel not available - qt.webChannelTransport missing");
                document.getElementById("graph-container").innerHTML = 
                    '<div class="error-message">WebChannel initialization failed - missing transport</div>';
            }
        }
...
<body>
    ...
    connect(webView, &QWebEngineView::loadFinished, this, [this](bool success) {
        if (success) {
            ...
            // First inject the WebChannel script, then call setupWebChannel
            webView->page()->runJavaScript(
                "(function() {"
        ...
        // Disconnect signal after handling to prevent multiple connections
                    ...
                    <script>
                        try {
                            ...
                            viz.renderSVGElement(`%1`)
                                .then(svg => {
                                    document.getElementById('graph-container').appendChild(svg);
                                    // Initial scaling to fit
                                    const container = document.getElementById('graph-container');
                                    const svgElement = svg;
                                    
                                    // Make sure SVG takes appropriate space but doesn't overflow
                            ...
                            console.error(e);
                            document.getElementById('graph-container').innerHTML = 
                                '<p style="color:red;text-align:center;padding:20px;">Error initializing visualization</p>';
                        }
                    </script>
                </body>
                </html>
...
QString MainWindow::generateInteractiveDot(std::shared_ptr<GraphGenerator::CFGGraph> graph) {
    ...
    stream << "  compound=true;\n"; // Enable compound arrows
    stream << "  nodesep=0.6;\n";
    stream << "  ranksep=0.8;\n";
    stream << "  node [shape=box, style=\"rounded,filled\", fillcolor=lightblue, fontname=\"Arial\", fontsize=11];\n";
    stream << "  edge [arrowsize=0.8, fontname=\"Arial\", fontsize=9];\n\n";

    // Add nodes with more detailed attributes
    const auto& nodes = graph->getNodes();
    for (const auto& [id, node] : nodes) {
        stream << "  node" << id << " [";
        
        // Different styling for entry/exit nodes
        if (id == findEntryNode()) {
            stream << "label=\"ENTRY\", shape=ellipse, fillcolor=\"#C5E1A5\", penwidth=1.5";
        } else if (node.successors.empty()) {
            stream << "label=\"EXIT\", shape=ellipse, fillcolor=\"#FFCCBC\", penwidth=1.5";
        } else {
            // Escape special characters in node labels
            QString escapedLabel = escapeDotLabel(node.label);
            
            // Truncate long labels
            if (escapedLabel.length() > 40) {
                escapedLabel = escapedLabel.left(37) + "...";
            }
            
            stream << "label=\"" << escapedLabel << "\"";
            
            // Add node type indicators
            if (graph->isNodeTryBlock(id)) {
                stream << ", fillcolor=\"#B3E0FF\", penwidth=1.5";
            } else if (graph->isNodeThrowingException(id)) {
                stream << ", fillcolor=\"#FFCDD2\", penwidth=1.5";
            }
            
            // Add node ID as tooltip
            stream << ", tooltip=\"Node " << id << "\"";
        }
        
        // Add node ID data for click handling
        stream << ", id=\"node" << id << "\"";
        stream << "];\n";
    }
    ...
    for (const auto& [id, node] : nodes) {
        for (int successor : node.successors) {
            ...
            } else if (successor <= id) { // Likely a loop back edge
                stream << " [color=blue, constraint=false]";
            }
...
void MainWindow::highlightCodeSection(int startLine, int endLine) {
    ...
    // Add more visible boundary markers
...
void MainWindow::on_analyzeButton_clicked()
{
    ...
    if (ui->multiFileGroup->isChecked() && ui->fileListWidget->count() > 0) {
    ...
    try {
        ...
        if (!fileInfo.exists() || !fileInfo.isReadable()) {
            QMessageBox::warning(this, "Error", "File does not exist or is not readable");
        ...
        // Create a scope for the result to ensure proper cleanup
        {
            // Make a local copy of the analysis result
            CFGAnalyzer::AnalysisResult result = analyzer.analyzeFile(filePath);
            ...
            // Copy any data we need to keep before the result goes out of scope
            if (!result.dotOutput.empty()) {
                // Store DOT content safely
                QString dotContent = QString::fromStdString(result.dotOutput);
                displayGraph(dotContent);
                
                // Parse the DOT content if needed
                m_currentGraph = parseDotToCFG(dotContent);
                
                if (m_currentGraph) {
                    // Now visualize using our copy
                    visualizeCFG(m_currentGraph);
                }
            }
            
            // Handle report content safely
            ...
            if (!result.jsonOutput.empty()) {
                // Process JSON if needed, making a safe copy
                QString jsonContent = QString::fromStdString(result.jsonOutput);
                // Process the JSON content as needed
            }
            
            // The result will be safely destructed when we exit this scope
        }
...
QString MainWindow::getNodeAtPosition(const QPoint& pos) const {
    ...
    // Find the node at given coordinates
    ...
    if (m_nodeInfoMap.contains(nodeId)) {
        ...
        if (info.startLine > 0 && info.endLine >= info.startLine) {
            ...
            if (!info.filePath.isEmpty() && (m_currentFile != info.filePath)) {
                ...
                // Just highlight if the file is already loaded
                highlightCodeSection(info.startLine, info.endLine);
                // Center in editor
...
void MainWindow::graphRenderingComplete()
{
    ...
    // Reset zoom to ensure we can see the whole graph
    QTimer::singleShot(100, this, &MainWindow::resetViewZoom);
...
void MainWindow::highlightNodeInCodeEditor(int nodeId) {
    ...
    // Main background highlight - bright yellow for visibility
    QTextEdit::ExtraSelection selection;
    selection.format.setBackground(QColor(255, 255, 0, 180));  // Bright yellow with some transparency
    selection.format.setProperty(QTextFormat::FullWidthSelection, true);
    
    QTextCursor cursor(doc);
    cursor.setPosition(doc->findBlockByNumber(startLine-1).position());
    ...
    border.format.setProperty(QTextFormat::FrameBorderBrush, QColor(0, 128, 0));  // Green edge
    ...
    QTextCursor startCursor(doc->findBlockByNumber(startLine - 1));
    QTextEdit::ExtraSelection startLineHighlight;
    startLineHighlight.format.setBackground(QColor(100, 255, 100));  // Bright green
    ...
    // Add distinct end line marker if different (bright red)
    if (startLine != endLine) {
        QTextCursor endCursor(doc->findBlockByNumber(endLine - 1));
        QTextEdit::ExtraSelection endLineHighlight;
        endLineHighlight.format.setBackground(QColor(255, 100, 100));  // Bright red
        endLineHighlight.format.setProperty(QTextFormat::FullWidthSelection, true);
        endLineHighlight.cursor = endCursor;
        selections.append(endLineHighlight);
    }
    
    // Clear any previous selections and set new ones
    ui->codeEditor->setExtraSelections(selections);
    
    // Insert visible notification at the top of the editor
    QString message = QString("--- NODE %1 HIGHLIGHTED (LINES %2-%3) ---").arg(nodeId).arg(startLine).arg(endLine);
    QTextCursor notifyCursor = ui->codeEditor->textCursor();
    notifyCursor.movePosition(QTextCursor::Start);
    notifyCursor.insertText(message + "\n\n");
    
    // Scroll to make the selection visible with context
    QTextCursor scrollCursor(doc->findBlockByNumber(qMax(0, startLine - 3)));  // Show 3 lines above
    ui->codeEditor->setTextCursor(scrollCursor);
    ui->codeEditor->ensureCursorVisible();
    
    // Force immediate update
    ui->codeEditor->update();
    QApplication::processEvents();
    
    qDebug() << "Highlight applied for node" << nodeId << "with" << selections.size() << "selection items";
    
    // Show in status bar
    statusBar()->showMessage(QString("Node %1 selected (lines %2-%3)").arg(nodeId).arg(startLine).arg(endLine), 5000);
};
...
void MainWindow::onAddFileClicked()
{
    ...
    QStringList files = QFileDialog::getOpenFileNames(
        this,
        "Select Source Files",
        QFileInfo(ui->filePathEdit->text()).absolutePath(),
    ...
    // Debug output to verify files were selected
    ...
    for (const QString& filePath : files) {
        ...
        for (int i = 0; i < ui->fileListWidget->count(); i++) {
            if (ui->fileListWidget->item(i)->data(Qt::UserRole).toString() == filePath) {
                isDuplicate = true;
                break;
            }
        }
    ...
    // If the main file path is empty, use the first selected file
    if (ui->filePathEdit->text().isEmpty() && !files.isEmpty()) {
        ui->filePathEdit->setText(files.first());
        qDebug() << "Set main file path to:" << files.first();
    }
    
    // Update UI state
    ui->analyzeButton->setEnabled(ui->fileListWidget->count() > 0 || !ui->filePathEdit->text().isEmpty());
}
...
void MainWindow::onRemoveFileClicked()
{
    ...
    if (selectedItems.isEmpty()) {
        ...
        QMessageBox::information(this, "Information", "Please select files to remove");
        return;
    }
    
    foreach (QListWidgetItem* item, selectedItems) {
        QString filePath = item->data(Qt::UserRole).toString();
        qDebug() << "Removing file from list:" << filePath;
...
void MainWindow::onClearFilesClicked()
{
    if (ui->fileListWidget->count() > 0) {
        ...
        if (reply == QMessageBox::Yes) {
            ...
            qDebug() << "Cleared all files from list";
            
            // Update UI state
            ui->analyzeButton->setEnabled(!ui->filePathEdit->text().isEmpty());
        }
    } else {
        QMessageBox::information(this, "Information", "File list is already empty");
    }
}

void MainWindow::analyzeMultipleFiles()
{
    // Collect all files to analyze
    QStringList filePaths;
    
    // Add primary file if it exists and is not empty
    QString primaryFile = ui->filePathEdit->text().trimmed();
    if (!primaryFile.isEmpty()) {
        QFileInfo info(primaryFile);
        if (info.exists() && info.isReadable()) {
            filePaths.append(primaryFile);
        }
    }
    
    // Add all files from the list widget
    for (int i = 0; i < ui->fileListWidget->count(); i++) {
        QString filePath = ui->fileListWidget->item(i)->data(Qt::UserRole).toString();
    ...
    if (filePaths.isEmpty()) {
        QMessageBox::warning(this, "Error", "No valid files to analyze");
        return;
    }
    
    qDebug() << "Analyzing multiple files:" << filePaths;
    
    // ...existing code...
    QApplication::setOverrideCursor(Qt::WaitCursor);
    
    try {
        // Show progress dialog for multi-file analysis
        ...
        // Convert file paths to std::vector<std::string> for analyzer
        ...
        // Set up a progress callback function
        auto progressCallback = [&progress](int current, int total) -> bool {
        ...
        // Analyze all files
        auto result = analyzer.analyzeMultipleFiles(stdFilePaths, progressCallback);
        
        if (progress.wasCanceled()) {
            statusBar()->showMessage("Analysis canceled", 3000);
            QApplication::restoreOverrideCursor();
            return;
        }
        ...
        QString report = "Multi-File Analysis Results\n========================\n\n";
        report += QString("Files analyzed: %1\n\n").arg(filePaths.size());
        report += QString::fromStdString(result.report);
        
        ui->reportTextEdit->setPlainText(report);
        statusBar()->showMessage("Multi-file analysis completed", 3000);
...
void MainWindow::initializeApplication() {
    ...
    // Initialize components after UI is fully set up
    ...
    // Set the window title to include version
    setWindowTitle("CFG Parser - Control Flow Graph Analyzer");
...
void MainWindow::showVisualizationContextMenu(const QPoint& pos) {
    ...
    QAction* exportAction = contextMenu.addAction("Export Graph");
    QMenu* exportSubMenu = contextMenu.addMenu("Export As...");
    exportSubMenu->addAction("PNG Image");
    exportSubMenu->addAction("SVG File");
    exportSubMenu->addAction("DOT File");
    exportSubMenu->addAction("PDF Document");
    
    contextMenu.addSeparator();
    
    QMenu* layoutMenu = contextMenu.addMenu("Layout");
    QAction* hierarchicalAction = layoutMenu->addAction("Hierarchical");
    QAction* forceDirectedAction = layoutMenu->addAction("Force-Directed");
    QAction* circularAction = layoutMenu->addAction("Circular");
    ...
    QAction* blueThemeAction = themeMenu->addAction("Blue");
    
    // Execute the menu and handle the result
    QAction* selectedAction = contextMenu.exec(webView->mapToGlobal(pos));
    
    if (selectedAction == zoomInAction) {
        webView->page()->runJavaScript("document.body.style.zoom = (parseFloat(document.body.style.zoom || 1) * 1.2);");
    }
    else if (selectedAction == zoomOutAction) {
        webView->page()->runJavaScript("document.body.style.zoom = (parseFloat(document.body.style.zoom || 1) / 1.2);");
    }
    else if (selectedAction == resetZoomAction) {
        webView->page()->runJavaScript("document.body.style.zoom = 1;");
    }
    else if (selectedAction == exportAction || selectedAction == exportSubMenu->actions().at(0)) {
        exportGraph("png");
    }
    else if (selectedAction == exportSubMenu->actions().at(1)) {
        exportGraph("svg");
    }
    else if (selectedAction == exportSubMenu->actions().at(2)) {
        exportGraph("dot");
    }
    else if (selectedAction == exportSubMenu->actions().at(3)) {
        exportGraph("pdf");
    }
    else if (selectedAction == hierarchicalAction) {
        m_currentLayoutAlgorithm = Hierarchical;
        if (m_graphView) m_graphView->applyHierarchicalLayout();
        
        // Apply layout changes in web view as well
        if (webView) {
            webView->page()->runJavaScript(
                "if (window.bridge) { window.bridge.updateLayout('hierarchical'); }"
            );
        }
    }
    else if (selectedAction == forceDirectedAction) {
        m_currentLayoutAlgorithm = ForceDirected;
        if (m_graphView) m_graphView->applyForceDirectedLayout();
        
        // Apply layout changes in web view as well
        if (webView) {
            webView->page()->runJavaScript(
                "if (window.bridge) { window.bridge.updateLayout('force'); }"
            );
        }
    }
    else if (selectedAction == circularAction) {
        m_currentLayoutAlgorithm = Circular;
        if (m_graphView) m_graphView->applyCircularLayout();
        
        // Apply layout changes in web view as well
        if (webView) {
            webView->page()->runJavaScript(
                "if (window.bridge) { window.bridge.updateLayout('circular'); }"
            );
        }
    }
    else if (selectedAction == lightThemeAction) {
        setGraphTheme(m_availableThemes["Light"]);
        
        // Immediately apply theme in HTML view
        if (webView) {
            webView->page()->runJavaScript(
                "document.body.style.background = '#f8f8f8';"
                "document.querySelectorAll('.node rect, .node ellipse, .node polygon').forEach(el => {"
                "el.setAttribute('fill', '#E8F4F8');"
                "el.setAttribute('stroke', '#2B6CB0');"
                "});"
                "document.querySelectorAll('.edge path').forEach(el => {"
                "el.setAttribute('stroke', '#718096');"
                "});"
            );
        }
    }
    else if (selectedAction == darkThemeAction) {
        setGraphTheme(m_availableThemes["Dark"]);
    ...
    else if (selectedAction == blueThemeAction) {
        setGraphTheme(m_availableThemes["Blue"]);
...
void MainWindow::resetViewZoom() {
    // Reset the zoom level for the QGraphicsView
    if (m_graphView && m_graphView->scene()) {
        // First ensure the scene rect is properly set
        QRectF itemsRect = m_graphView->scene()->itemsBoundingRect();
        m_graphView->scene()->setSceneRect(itemsRect);
        
        // Reset transform and fit view, leaving small margins
        m_graphView->resetTransform();
        m_graphView->fitInView(itemsRect, Qt::KeepAspectRatio);
        
        // Add a small scaling factor to ensure there's margin around the graph
        m_graphView->scale(0.95, 0.95);
    }
    
    // For web view, reset zoom with JavaScript
    if (webView) {
        webView->page()->runJavaScript(
            "document.body.style.zoom = 1.0;"
            "if (document.querySelector('svg')) {"
            "  var svg = document.querySelector('svg');"
            "  svg.style.maxWidth = '98%';"
            "  svg.style.maxHeight = '98%';"
            "  svg.style.margin = '1%';"
            // Make sure all labels are visible
            "  document.querySelectorAll('.node text').forEach(function(text) {"
