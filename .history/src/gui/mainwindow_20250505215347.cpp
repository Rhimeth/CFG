...
void MainWindow::displayGraph(const QString& dotContent) {
    ...
    // Disconnect any previous connections to prevent callback conflicts
    disconnect(webView, &QWebEngineView::loadFinished, nullptr, nullptr);
    
    // Create a fresh WebChannel to avoid stale callbacks
    if (m_webChannel) {
        delete m_webChannel;
    }
    m_webChannel = new QWebChannel(this);
    
    // Register this object with a consistent name
    m_webChannel->registerObject("bridge", this);
    webView->page()->setWebChannel(m_webChannel);

    // Store the DOT content for potential exports
...
<body>
    ...
    <script>
        ...
        let bridge = null;
        let highlighted = { node: null, edge: null };
        let collapsedNodes = {};
        let graphData = {};
        ...
        // Called when a method isn't available
        function onBridgeMethodMissing(methodName) {
            showError(`Method not available: ${methodName}`);
            console.error(`Qt bridge method not found: ${methodName}`);
            console.log('Available methods:', bridge ? Object.keys(bridge).join(', ') : 'bridge is null');
        }

        // Safely call a bridge method
        function callBridgeMethod(methodName, ...args) {
            if (!bridge) {
                showError('Bridge not initialized');
                return;
            }
            ...
            if (typeof bridge[methodName] === 'function') {
                try {
                    ...
                    console.log(`Called ${methodName}(${args.join(', ')})`);
                } catch (e) {
                    showError(`Error calling ${methodName}: ${e.message}`);
                    console.error('Call error:', e);
                }
            } else {
                onBridgeMethodMissing(methodName);
            }
        }

        function toggleNodeExpansion(nodeId, shouldExpand) {
            const node = document.getElementById('node' + nodeId);
            if (!node) return;
    
            if (shouldExpand) {
                // Fetch details from Qt
                window.bridge.getNodeDetails(nodeId).then(function(details) {
                    // Create expansion element
                    const expandDiv = document.createElement('div');
                    expandDiv.className = 'node-expansion';
                    expandDiv.innerHTML = '<div class="node-details">' + details + '</div>';
                ...
                // Remove expansion
                const expandDiv = node.querySelector('.node-expansion');
                if (expandDiv) {
                    node.removeChild(expandDiv);
                }
                node.classList.remove('expanded');
            }
        }

        // Initialize WebChannel exactly once
        function initWebChannel() {
            ...
            new QWebChannel(qt.webChannelTransport, function(channel) {
                ...
                if (bridge) {
                    // Signal successful connection to Qt
                    if (typeof bridge.webChannelInitialized === 'function') {
                        bridge.webChannelInitialized();
                    }
                } else {
                    showError('Failed to obtain bridge object');
                }
            });
        }

        // Ensure document is loaded before working with DOM
        ...
        function toggleNode(nodeId) {
            ...
            if (collapsedNodes[nodeId]) {
                callBridgeMethod('handleNodeCollapse', nodeId.toString());
            } else {
                callBridgeMethod('handleNodeExpand', nodeId.toString());
            }
        }
        ...
        viz.renderSVGElement(dot)
            .then(svg => {
                ...
                svg.addEventListener('click', (e) => {
                    ...
                    if (node) {
                        ...
                        if (graphData[nodeId]?.isCollapsible) {
                            ...
                            callBridgeMethod('handleNodeClick', nodeId);
                        }
                    } 
                    else if (edge) {
                        ...
                        if (edgeId) {
                            ...
                            if (from && to) {
                                ...
                                callBridgeMethod('handleEdgeClick', from, to);
                            }
                        }
                    }
                });
                ...
                if (container) {
                    ...
                    // Signal to Qt that rendering is complete
    ...
    // Connect only once to handle load completion
    connect(webView, &QWebEngineView::loadFinished, [this](bool success) {
        if (!success) {
...
// Add these Q_INVOKABLE methods to support the JavaScript calls
Q_INVOKABLE void MainWindow::webChannelInitialized() {
    qDebug() << "WebChannel successfully initialized from JavaScript";
}

Q_INVOKABLE void MainWindow::graphRenderingComplete() {
    qDebug() << "Graph rendering completed in JavaScript";
}
...
void MainWindow::visualizeCFG(std::shared_ptr<GraphGenerator::CFGGraph> graph)
{
    ...
    try {
        std::string dotContent = generateInteractiveDot(graph);
...
std::string MainWindow::generateInteractiveDot(std::shared_ptr<GraphGenerator::CFGGraph> graph) 
{
    if (!graph) {
        return R"(digraph G {
            label="Empty Graph";
...
std::string MainWindow::generateProgressiveDot(std::shared_ptr<GraphGenerator::CFGGraph> graph, int startNode)
{
    std::stringstream dot;
    dot << "digraph G {\n";
    dot << "  rankdir=TB;\n";
    dot << "  node [shape=rectangle, style=\"rounded,filled\"];\n\n";
    
    // Mark starting node
    dot << "  node" << startNode << " [fillcolor=\"#4CAF50\", penwidth=2];  // Green for start\n";
    
    // Add visible nodes and edges
    for (const auto& [id, node] : graph->getNodes()) {
        if (node.visible) {
            dot << "  node" << id << " [label=\"" << escapeDotLabel(node.label) << "\"];\n";
            
            for (int succ : node.successors) {
                if (graph->getNodes().at(succ).visible) {
    ...
    // Add expandable nodes (grayed out)
    for (const auto& [id, node] : graph->getNodes()) {
        if (!node.visible && node.expanded) {
            dot << "  node" << id << " [label=\"+\", fillcolor=\"#CCCCCC\", shape=ellipse];\n";
            dot << "  node" << id << " -> node" << id << "_hidden [style=dashed, color=gray];\n";
        }
    }
...
std::shared_ptr<GraphGenerator::CFGGraph> MainWindow::parseDotToCFG(const QString& dotContent) {
    ...
    // Improved regex patterns with better node and edge detection
    static const QRegularExpression nodeRegex("\\b([\\w\\d_]+)\\s*\\[([^\\]]*)]");
    static const QRegularExpression edgeRegex("\\b([\\w\\d_]+)\\s*->\\s*([\\w\\d_]+)(?:\\s*\\[([^\\]]*)])?");
    ...
    try {
        ...
        // Validate regex patterns to avoid runtime issues
        ...
        for (const QString& line : lines) {
            ...
            if (trimmed.isEmpty() || trimmed.startsWith("//") || 
                ...
                trimmed.startsWith("graph") || trimmed.startsWith("node") ||
                trimmed.startsWith("edge") || trimmed.startsWith("rankdir") ||
                trimmed == "{" || trimmed == "}") {
                continue;
            }
            ...
            if (nodeMatch.hasMatch()) {
                ...
                // Skip reserved identifiers that might cause confusion
                if (nodeName == "node" || nodeName == "edge" || nodeName == "graph") {
                    continue;
                }
                
                // Generate a safe, unique node ID
                int nodeId;
        ...
        for (const QString& line : lines) {
            ...
            if (edgeMatch.hasMatch()) {
                ...
                // Skip if using reserved identifiers
                if (fromName == "node" || fromName == "edge" || fromName == "graph" ||
                    toName == "node" || toName == "edge" || toName == "graph") {
                    continue;
                }
                ...
                if (nodeNameToId.contains(fromName)) {
                    fromId = nodeNameToId[fromName];
                } else {
                    fromId = nextId++;
                    nodeNameToId[fromName] = fromId;
                    graph->addNode(fromId, fromName);
                }
                
                if (nodeNameToId.contains(toName)) {
                    toId = nodeNameToId[toName];
                } else {
                    toId = nextId++;
                    nodeNameToId[toName] = toId;
                    graph->addNode(toId, toName);
                }
                
                // Add edge safely if both nodes exist
        ...
        qDebug() << "Parsed CFG with" << graph->getNodes().size() << "nodes";
    }
    catch (const std::exception& e) {
...
void MainWindow::onNodeClicked(const QString& nodeId)
{
    ...
    // First click - initialize visualization
    if (!nodes.at(id).visible) {
        // Reset all nodes
        ...
        // Show starting node
        nodes.at(id).visible = true;
        nodes.at(id).expanded = true;
    } 
    // Subsequent clicks - expand node
    else {
        nodes.at(id).expanded = true;
        for (int succ : nodes.at(id).successors) {
            nodes.at(succ).visible = true;
        }
    }

    // Update visualization
    std::string dot = generateProgressiveDot(m_currentGraph, id);
    displayGraph(QString::fromStdString(dot));
};
