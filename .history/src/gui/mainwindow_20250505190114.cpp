...
std::shared_ptr<GraphGenerator::CFGGraph> MainWindow::parseDotToCFG(const QString& dotContent) {
    ...
    static const QRegularExpression edgeRegex(R"(node(\d+)\s*->\s*node(\d+))");
    ...
    try {
        ...
        for (const QString& line : lines) {
            ...
            if (edgeMatch.hasMatch()) {
                ...
                const QString toStr = edgeMatch.captured(2);

                bool ok1, ok2;
                int fromId = fromStr.toInt(&ok1);
                int toId = toStr.toInt(&ok2);
