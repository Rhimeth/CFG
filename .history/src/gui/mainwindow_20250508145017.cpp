...
void MainWindow::selectNode(int nodeId) {
    ...
    if (currentNode != nodes.end() && !currentNode->second.successors.empty()) {
        for (int successor : currentNode->second.successors) {
            ...
            // Add a clickable link for navigation
            report += QString("• <a href=\"node:%1\">Node %1%2%3</a>\n")
                      .arg(successor)
                      .arg(calleeInfo)
                      .arg(edgeType);
        }
    ...
    // Connect the text browser to handle links
    connect(ui->reportTextEdit, &QTextEdit::anchorClicked, this, [this](const QUrl& url) {
        if (url.scheme() == "node") {
            bool ok;
