...
bool analyzeAndVisualizeMultipleFiles(const QStringList& filePaths, QGraphicsScene* scene) {
    ...
    // Debug information
    qDebug() << "Starting multi-file analysis with" << filePaths.size() << "files";

    CFGAnalyzer::CFGAnalyzer analyzer;
    auto result = analyzer.analyzeMultipleFiles(stdFilePaths);
    
    if (!result.success) {
        qWarning() << "Multi-file analysis failed:" << QString::fromStdString(result.report);
        return false;
    }

    // Write the combined DOT to a temporary file
    QString dotFilePath = tempDir.filePath("combined_cfg.dot");
    QFile dotFile(dotFilePath);
    if (!dotFile.open(QIODevice::WriteOnly | QIODevice::Text)) {
        qWarning() << "Failed to open temp file for writing:" << dotFilePath;
        return false;
    }
    
    // Write DOT content to file
    QTextStream outStream(&dotFile);
    outStream << QString::fromStdString(result.dotOutput);
    dotFile.close();

    // Render the combined DOT
