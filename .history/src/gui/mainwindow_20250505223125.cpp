...
int MainWindow::findEntryNode()
{
    ...
    // Simple implementation - finds node with no predecessors
    std::unordered_set<int> hasPredecessors;
    
    for (const auto& [id, node] : nodes) {
        for (int succ : node.successors) {
            hasPredecessors.insert(succ);
        }
    }
    
    for (const auto& [id, node] : nodes) {
        if (hasPredecessors.find(id) == hasPredecessors.end()) {
            return id;
        }
    }
    
    return nodes.begin()->first; // Fallback to first node
};
