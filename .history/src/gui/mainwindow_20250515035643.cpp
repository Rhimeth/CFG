...
void MainWindow::analyzeMultipleFiles()
{
    if (m_loadedFiles.isEmpty()) {
        QMessageBox::warning(this, tr("No Files Selected"), 
            tr("Please add files for analysis first."));
        return;
    }
    ...
    QProgressDialog progress(tr("Analyzing files..."), tr("Cancel"), 0, m_loadedFiles.size(), this);
    ...
    // Analyze the files
    CFGAnalyzer::AnalysisResult result = m_analyzer.analyzeMultipleFiles(
        filePaths, progressCallback);
    
    progress.setValue(m_loadedFiles.size());
