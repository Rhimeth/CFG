...
void MainWindow::displayGraph(const QString& dotContent, bool isProgressive, int rootNode) 
{
    ...
    QString escapedDot = escapeDotLabel(QString::fromStdString(processedDot));
...
std::string MainWindow::generateProgressiveDot(const QString& fullDot, int rootNode) 
{
    ...
    for (auto it = adjacencyList.begin(); it != adjacencyList.end(); ++it) {
        ...
        for (int to : it.value()) {
            if (m_visibleNodes[to]) {
                ...
                stream << "  node" << to << " [label=\"+\", shape=ellipse, fillcolor=\"#9E9E9E\"];\n";
