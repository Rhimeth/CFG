...
{
    // Validate inputs and UI state
    if (!graph) {
        qWarning() << "Null graph provided";
        QMessageBox::warning(this, "Error", "No graph data to visualize");
        return;
    }

    if (!webView || !webView->isVisible()) {
        qWarning() << "Web view not available or not visible";
        // Fall back to text display if web view fails
        std::string dotContent = generateValidDot(graph.get());
        ui->reportTextEdit->setPlainText(QString::fromStdString(dotContent));
        return;
    }

    try {
        // Generate DOT content with additional validation
        std::string dotContent = generateInteractiveDot(graph.get());
        if (dotContent.empty()) {
            throw std::runtime_error("Generated empty DOT content");
        }

        // Store the graph only after successful generation
        m_currentGraph = graph;

        // Generate HTML with proper error handling
        QString html = generateInteractiveGraphHtml(QString::fromStdString(dotContent));
        if (html.isEmpty()) {
            throw std::runtime_error("Generated empty HTML content");
        }

        // Safely update the web view
        QMetaObject::invokeMethod(this, [this, html]() {
            if (webView) {  // Check again in case webView was deleted
                webView->setHtml(html);
                statusBar()->showMessage("Graph visualized", 3000);
            }
        }, Qt::QueuedConnection);
        ...
        // Show error in both message box and report area
        QString errorMsg = QString("Failed to visualize graph:\n%1").arg(e.what());
        QMessageBox::critical(this, "Error", errorMsg);
        ui->reportTextEdit->append(errorMsg);
        
        // Fall back to basic DOT display
        if (graph) {
            std::string dotContent = generateValidDot(graph.get());
            ui->reportTextEdit->setPlainText(QString::fromStdString(dotContent));
        }
