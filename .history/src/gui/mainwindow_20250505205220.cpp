...
{
    // Debug checks
    qDebug() << "Entering visualizeCFG on thread:" << QThread::currentThread();
    
    if (!graph) {
        qCritical() << "Null graph provided";
        return;
    }

    // Thread-safe Graphviz operations
    QMutexLocker locker(&m_graphvizMutex);
    
    try {
        std::string dotContent;
        {
            // Isolate Graphviz operations
            QProcess dotProcess;
            QStringList args = {"-Tdot"};
            dotProcess.start("dot", args);
            if (!dotProcess.waitForStarted()) {
                throw std::runtime_error("Failed to start Graphviz");
            }
            
            // Write your graph data to the process...
            dotProcess.waitForFinished();
            dotContent = dotProcess.readAllStandardOutput().toStdString();
        }  // Process automatically cleaned up here

        // Ensure we're on the GUI thread for Qt operations
        QMetaObject::invokeMethod(this, [this, dotContent]() {
            if (!webView) return;
            
            QString html = generateInteractiveGraphHtml(
