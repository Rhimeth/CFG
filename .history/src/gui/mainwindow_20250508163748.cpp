...
void MainWindow::displayGraph(const QString& dotContent, bool isProgressive, int rootNode) 
{
    ...
    // Generate HTML with visualization - Use custom delimiter to avoid issues
    QString html = QString(R"delimiter(
<!DOCTYPE html>
<html>
<head>
    <title>CFG Visualization</title>
    ...
    <style>
        body { margin:0; padding:0; background:#2D2D2D; height:100vh; width:100vw; overflow:hidden; }
        #graph-container { 
            width:100%; 
            height:100%; 
            overflow:auto;
...
<body>
    ...
    <script>
        ...
        function zoomToFit() {
            ...
            currentTranslate.x = (containerWidth - scaledWidth) / 2 - bbox.x * currentScale;
            currentTranslate.y = (containerHeight - scaledHeight) / 2 - bbox.y * currentScale;
        ...
        function renderGraph(dot) {
            ...
            viz.renderSVGElement(dot)
                .then(svgElement => {
                    ...
                    document.getElementById("graph-container").appendChild(svgElement);
                    svg = svgElement;
                    ...
                    document.addEventListener("mousemove", function(e) {
                        if (isDragging) {
                            ...
                            currentTranslate.x = startTranslateX + dx;
                            currentTranslate.y = startTranslateY + dy;
                            applyTransform();
                        }
                    });
                    ...
                    // Add click handlers
                    svgElement.querySelectorAll("[id^=node]").forEach(node => {
                    ...
                    svgElement.addEventListener("wheel", function(e) {
                        if (e.ctrlKey) {
                            ...
                            currentTranslate.x = mouseX - (mouseX - currentTranslate.x) * currentScale / prevScale;
                            currentTranslate.y = mouseY - (mouseY - currentTranslate.y) * currentScale / prevScale;
...
)delimiter").arg(escapedDot).arg(rootNode);
...
void MainWindow::selectNode(int nodeId) {
    ...
    // Build node summary report with HTML formatting for better readability
    QString report = QString("<div style='background-color:#f0f0f0; padding:10px; border-left:4px solid #4CAF50;'>"
                           "<h3>Node %1 Summary</h3>").arg(nodeId);
    
    report += QString("<p><b>File:</b> %1</p>").arg(nodeInfo.filePath);
    report += QString("<p><b>Lines:</b> %1-%2</p>").arg(nodeInfo.startLine).arg(nodeInfo.endLine);
    if (!nodeInfo.functionName.isEmpty()) {
        report += QString("<p><b>Function:</b> %1</p>").arg(nodeInfo.functionName);
    }
    
    // Called by section
    report += "<h4>Called By:</h4><ul>";
    ...
    for (const auto& [otherId, otherNode] : m_currentGraph->getNodes()) {
        if (std::find(otherNode.successors.begin(), otherNode.successors.end(), nodeId) != otherNode.successors.end()) {
            ...
            QString edgeType = m_currentGraph->isExceptionEdge(otherId, nodeId) ? 
                "<span style='color:red;'>(exception path)</span>" : "<span style='color:green;'>(normal flow)</span>";
            if (m_nodeInfoMap.contains(otherId)) {
                const auto& caller = m_nodeInfoMap[otherId];
                report += QString("<li>Node %1 [Lines %2-%3] %4</li>")
                          .arg(otherId).arg(caller.startLine).arg(caller.endLine).arg(edgeType);
            } else {
                report += QString("<li>Node %1 %2</li>").arg(otherId).arg(edgeType);
            }
        }
    }
    if (!hasIncoming) {
        report += "<li>None (entry point)</li>";
    }
    report += "</ul>";

    // Calls to section
    report += "<h4>Calls To:</h4><ul>";
    const auto& nodes = m_currentGraph->getNodes();
    ...
    if (currentNode != nodes.end() && !currentNode->second.successors.empty()) {
        for (int successor : currentNode->second.successors) {
            ...
            if (m_nodeInfoMap.contains(successor)) {
                ...
                report += QString("<li>Node %1 %2</li>").arg(successor).arg(edgeType);
            }
        }
    } else {
        report += "<li>None (exit point)</li>";
    }
    report += "</ul>";

    // Code content section with syntax highlighting
    report += "<h4>Code Content:</h4>";
    if (!nodeInfo.statements.isEmpty()) {
        report += "<pre style='background-color:#f8f8f8; padding:8px; border-left:2px solid #ddd; overflow:auto;'>";
        for (const QString& stmt : nodeInfo.statements) {
            // Apply basic HTML escaping
            QString escapedStmt = stmt;
            escapedStmt.replace("<", "&lt;").replace(">", "&gt;");
            report += escapedStmt + "\n";
        }
        report += "</pre>";
    } else {
        report += "<p>[No code content available]</p>";
    }
    
    report += "</div>";
    
    // Display the formatted report in the analysis panel
    ui->reportTextEdit->clear();
    ui->reportTextEdit->setHtml(report);
    ...
    webView->page()->runJavaScript(
        QString("(function() {"
                ...
                "            const centerX = containerWidth/2 - (ctm.e + bbox.width/2);"
                "            const centerY = containerHeight/2 - (ctm.f + bbox.height/2);"
...
void MainWindow::handleEdgeClick(const QString& fromId, const QString& toId) {
    ...
    if (ok1 && ok2 && m_currentGraph) {
        ...
        // Create a more detailed edge report
        QString edgeReport = QString("<div style='background-color:#f0f0f0; padding:10px; border-left:4px solid #FFA500;'>"
                                    "<h3>Edge Details: %1 → %2</h3>"
                                    "<p><b>Type:</b> %3</p>")
                            .arg(from).arg(to).arg(edgeType);
        
        // Add source and target node information
        if (m_nodeInfoMap.contains(from) && m_nodeInfoMap.contains(to)) {
            const NodeInfo& sourceInfo = m_nodeInfoMap[from];
            const NodeInfo& targetInfo = m_nodeInfoMap[to];
            
            edgeReport += QString("<h4>Source Node (%1):</h4>").arg(from);
            if (!sourceInfo.functionName.isEmpty()) {
                edgeReport += QString("<p><b>Function:</b> %1</p>").arg(sourceInfo.functionName);
            }
            edgeReport += QString("<p><b>File:</b> %1 (Lines %2-%3)</p>")
                         .arg(QFileInfo(sourceInfo.filePath).fileName())
                         .arg(sourceInfo.startLine)
                         .arg(sourceInfo.endLine);
            
            edgeReport += QString("<h4>Target Node (%1):</h4>").arg(to);
            if (!targetInfo.functionName.isEmpty()) {
                edgeReport += QString("<p><b>Function:</b> %1</p>").arg(targetInfo.functionName);
            }
            edgeReport += QString("<p><b>File:</b> %1 (Lines %2-%3)</p>")
                         .arg(QFileInfo(targetInfo.filePath).fileName())
                         .arg(targetInfo.startLine)
                         .arg(targetInfo.endLine);
        }
        
        edgeReport += "</div>";
        
        // Display the formatted edge report
        ui->reportTextEdit->clear();
        ui->reportTextEdit->setHtml(edgeReport);
