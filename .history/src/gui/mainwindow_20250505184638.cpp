...
std::shared_ptr<GraphGenerator::CFGGraph> MainWindow::parseDotToCFG(const QString& dotContent) {
    ...
    // Pre-compiled regex patterns with standard string literals
    static const QRegularExpression nodeRegex(R"(node(\d+)\s*\[([^\]]*)\])");
    static const QRegularExpression edgeRegex(R"(edge\d+_\d+\s*\[([^\]]*)\])");
    static const QRegularExpression attrRegex("(\\w+)\\s*=\\s*(?:\"([^\"]*)\"|([^\\s;,]+))");
    ...
    try {
        ...
        for (const QString& line : lines) {
            ...
            if (nodeMatch.hasMatch()) {
                nodesFound = true;
                const QString nodeIdStr = nodeMatch.captured(1);
                const QString attributes = nodeMatch.captured(2);
                
                // Convert node ID
                bool ok;
                int id = nodeIdStr.toInt(&ok);
                if (!ok && nodeIdStr.startsWith("B")) {
                    id = nodeIdStr.mid(1).toInt(&ok);
                }
                if (!ok) {
                    id = qHash(nodeIdStr);
                    qDebug("Converted non-numeric node ID %s to %d", 
                          qPrintable(nodeIdStr), id);
                }

                graph->addNode(id);
                m_expandedNodes[id] = false;

                QString attributes = nodeMatch.captured(2);
                m_nodeDetails[id] = parseNodeAttributes(attributes);
                
                // Parse node attributes
                QString label, functionName, filename;
                int startLine = -1, endLine = -1;
                bool isTryBlock = false;
                bool throwsException = false;
                
                auto attrIter = attrRegex.globalMatch(attributes);
                while (attrIter.hasNext()) {
                    auto match = attrIter.next();
                    const QString key = match.captured(1).toLower();
                    const QString value = match.captured(2).isEmpty() ? 
                                        match.captured(3) : match.captured(2);

                    if (key == "label") {
                        label = value;
                        graph->addStatement(id, value);
                    } 
                    else if (key == "function") {
                        functionName = value;
                        graph->setNodeFunctionName(id, value);
                    }
                    else if (key == "location") {
                        auto parts = value.split(':');
                        if (parts.size() == 2) {
                            filename = parts[0];
                            auto range = parts[1].split('-');
                            if (range.size() == 2) {
                                startLine = range[0].toInt();
                                endLine = range[1].toInt();
                        ...
                        throwsException = true;
                            ...
                            (value == "lightpink" || value.startsWith("#ff"))) {
                        throwsException = true;
                    }
                }

                if (isTryBlock) graph->markNodeAsTryBlock(id);
                if (throwsException) graph->markNodeAsThrowingException(id);
            ...
            if (edgeMatch.hasMatch()) {
                const QString fromStr = edgeMatch.captured(1);
                const QString toStr = edgeMatch.captured(3);
                const QString edgeAttrs = edgeMatch.captured(4);

                auto convertId = [](const QString& idStr) {
                    bool ok;
                    int id = idStr.toInt(&ok);
                    if (!ok && idStr.startsWith("B")) {
                        id = idStr.mid(1).toInt(&ok);
                    }
                    return ok ? id : qHash(idStr);
                };

                const int fromId = convertId(fromStr);
                const int toId = convertId(toStr);

                // Ensure nodes exist
                if (graph->getNodes().find(fromId) == graph->getNodes().end()) {
                    graph->addNode(fromId);
                }
                if (graph->getNodes().find(toId) == graph->getNodes().end()) {
                    graph->addNode(toId);
                }
                ...
                if (!edgeAttrs.isEmpty()) {
                    auto attrMatch = attrRegex.match(edgeAttrs);
                    if (attrMatch.hasMatch()) {
                        const QString key = attrMatch.captured(1).toLower();
                        const QString value = attrMatch.captured(2).isEmpty() ? 
                                           attrMatch.captured(3) : attrMatch.captured(2);
        ...
        qDebug("Parsed CFG with %zu nodes", graph->getNodes().size());
    }
    catch (const std::exception& e) {
        qCritical("Exception in parseDotToCFG: %s", e.what());
