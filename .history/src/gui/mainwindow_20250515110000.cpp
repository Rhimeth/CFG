...
void MainWindow::initializeWebChannel()
{
    ...
    // Make sure web view is properly initialized
    if (webView && webView->page()) {
        ...
        webView->page()->runJavaScript(
            ...
            [this](const QVariant& result) {
                // Ensure we retry display if we had pending content
                if (!m_pendingDotContent.isEmpty()) {
                    QTimer::singleShot(300, this, [this]() {
                        QString content = m_pendingDotContent;
                        m_pendingDotContent.clear();
                        m_webChannelReady = true; // Mark as ready before displaying
...
{
    // Get the fileListWidget
    QListWidget* fileListWidget = ui->centralwidget->findChild<QListWidget*>("fileListWidget");
    if (!fileListWidget || fileListWidget->count() == 0) {
        // Fall back to single file from the filePathEdit if file list is empty
    ...
    // Collect all files from the file list widget
    ...
    // Show the cursor with wait state
    ...
    try {
        ...
        // First try analyzing all files as a group
        auto result = analyzer.analyzeFiles(sourceFiles);
        if (!result.success) {
            // If analyzing as a group failed, try analyzing each file separately
            ...
            for (const auto& file : sourceFiles) {
                ...
                if (singleResult.success) {
                    // Extract subgraph from DOT output and append
...
// Helper function to filter DOT output for inclusion in merged graph
