...
void MainWindow::visualizeCurrentGraph() {
    ...
    QString dot = Visualizer::generateDotRepresentation(m_currentGraph.get());
...
void MainWindow::connectSignals() {
    connect(ui->analyzeButton, &QPushButton::clicked, this, [this](){
        QString filePath = ui->filePathEdit->text();
        if (!filePath.isEmpty()) {
            std::vector<QString> sourceFiles = { filePath.toStdString() };
...
void MainWindow::exportGraph(const QString& format) {
    ...
    if (format.toLower() == "dot") {
        ...
        if (m_currentGraph) {
            QString dotContent = generateValidDot(m_currentGraph);
            QFile file(fileName);
