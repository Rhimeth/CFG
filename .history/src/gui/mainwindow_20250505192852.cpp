...
std::shared_ptr<GraphGenerator::CFGGraph> MainWindow::parseDotToCFG(const QString& dotContent) {
    ...
    static const QRegularExpression nodeRegex("(\\w+)\\s*\\[([^\\]]*)\\]");
    static const QRegularExpression edgeRegex("(\\w+)\\s*->\\s*(\\w+)");
    ...
    try {
        ...
        for (const QString& line : lines) {
            ...
            if (nodeMatch.hasMatch() && nodeMatch.captured(1).startsWith("node")) {
                nodesFound = true;
                QString nodeIdStr = nodeMatch.captured(1).mid(4); // Remove "node" prefix
            ...
            if (edgeMatch.hasMatch()) {
                ...
                // Handle both "nodeX" and numeric IDs
                int fromId = fromStr.startsWith("node") ? fromStr.mid(4).toInt() : fromStr.toInt();
                int toId = toStr.startsWith("node") ? toStr.mid(4).toInt() : toStr.toInt();
