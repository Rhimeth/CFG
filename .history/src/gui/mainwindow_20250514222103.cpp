...
void MainWindow::setupBasicUI()
{
    ...
    // Create file list widget if it doesn't exist
    if (!ui->centralwidget->findChild<QListWidget*>("fileListWidget")) {
        ...
        if (ui->centralwidget->layout()) {
            // Insert at the beginning of the existing layout
...
void MainWindow::analyzeMultipleFiles()
{
    ...
    if (!fileListWidget) {
        QMessageBox::warning(this, "UI Error", "File list widget not found");
        return;
    }
    
    // Get files from list widget
    QStringList files;
    for (int i = 0; i < fileListWidget->count(); i++) {
        files.append(fileListWidget->item(i)->data(Qt::UserRole).toString());
    }
    
    if (files.isEmpty()) {
        QMessageBox::warning(this, "No Files", "Please add source files to analyze first");
        return;
    }
    
    setUiEnabled(false);
    statusBar()->showMessage("Analyzing multiple files...");
    
    // Convert QString to std::string
    std::vector<std::string> sourceFiles;
    for (const QString& file : files) {
        sourceFiles.push_back(file.toStdString());
    }
    
    // Create a progress dialog
    QProgressDialog progressDialog("Analyzing files...", "Cancel", 0, files.size(), this);
    progressDialog.setWindowModality(Qt::WindowModal);
    progressDialog.show();
    
    // Create and run analysis thread
    CFGAnalyzer::CFGAnalyzer analyzer;
    
    // Use shared_ptr to handle the result properly (avoiding copy constructor issue)
    auto resultPtr = std::make_shared<CFGAnalyzer::AnalysisResult>();
    
    QtConcurrent::run([this, sourceFiles, &analyzer, &progressDialog, resultPtr]() {
        try {
            *resultPtr = analyzer.analyzeMultipleFiles(
                sourceFiles,
                [&progressDialog](int current, int total) -> bool {
                    QMetaObject::invokeMethod(&progressDialog, "setValue", 
                                          Q_ARG(int, current));
            ...
            QMetaObject::invokeMethod(this, [this, resultPtr]() {
                handleMultiFileAnalysisResult(*resultPtr);
            });
        } catch (const std::exception& e) {
            QMetaObject::invokeMethod(this, [this, e]() {
                QMessageBox::critical(this, "Analysis Error", 
...
void MainWindow::handleMultiFileAnalysisResult(const CFGAnalyzer::AnalysisResult& result)
{
    setUiEnabled(true);
    ...
    // Visualize the graph if we have dot output
    if (!result.dotOutput.empty()) {
        m_currentGraph = parseDotToCFG(QString::fromStdString(result.dotOutput));
        visualizeCFG(m_currentGraph);
    }
    
    statusBar()->showMessage("Multi-file analysis completed", 5000);
}