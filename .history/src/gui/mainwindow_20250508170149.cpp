    ...
    m_recentFiles()
{
    ...
    // Connect web view node click to our signal
...
void MainWindow::onNodeClicked(const QString& nodeId) {
    ...
    if (filename.isEmpty() || startLine < 0 || endLine < 0) {
        return; // No source location info available
    }

    if (it != nodes.end()) {
        // Display node details in the report panel
        displayNodeDetails(id, it->second);
        
        // Highlight corresponding code in the text editor
...
void MainWindow::highlightNodeInCodeEditor(int nodeId) {
    ...
    // First, ensure the file is loaded in the editor
    if (m_currentFile != info.filePath) {
        loadCodeFile(info.filePath);
    }
    ...
    cursor.setPosition(ui->codeEditor->document()->findBlockByNumber(info.startLine - 1).position());
    cursor.movePosition(QTextCursor::Down, QTextCursor::KeepAnchor, info.endLine - info.startLine + 1);
    ...
    QTextEdit::ExtraSelection startMarker;
    cursor.setPosition(ui->codeEditor->document()->findBlockByNumber(info.startLine - 1).position());
    startMarker.format.setBackground(QColor(150, 255, 150)); // Light green
    startMarker.format.setProperty(QTextFormat::FullWidthSelection, true);
    startMarker.cursor = cursor;
    extraSelections.append(startMarker);
    
    if (info.startLine != info.endLine) {
        QTextEdit::ExtraSelection endMarker;
        cursor.setPosition(ui->codeEditor->document()->findBlockByNumber(info.endLine - 1).position());
        endMarker.format.setBackground(QColor(255, 150, 150)); // Light red
        endMarker.format.setProperty(QTextFormat::FullWidthSelection, true);
        endMarker.cursor = cursor;
        extraSelections.append(endMarker);
    }
    
    ui->codeEditor->setExtraSelections(extraSelections);
    
    // Scroll to the node
    QTextCursor scrollCursor(ui->codeEditor->document()->findBlockByNumber(info.startLine - 1));
    ui->codeEditor->setTextCursor(scrollCursor);
    ui->codeEditor->ensureCursorVisible();
};
