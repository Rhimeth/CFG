    ...
    <script>
        ...
        viz.renderSVGElement(dot)
            .then(svg => {
                ...
                svg.addEventListener('click', (e) => { to avoid having two handlers for the same click
                    const node = e.target.closest('[id^="node"]');ditor highlights the corresponding code
                    if (node) {tener('mouseover', (e) => {
                        const nodeId = node.id.replace('node', '');
                        console.log("Node clicked:", nodeId);
                        if (window.bridge && typeof window.bridge.handleNodeClick === 'function') {
                            window.bridge.handleNodeClick(nodeId);ight code in editor');
                        } else {
                            console.error("Bridge or handleNodeClick not available");
                        }
                    }d to DOM
                });st container = document.getElementById('graph-container');
                if (container) {
                // Add to DOM.innerHTML = '';
                const container = document.getElementById('graph-container');
                if (container) {
                    container.innerHTML = '';
                    container.appendChild(svg);
                }onsole.error("Graph error:", err);
            })  showError("Failed to render graph");
            .catch(err => {tElementById('loading').textContent = "Render failed";
                console.error("Graph error:", err);
                showError("Failed to render graph");
                document.getElementById('loading').textContent = "Render failed";
            }); stroke-width: 3px !important;
            .highlighted {op-shadow(0 0 5px rgba(255, 165, 0, 0.7));
                stroke: #FFA500 !important;
                stroke-width: 3px !important;
                filter: drop-shadow(0 0 5px rgba(255, 165, 0, 0.7));
            }
    </script>_currentTheme.backgroundColor.name())
</body>arg(escapedDotContent);
</html>
    )").arg(m_currentTheme.backgroundColor.name())
      .arg(escapedDotContent);
    
    return html;Window::generateValidDot(std::shared_ptr<GraphGenerator::CFGGraph> graph) 
};
...
{   QTextStream stream(&dotContent);
    ...
    QTextStream stream(&dotContent);
               << "    label=\"Empty Graph\";\n"
    if (!graph) { "    empty [shape=plaintext, label=\"No graph available\"];\n"
        stream << "digraph G {\n"
               << "    label=\"Empty Graph\";\n"
        ...
        return dotContent.toStdString();
           ...
           << "  rankdir=TB;\n"
    // Write graph header2,12\";\n"
    stream << "digraph G {\n"
           << "  rankdir=TB;\n"ctangle, style=filled, fillcolor=lightgray];\n\n";
           << "  size=\"12,12\";\n"
           << "  dpi=150;\n"
    ...
    for (const auto& [id, node] : graph->getNodes()) {
        ...
        QString escapedLabel;capedLabel += "\\\""; break;
        for (const QChar& c : node.label) {"\\\\"; break;
            switch (c.unicode()) {Label += "\\n"; break;
        ...
        // Add node attributes
        if (graph->isNodeTryBlock(id)) {on(id)) {
            stream << ", shape=ellipse, fillcolor=lightblue";
        }
        ...
        stream << "];\n";
    }or (const auto& [id, node] : graph->getNodes()) {
        for (int successor : node.successors) {
    // Add edgesam << "  node" << id << " -> node" << successor;
    for (const auto& [id, node] : graph->getNodes()) {
        for (int successor : node.successors) {essor)) {
    ...
    }eturn dotContent.toStdString();
};
    stream << "}\n";
    return dotContent.toStdString();onst QString& input)
};
    QString output;
QString MainWindow::escapeDotLabel(const QString& input)
{   
    QString output;r& c : input) {
    ...
    return output;nVisualizationError(const QString& error) {
...
void MainWindow::showEdgeContextMenu(const QPoint& pos) {mplemented yet", 2000);
    QMenu menu;
    menu.addAction("Highlight Path", this, [this](){
        statusBar()->showMessage("Path highlighting not implemented yet", 2000);
    });
    menu.exec(m_graphView->mapToGlobal(pos));nWindow::parseDotToCFG(const QString& dotContent) {
...
std::shared_ptr<GraphGenerator::CFGGraph> MainWindow::parseDotToCFG(const QString& dotContent) {
    ...
    if (dotContent.isEmpty()) {
        ...
        return graph;content sample:" << dotContent.left(200) << "...";
    ...
    qDebug() << "Processing" << lines.size() << "lines from DOT content";s*->\s*\"?([a-zA-Z_][a-zA-Z0-9_:<>]*)\"?\s*(\[([^\]]*)\])?\s*;?)");
    ...
    if (!nodeRegex.isValid() || !edgeRegex.isValid()) {
        qWarning() << "Invalid regex patterns";
        return graph;> nodeNameToId;
    }nt nextId = 1;
    bool graphHasNodes = false;
    QMap<QString, int> nodeNameToId;
    int nextId = 1; i < lines.size(); i++) {
    bool graphHasNodes = false;ines[i].trimmed();
        if (line.isEmpty() || line.startsWith("//") || line.startsWith("#") || 
    for (int i = 0; i < lines.size(); i++) {ne.startsWith("graph") ||
        const QString& line = lines[i].trimmed();sWith("edge") ||
        if (line.isEmpty() || line.startsWith("//") || line.startsWith("#") || 
            ...
            line == "{" || line == "}") {
            continue;ssionMatch nodeMatch = nodeRegex.match(line);
        }f (nodeMatch.hasMatch() && !line.contains("->")) {
        ...
        if (nodeMatch.hasMatch() && !line.contains("->")) {
            QString nodeName = nodeMatch.captured(1);, nodeName);
            if (!nodeNameToId.contains(nodeName)) {
                nodeNameToId[nodeName] = nextId++;ame << "with ID:" << nodeNameToId[nodeName];
    ...
    if (!graphHasNodes) {Line" << i << ":" << lines[i];
        qWarning() << "No valid nodes found in DOT content";
        for (int i = 0; i < qMin(10, lines.size()); i++) {
            qDebug() << "Line" << i << ":" << lines[i];
        }
        return graph;reate all edges
    }or (const QString& line : lines) {
        QRegularExpressionMatch edgeMatch = edgeRegex.match(line);
    // Second pass: create all edges
    for (const QString& line : lines) {h.captured(1);
        QRegularExpressionMatch edgeMatch = edgeRegex.match(line);
        if (edgeMatch.hasMatch()) {
            QString fromName = edgeMatch.captured(1);deNameToId.contains(toName)) {
            ...
            if (nodeNameToId.contains(fromName) && nodeNameToId.contains(toName)) {
                int fromId = nodeNameToId[fromName];e << "->" << toName 
                int toId = nodeNameToId[toName];" << toId << ")";
    ...
    return graph;w::parseNodeAttributes(const QString& attributes) {
};  QString details;
    static const QRegularExpression labelRegex(R"(label="([^"]*))");
QString MainWindow::parseNodeAttributes(const QString& attributes) {*))");
    QString details;gularExpression locationRegex(R"(location="([^"]*)");
    static const QRegularExpression labelRegex(R"(label="([^"]*))");
    ...
    return details;adAndProcessJson(const QString& filePath) 
};
...
{       QMessageBox::warning(this, "Error", "JSON file not found: " + filePath);
    if (!QFile::exists(filePath)) {
        ...
        QMessageBox::warning(this, "Error", "JSON file not found: " + filePath);
        return;filePath);
    }f (!file.open(QIODevice::ReadOnly)) {
        qWarning() << "Could not open JSON file:" << file.errorString();
    QFile file(filePath);ing(this, "Error", "Could not open JSON file: " + file.errorString());
    if (!file.open(QIODevice::ReadOnly)) {
        qWarning() << "Could not open JSON file:" << file.errorString();
    ...
    // Read and parse JSONsonDocument::fromJson(jsonData, &parseError);
    QJsonParseError parseError;
    QByteArray jsonData = file.readAll();or::NoError) {
    QJsonDocument doc = QJsonDocument::fromJson(jsonData, &parseError); << ":" << parseError.errorString();
        QMessageBox::warning(this, "JSON Error", 
    if (parseError.error != QJsonParseError::NoError) {tion %1: %2")
        qWarning() << "JSON parse error at offset" << parseError.offset << ":" << parseError.errorString();
        QMessageBox::warning(this, "JSON Error", tring()));
    ...
    try {/ Example processing - adapt to your needs
        QJsonObject jsonObj = doc.object();onObj["nodes"].isArray()) {
        ...
        if (jsonObj.contains("nodes") && jsonObj["nodes"].isArray()) {
            ...
            }Object::invokeMethod(this, [this, jsonObj]() {
        ...
        QMetaObject::invokeMethod(this, [this, jsonObj]() {
            m_graphView->parseJson(QJsonDocument(jsonObj).toJson());
            statusBar()->showMessage("JSON loaded successfully", 3000);
        });ssageBox::critical(this, "Processing Error", 
    } catch (const std::exception& e) {ror processing JSON: %1").arg(e.what()));
    ...
    }MainWindow::initializeGraphviz()
};
    QString dotPath = QStandardPaths::findExecutable("dot");
void MainWindow::initializeGraphviz()
{       qCritical() << "Graphviz 'dot' not found in PATH";
    QString dotPath = QStandardPaths::findExecutable("dot");
    if (dotPath.isEmpty()) {"Graphviz 'dot' executable not found.\n"
        qCritical() << "Graphviz 'dot' not found in PATH";nsure it's in your PATH.");
        QMessageBox::critical(this, "Error", 
                            "Graphviz 'dot' executable not found.\n"
                            "Please install Graphviz and ensure it's in your PATH.");
        startTextOnlyMode();
        return; "Found Graphviz dot at:" << dotPath;
    }etupGraphView();
};
    qDebug() << "Found Graphviz dot at:" << dotPath;
    setupGraphView();yzeDotFile(const QString& filePath) {
};  if (!verifyDotFile(filePath)) return;

void MainWindow::analyzeDotFile(const QString& filePath) {
    if (!verifyDotFile(filePath)) return;).completeBaseName();
    QString pngPath = tempDir + "/" + baseName + "_graph.png";
    QString tempDir = QDir::tempPath();aseName + "_graph.svg";
    ...
    if (renderDotToImage(filePath, pngPath)) {
        displayImage(pngPath);
        return; to SVG
    }f (renderDotToImage(filePath, svgPath)) {
        displaySvgInWebView(svgPath);
    // Fallback to SVG
    if (renderDotToImage(filePath, svgPath)) {
        displaySvgInWebView(svgPath);
        return;ontent(filePath);
    }

    showRawDotContent(filePath);e(const QString& dotPath, const QString& outputPath, const QString& format)
};
...
{   if (!dotFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
    // 1. Enhanced DOT file validationt open DOT file:\n%1\nError: %2")
    QFile dotFile(dotPath);dotPath)
    if (!dotFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        QString error = QString("Cannot open DOT file:\n%1\nError: %2")
                      .arg(dotPath) "DOT File Error", error);
                      .arg(dotFile.errorString());
        qWarning() << error;
        QMessageBox::critical(this, "DOT File Error", error);
        return false;t = dotFile.readAll();
    }otFile.close();
    if (dotContent.trimmed().isEmpty()) {
    QString dotContent = dotFile.readAll();or contains only whitespace";
    dotFile.close();< error;
    if (dotContent.trimmed().isEmpty()) {File Error", error);
        QString error = "DOT file is empty or contains only whitespace";
        qWarning() << error;
        QMessageBox::critical(this, "DOT File Error", error);tsWith("graph")) {
        return false; = QString("Invalid DOT file format. Must start with 'digraph' or 'graph'.\n"
    ...
    // 2. Format handlingdsWith(".png", Qt::CaseInsensitive)) outputFormat = "png";
    QString outputFormat = format.toLower(); Qt::CaseInsensitive)) outputFormat = "svg";
    if (outputFormat.isEmpty()) {ith(".pdf", Qt::CaseInsensitive)) outputFormat = "pdf";
        if (outputPath.endsWith(".png", Qt::CaseInsensitive)) outputFormat = "png";
        else if (outputPath.endsWith(".svg", Qt::CaseInsensitive)) outputFormat = "svg";Path);
        else if (outputPath.endsWith(".pdf", Qt::CaseInsensitive)) outputFormat = "pdf";
        else {essageBox::critical(this, "Export Error", error);
            QString error = QString("Unsupported output format for file: %1").arg(outputPath);
            qWarning() << error;
            QMessageBox::critical(this, "Export Error", error);
            return false;
        } Graphviz executable handling
    }String dotExecutablePath;
    QStringList potentialPaths = {
    // 3. Graphviz executable handling
    QString dotExecutablePath;
    QStringList potentialPaths = {
        "dot",ogram Files/Graphviz/bin/dot.exe"
        "/usr/local/bin/dot",
        "/usr/bin/dot",&path : potentialPaths) {
    ...
    if (dotExecutablePath.isEmpty()) {
        ...
        return false;cution with better error handling
    }StringList arguments = {
        "-Gsize=12,12",         // Larger default size
    // 4. Process execution with better error handling
    QStringList arguments = {   // Add some margin
        "-Gsize=12,12",         // Larger default sizeize
    ...
    dotProcess.start(dotExecutablePath, arguments);aphviz:\n%1\nCommand: %2 %3")
                       .arg(dotProcess.errorString())
    if (!dotProcess.waitForStarted(3000)) {th)
        QString error = QString("Failed to start Graphviz:\n%1\nCommand: %2 %3")
                       .arg(dotProcess.errorString())
                       .arg(dotExecutablePath)rror", error);
                       .arg(arguments.join(" "));
        qWarning() << error;
        QMessageBox::critical(this, "Process Error", error);
        return false;out and process output
    }ByteArray processOutput;
    QElapsedTimer timer;
    // Wait with timeout and process output
    QByteArray processOutput;
    QElapsedTimer timer;aitForFinished(500)) {
    timer.start();put += dotProcess.readAll();
        QCoreApplication::processEvents();
    while (!dotProcess.waitForFinished(500)) {econd timeout
        processOutput += dotProcess.readAll();
        QCoreApplication::processEvents();viz timed out after 15 seconds\nPartial output:\n%1")
        if (timer.hasExpired(15000)) { // 15 second timeout
            ...
            return false;
        }ssOutput += dotProcess.readAll();
    }
    // 5. Output validation. Content verification
    processOutput += dotProcess.readAll();le::exists(outputPath)) {
        QString error = QString("Graphviz failed (exit code %1)\nError output:\n%2")
    // 5. Output validation. Content verification
    if (dotProcess.exitCode() != 0 || !QFile::exists(outputPath)) {
        QString error = QString("Graphviz failed (exit code %1)\nError output:\n%2")
                      .arg(dotProcess.exitCode())ror", error);
                      .arg(QString(processOutput));
        qWarning() << error;tputPath);
        QMessageBox::critical(this, "Rendering Error", error);
        if (QFile::exists(outputPath)) {
            QFile::remove(outputPath);
        }
        return false;ification
    }FileInfo outputInfo(outputPath);
    if (outputInfo.size() < 100) { // Minimum expected file size
    // 6. Content verificationg("Output file too small (%1 bytes)\nGraphviz output:\n%2")
    QFileInfo outputInfo(outputPath);.size())
    if (outputInfo.size() < 100) { // Minimum expected file size
        QString error = QString("Output file too small (%1 bytes)\nGraphviz output:\n%2")
                      .arg(outputInfo.size())
                      .arg(QString(processOutput));
        qWarning() << error;
        if (QFile::exists(outputPath)) {put Error", error);
            QFile::remove(outputPath);
        }
        QMessageBox::critical(this, "Output Error", error);
        return false;ific validation
    }f (outputFormat == "png") {
        QFile file(outputPath);
    // 7. Format-specific validationdOnly)) {
    if (outputFormat == "png") {file.read(8);
        QFile file(outputPath);
        if (file.open(QIODevice::ReadOnly)) {) {
            QByteArray header = file.read(8);
            file.close();rror = "Invalid PNG file header - corrupted output";
            if (!header.startsWith("\x89PNG")) {
                QFile::remove(outputPath);, "PNG Error", error);
    ...
    else if (outputFormat == "svg") {ad(1024);
        QFile file(outputPath);
        if (file.open(QIODevice::ReadOnly)) {
            QString content = file.read(1024);
            file.close();rror = "Invalid SVG content - missing SVG tag";
            if (!content.contains("<svg")) {
                QFile::remove(outputPath);, "SVG Error", error);
                QString error = "Invalid SVG content - missing SVG tag";
                qWarning() << error;
                QMessageBox::critical(this, "SVG Error", error);
                return false;
            }
        }g() << "Successfully exported graph to:" << outputPath;
    }eturn true;
};
    qDebug() << "Successfully exported graph to:" << outputPath;
    return true;:showRawDotContent(const QString& dotPath) {
};  QFile file(dotPath);
    if (file.open(QIODevice::ReadOnly)) {
void MainWindow::showRawDotContent(const QString& dotPath) {
    ...
    if (file.open(QIODevice::ReadOnly)) {
        ...
        file.close();
    }MainWindow::visualizeCurrentGraph() {
};  if (!m_currentGraph) return;

void MainWindow::visualizeCurrentGraph() {DotRepresentation(m_currentGraph.get());
    if (!m_currentGraph) return;
    // Load into web view
    ...
    // Load into web view
    QString html = QString(R"(
<!DOCTYPE html>Visualization</title>
<html>cript src="qrc:/qtwebchannel/qwebchannel.js"></script>
<head>cript src="https://cdnjs.cloudflare.com/ajax/libs/viz.js/2.1.2/viz.js"></script>
    <title>CFG Visualization</title>flare.com/ajax/libs/viz.js/2.1.2/full.render.js"></script>
    <script src="qrc:/qtwebchannel/qwebchannel.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/viz.js/2.1.2/viz.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/viz.js/2.1.2/full.render.js"></script>
    <style>de:hover { stroke-width:2px; cursor:pointer; }
        body { margin:0; background:#2D2D2D; }-width:2px; }
        #graph-container { width:100%; height:100%; }ext-align: center; }
        .node:hover { stroke-width:2px; cursor:pointer; }
        .expanded-node { fill: #ffffcc; stroke-width:2px; }
        .error-message { color: red; padding: 20px; text-align: center; }
        .highlighted {op-shadow(0 0 5px rgba(255, 165, 0, 0.7));
    ...
    </style>"graph-container"></div>
</head>ript>
<body>  new QWebChannel(qt.webChannelTransport, function(channel) {
    <div id="graph-container"></div>objects.bridge;
    <script>
        new QWebChannel(qt.webChannelTransport, function(channel) {
            window.bridge = channel.objects.bridge;
        }); .then(element => {
        const viz = new Viz();handling
        viz.renderSVGElement(`%1`)stener('click', (e) => {
            .then(element => { = e.target.closest('[id^="node"]');
                // Node click handling.bridge) {
                element.addEventListener('click', (e) => {d.replace('node', ''));
                    const node = e.target.closest('[id^="node"]');
                    if (node && window.bridge) {
                        window.bridge.onNodeClicked(node.id.replace('node', ''));
                    }ge hover handling
                });ment.addEventListener('mousemove', (e) => {
                    const edge = e.target.closest('[id^="edge"]');
                // Edge hover handling.bridge) {
                element.addEventListener('mousemove', (e) => {ge', '').split('_');
                    const edge = e.target.closest('[id^="edge"]');
                    if (edge && window.bridge) {
                        const [from, to] = edge.id.replace('edge', '').split('_');
                        window.bridge.onEdgeHovered(from, to);
                    }ent.getElementById('graph-container').appendChild(element);
                });
                ch(error => {
                document.getElementById('graph-container').appendChild(element);
            })  const container = document.getElementById('graph-container');
            .catch(error => {) {
                console.error(error);ML = '<div class="error-message">Graph rendering failed: ' + 
                const container = document.getElementById('graph-container');
                if (container) {
    ...
    </script>String::fromStdString(dot));
</body>
    ...
    connect(webView, &QWebEngineView::loadFinished, [this](bool success) {
        if (success) {
            qDebug() << "Web view loaded successfully, initializing web channel...";
            webView->page()->runJavaScript(nel not loaded");
                R"(lse {
                if (typeof QWebChannel === "undefined") {t, function(channel) {
                    console.error("QWebChannel not loaded");ge;
                } else {console.log("Web channel established");
                    new QWebChannel(qt.webChannelTransport, function(channel) {tialized === "function") {
                        window.bridge = channel.objects.bridge;();
                ...
                )"ng() << "Failed to load web view";
            );
        } else {
            qWarning() << "Failed to load web view";
        }
    });inWindow::highlightNode(int nodeId, const QColor& color)
};
...
{   // Reset previous highlighting
    ...
    // Reset previous highlightingtive
    resetHighlighting();ew->isVisible()) {
        webView->page()->runJavaScript(
    // Highlight in web view if active'node', '%1');").arg(nodeId)
    if (webView && webView->isVisible()) {
        webView->page()->runJavaScript(
            QString("highlightElement('node', '%1');").arg(nodeId)
        );hlight in graphics view
    }oreach (QGraphicsItem* item, m_graphView->scene()->items()) {
        if (item->data(MainWindow::NodeItemType).toInt() == 1) {
    // Highlight in graphics viewraphicsitem_cast<QGraphicsEllipseItem*>(item)) {
    foreach (QGraphicsItem* item, m_graphView->scene()->items()) {odeId) {
        if (item->data(MainWindow::NodeItemType).toInt() == 1) {
            if (auto ellipse = qgraphicsitem_cast<QGraphicsEllipseItem*>(item)) {
                if (item->data(MainWindow::NodeIdKey).toInt() == nodeId) {
                    ...
                    pen.setWidth(3);llipse->brush();
                    pen.setColor(Qt::darkBlue);
                    ellipse->setPen(pen);sh);
                    QBrush brush = ellipse->brush();
                    brush.setColor(color);item);
    ...
    }MainWindow::highlightEdge(int fromId, int toId, const QColor& color)
};
    // Reset previous highlighting
...
{       if (auto line = qgraphicsitem_cast<QGraphicsLineItem*>(m_highlightEdge)) {
    ...
    if (m_scene) { (item->data(EdgeFromKey).toInt() == fromId &&
        for (QGraphicsItem* item : m_scene->items()) {oId) {
            if (item->data(EdgeItemType).toInt() == 1) {GraphicsLineItem*>(item)) {
    ...
    }MainWindow::resetHighlighting()
};
    if (m_highlightNode) {
void MainWindow::resetHighlighting()item_cast<QGraphicsEllipseItem*>(m_highlightNode)) {
{           QPen pen = ellipse->pen();
    if (m_highlightNode) {);
        if (auto ellipse = qgraphicsitem_cast<QGraphicsEllipseItem*>(m_highlightNode)) {
            QPen pen = ellipse->pen();
            pen.setWidth(1);h(QBrush(Qt::lightGray));
            pen.setColor(Qt::black);
            ellipse->setPen(pen);;
            ellipse->setBrush(QBrush(Qt::lightGray));
        }_highlightEdge) {
        m_highlightNode = nullptr;tem_cast<QGraphicsLineItem*>(m_highlightEdge)) {
    }       QPen pen = line->pen();
    if (m_highlightEdge) {);
        if (auto line = qgraphicsitem_cast<QGraphicsLineItem*>(m_highlightEdge)) {
            QPen pen = line->pen();
        ...
        m_highlightEdge = nullptr;
    }MainWindow::expandNode(const QString& nodeIdStr)
};
    bool ok;
...
{   if (!ok || !m_currentGraph) return;
    ...
    if (!ok || !m_currentGraph) return;>getNodes();
    auto it = nodes.find(nodeId);
    m_expandedNodes[nodeId] = true;
    const auto& nodes = m_currentGraph->getNodes();
    auto it = nodes.find(nodeId);= true;
    if (it != nodes.end()) {
        for (int succ : it->second.successors) {
            m_visibleNodes[succ] = true;ue, m_currentRootNode);
        }
    }
    displayGraph(m_currentDotContent, true, m_currentRootNode);
};  if (!m_currentGraph) return -1;

int MainWindow::findEntryNode() {Graph->getNodes();
    if (!m_currentGraph) return -1;
    ...
    return nodes.begin()->first;nodeId) {
...
void MainWindow::selectNode(int nodeId) {
    ...
    m_currentlySelectedNodeId = nodeId; || !m_currentGraph) {
    ...
    if (!m_nodeInfoMap.contains(nodeId) || !m_currentGraph) {
        qWarning() << "No node info available for node:" << nodeId;
        return;t in graph
    }ighlightNode(nodeId, QColor(Qt::yellow));
    
    // Highlight in graphntains(nodeId) || !m_currentGraph) {
    highlightNode(nodeId, QColor(Qt::yellow));
    }
    if (!m_nodeInfoMap.contains(nodeId) || !m_currentGraph) {
        return; information
    }onst NodeInfo& nodeInfo = m_nodeInfoMap[nodeId];
    
    // Get node informationeport
    const NodeInfo& nodeInfo = m_nodeInfoMap[nodeId];
    report += QString("=== Node %1 Summary ===\n").arg(nodeId);
    // Build node summary reportn").arg(nodeInfo.filePath);
    QString report;ng("Lines: %1-%2\n").arg(nodeInfo.startLine).arg(nodeInfo.endLine);
    report += QString("=== Node %1 Summary ===\n").arg(nodeId);
    ...
    }ool hasIncoming = false;
    for (const auto& [otherId, otherNode] : m_currentGraph->getNodes()) {
    report += "\n=== Called By ===\n";sors.begin(), otherNode.successors.end(), nodeId) != otherNode.successors.end()) {
    bool hasIncoming = false;e;
    for (const auto& [otherId, otherNode] : m_currentGraph->getNodes()) {odeId) ? 
        if (std::find(otherNode.successors.begin(), otherNode.successors.end(), nodeId) != otherNode.successors.end()) {
            ...
            // Get caller node info if availableap[otherId];
            QString callerInfo;tring(" [Lines %1-%2]").arg(caller.startLine).arg(caller.endLine);
            if (m_nodeInfoMap.contains(otherId)) {
    ...
    report += "\n=== Calls To ===\n";
    const auto& nodes = m_currentGraph->getNodes();>second.successors.empty()) {
    auto currentNode = nodes.find(nodeId);second.successors) {
    ...
    if (currentNode != nodes.end() && !currentNode->second.successors.empty()) {
        for (int successor : currentNode->second.successors) {
            ...
            QString calleeInfo;tring(" [Lines %1-%2]").arg(callee.startLine).arg(callee.endLine);
    ...
    // Add actual code content sectioneInfo.statements) {
    report += "\n=== Code Content ===\n";
    if (!nodeInfo.statements.isEmpty()) {
    ...
    }i->reportTextEdit->setPlainText(report);
    ...
    if (!nodeInfo.filePath.isEmpty()) {
        loadAndHighlightCode(nodeInfo.filePath, nodeInfo.startLine, nodeInfo.endLine);
    }MainWindow::centerOnNode(const QString& nodeId) {
};  bool ok;
    int id = nodeId.toInt(&ok);
...
void MainWindow::clearCodeHighlights() {lockByNumber(startLine - 1));
    ...
    updateExpandedNode(id, detailedContent);ections);
...
void MainWindow::onNodeCollapsed(const QString& nodeId) {
    ui->reportTextEdit->clear();
    statusBar()->showMessage(QString("Collapsed node %1").arg(nodeId), 2000);
};  bool ok;  QFile file(filePath);
    int id = nodeId.toInt(&ok);    if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
void MainWindow::loadCodeFile(const QString& filePath) {turn;hlights
    QFile file(filePath);
    if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        clearCodeHighlights(); // Clear any existing highlights
        codeEditor->setPlainText(file.readAll());de %1").arg(nodeId), 2000);ilePath;
        file.close();r", 
    } else {   QString("Could not open file:\n%1").arg(filePath));
        qWarning() << "Could not open file:" << filePath;
        QMessageBox::warning(this, "Error", 
                           QString("Could not open file:\n%1").arg(filePath));
    }nWindow::onEdgeHovered(const QString& from, const QString& to)
};
void MainWindow::loadCodeFile(const QString& filePath) {    bool ok1, ok2;
void MainWindow::onEdgeHovered(const QString& from, const QString& to)
{   if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {   int toId = to.toInt(&ok2);
    bool ok1, ok2;ighlights(); // Clear any existing highlights
    int fromId = from.toInt(&ok1);ile.readAll());
    int toId = to.toInt(&ok2);ing("Edge %1 → %2").arg(fromId).arg(toId), 2000);
    } else {} else {
    if (ok1 && ok2) { "Could not open file:" << filePath;->showMessage(QString("Edge %1 → %2").arg(from).arg(to), 2000);
    ...
    }MainWindow::getDetailedNodeContent(int nodeId) {
...
QString MainWindow::getDetailedNodeContent(int nodeId) {
    ...
    return content;andedNode(int nodeId, const QString& content) {
};      ui->statusbar->showMessage(QString("Edge %1 → %2").arg(from).arg(to), 2000);  // Execute JavaScript to update the node
    }    webView->page()->runJavaScript(
void MainWindow::updateExpandedNode(int nodeId, const QString& content) {
    // Execute JavaScript to update the node
    webView->page()->runJavaScript(Content(int nodeId) {querySelector('text');"
        QString("var node = document.getElementById('node%1');"
                "if (node) {"entGraph->getNodes().at(nodeId);Id).arg(content));
                "  var text = node.querySelector('text');"
                "  if (text) text.textContent = '%2';"
                "}").arg(nodeId).arg(content));
};  }  // Execute JavaScript to collapse the node
    return content;    webView->page()->runJavaScript(
void MainWindow::updateCollapsedNode(int nodeId) {
    // Execute JavaScript to collapse the node
    webView->page()->runJavaScript((int nodeId, const QString& content) {querySelector('text');"
        QString("var node = document.getElementById('node%1');"
                "if (node) {"cript(Id).arg(nodeId));
                "  var text = node.querySelector('text');"1');"
                "  if (text) text.textContent = 'Node %2';"
                "}").arg(nodeId).arg(nodeId));or('text');"int& pos) {
};              "  if (text) text.textContent = '%2';"  QMenu menu;
                "}").arg(nodeId).arg(content));    
void MainWindow::showNodeContextMenu(const QPoint& pos) {
    QMenu menu;mapFromGlobal(pos);
     MainWindow::updateCollapsedNode(int nodeId) {QString nodeId = getNodeAtPosition(viewPos);
    // Get node under cursor collapse the node
    QPoint viewPos = webView->mapFromGlobal(pos);
    QString nodeId = getNodeAtPosition(viewPos);yId('node%1');"nodeId]() {
                "if (node) {"        bool ok;
    if (!nodeId.isEmpty()) {= node.querySelector('text');"toInt(&ok);
        menu.addAction("Show Node Info", [this, nodeId]() {
            bool ok;.arg(nodeId).arg(nodeId));
            int id = nodeId.toInt(&ok);
            if (ok) displayNodeInfo(id);
        });ndow::showNodeContextMenu(const QPoint& pos) { bool ok;
        u menu;    int id = nodeId.toInt(&ok);
        menu.addAction("Go to Code", [this, nodeId]() {
            bool ok;r cursortCursor cursor = m_nodeCodePositions[id];
            int id = nodeId.toInt(&ok);obal(pos);or(cursor);
