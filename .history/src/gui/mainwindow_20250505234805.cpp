...
void MainWindow::displayGraph(const QString& dotContent, bool isProgressive = false, int rootNode = -1) 
{
    if (!webView) {
    ...
    std::string escapedDot = escapeDotLabel(processedDot);
...
<body>
    ...
    )").arg(QString::fromStdString(escapedDot)).arg(rootNode);
...
{
    // Parse the full DOT to extract node relationships
    QMap<int, QList<int>> adjacencyList;
    QRegularExpression edgeRegex("node(\\d+)\\s*->\\s*node(\\d+)");
    
    auto matches = edgeRegex.globalMatch(fullDot);
    ...
    // Generate modified DOT
    QString output = "digraph G {\n";
    output += "  rankdir=TB;\n";
    output += "  node [shape=rectangle, style=\"rounded,filled\"];\n\n";

    // Add visible nodes (from original DOT)
    QRegularExpression nodeRegex("node(\\d+)\\s*\\[([^\\]]+)\\]");
    matches = nodeRegex.globalMatch(fullDot);
    ...
    for (auto it = adjacencyList.begin(); it != adjacencyList.end(); ++it) {
        ...
        for (int to : it.value()) {
            if (m_visibleNodes[to]) {
                output += QString("  node%1 -> node%2;\n").arg(from).arg(to);
            } else if (m_expandedNodes[from]) {
                // Show expandable stub
                output += QString("  node%1 -> node%2 [style=dashed, color=gray];\n").arg(from).arg(to);
                output += QString("  node%2 [label=\"+\", shape=ellipse, fillcolor=\"#9E9E9E\"];\n").arg(to);
            }
        }
    }
...
void MainWindow::displayProgressiveGraph()
{
    ...
    for (const auto& [id, node] : m_currentGraph->getNodes()) {
        if (m_visibleNodes[id]) {
            ...
            if (id == m_currentRootNode) {
                dot << ", fillcolor=\"#4CAF50\", penwidth=2";
            }
    ...
    for (const auto& [id, node] : m_currentGraph->getNodes()) {
        ...
        for (int succ : node.successors) {
            if (m_visibleNodes[succ]) {
                ...
                dot << "  node" << succ << " [label=\"+\", shape=ellipse, fillcolor=\"#9E9E9E\"];\n";
                dot << "  node" << id << " -> node" << succ << " [style=dashed, color=gray];\n";
            }
        }
    }
    ...
    displayGraph(QString::fromStdString(dot.str()));
};
...
void MainWindow::handleProgressiveNodeClick(const QString& nodeId)
{
    ...
    if (m_currentGraph->getNodes().count(id) && !m_visibleNodes[id]) {
        // Mark node as expanded and visible
        m_expandedNodes[id] = true;
        m_visibleNodes[id] = true;
        
        // Make successors visible
        for (int succ : m_currentGraph->getNodes()[id].successors) {
...
Q_INVOKABLE void MainWindow::expandNode(const QString& nodeIdStr)
{
    bool ok;
    ...
    if (nodes.find(nodeId) != nodes.end()) {
        for (int succ : nodes[nodeId].successors) {
            m_visibleNodes[succ] = true;
        }
    }
...
int MainWindow::findEntryNode()
{
    ...
    return nodes.begin()->first; // Fallback to first node if no clear entry
};
...
void MainWindow::onNodeClicked(const QString& nodeId) {
    ...
    // Create a non-const reference to nodes if possible
    auto& nodes = const_cast<std::map<int, GraphGenerator::CFGNode>&>(m_currentGraph->getNodes());
    
    // Or better, add a non-const getNodes() method to your graph class
    // auto& nodes = m_currentGraph->getMutableNodes();
