...
void MainWindow::visualizeCFG(std::shared_ptr<GraphGenerator::CFGGraph> graph)
{
    ...
    if (!webView || !webView->isVisible()) {
        ...
        // Pass the shared_ptr directly instead of using get()
        std::string dotContent = generateValidDot(graph);
    ...
    try {
        ...
        // Fall back to basic DOT display - pass the shared_ptr directly
        std::string dotContent = generateValidDot(graph);
        ui->reportTextEdit->setPlainText(QString::fromStdString(dotContent));
    }
};
