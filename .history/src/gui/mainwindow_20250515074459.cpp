...
void MainWindow::loadCodeFile(const QString& filePath) {
    ...
    try {
        // Clear any previous analysis data that might cause crashes
        if (m_currentGraph) {
...
void MainWindow::on_analyzeButton_clicked()
{
    ...
    try {
        ...
        // Display the graph - this is where it might fail
...
void MainWindow::displayGraph(const QString& dotContent, bool isProgressive, int rootNode) 
{
    ...
    // Create HTML with viz.js for rendering
    QString html = QString(R"(
<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    <script>
        ...
        viz.renderSVGElement(`%1`)
            .then(svg => {
                ...
                if (window.bridge && typeof window.bridge.graphRenderingComplete === 'function') {
                    window.bridge.graphRenderingComplete();
                }
            })
            .catch(error => {
                document.getElementById('loading').style.display = 'none';
    ...
    // Setup bridge when loading completes
    connect(webView, &QWebEngineView::loadFinished, this, [this](bool success) {
        // Disconnect to make this a single-shot connection
        disconnect(webView, &QWebEngineView::loadFinished, this, nullptr);
        
        if (!success) {
            qWarning() << "Failed to load visualization in web view";
            QMessageBox::warning(this, "Visualization Error", 
                               "Failed to load graph visualization. Check browser console for details.");
            return;
        }
        
        qDebug() << "Web view loaded successfully, injecting web channel";
        
        // Inject QWebChannel script and setup bridge
        webView->page()->runJavaScript(
            "if (typeof QWebChannel === 'undefined') {"
            "  var script = document.createElement('script');"
            "  script.src = 'qrc:/qtwebchannel/qwebchannel.js';"
            ...
            "    document.getElementById('error-container').textContent = 'Failed to load communication channel';"
            "    document.getElementById('error-container').style.display = 'block';"
            "  };"
            "  document.head.appendChild(script);"
            "}"
        ...
        // Check if Viz.js loaded properly
        webView->page()->runJavaScript(
            "if (typeof Viz === 'undefined') {"
            "  document.getElementById('error-container').textContent = 'Viz.js library failed to load';"
            "  document.getElementById('error-container').style.display = 'block';"
            "  console.error('Viz.js not available');"
            "}"
        );
