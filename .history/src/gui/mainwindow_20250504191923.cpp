...
<body>
    ...
    <script>
        ...
        function toggleNode(nodeId) {
            ...
            safeBridgeCall(
                collapsedNodes[nodeId] ? 'nodeCollapsed' : 'nodeExpanded', 
                nodeId.toString()
            );
        }
                ...
                svg.addEventListener('click', (e) => {
                    ...
                    if (node) {
                        ...
                        if (graphData[nodeId]?.isCollapsible) {
                            ...
                            safeBridgeCall('nodeClicked', nodeId);
                        }
                    } 
                    else if (edge) {
                        ...
                        if (edgeId) {
                            ...
                            if (from && to) {
                                ...
                                safeBridgeCall('edgeClicked', from, to);
                            }
                        }
                    }
                });
...
std::shared_ptr<GraphGenerator::CFGGraph> MainWindow::parseDotToCFG(const QString& dotContent) {
    ...
    for (const QString& line : lines) {
        ...
        if (nodeMatch.hasMatch()) {
            ...
            // Parse label and extract function name if present
            ...
            if (labelMatch.hasMatch()) {
                ...
                if (functionMatch.hasMatch()) {
                    ...
                    // Fallback: look for function name patterns in label
                    static QRegularExpression funcNamePattern(R"(\b([a-zA-Z_][a-zA-Z0-9_]*)\s*\()");
                    auto match = funcNamePattern.match(label);
                    if (match.hasMatch()) {
                        graph->setNodeFunctionName(id, match.captured(1));
                    }
                }
            }
    ...
    for (const auto& [id, node] : graph->getNodes()) {
        ...
        info.functionName = node.functionName;
        ...
        // Store successors and predecessors
        info.successors = node.successors;
        for (int succ : node.successors) {
            if (auto succNode = graph->getNode(succ)) {
                succNode->predecessors.append(id);
...
void MainWindow::onNodeClicked(const QString& nodeId) {
    ...
    // Get the source location from the graph node
    auto [filename, startLine, endLine] = m_currentGraph->getNodeSourceRange(id);
    
    if (startLine != -1 && endLine != -1) {
        // If this node is from a different file, load that file first
        if (!m_currentFile.isEmpty() && m_currentFile != filename) {
        ...
        highlightNode(id, QColor("#ffffa0")); // Light yellow highlight
    }
};
...
void MainWindow::on_searchButton_clicked() {
    ...
    for (const auto& [id, info] : m_nodeInfoMap) {
...
void MainWindow::showNextSearchResult() {
    ...
    int nodeId = *std::next(m_searchResults.begin(), m_currentSearchIndex);
...
void MainWindow::displayNodeInfo(int nodeId) {
    ...
    report += "\n  Predecessors: ";
    for (int pred : info.predecessors) {
        report += QString::number(pred) + " ";
    }
...
void MainWindow::loadNodeInformation(const QString& filePath) {
    ...
    if (doc.isArray()) {
        ...
        for (const QJsonValue& val : doc.array()) {
            NodeInfo info = NodeInfo::fromJson(val.toObject());
