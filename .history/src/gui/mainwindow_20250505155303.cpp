...
std::shared_ptr<GraphGenerator::CFGGraph> MainWindow::parseDotToCFG(const QString& dotContent) {
    ...
    // Pre-compiled regex patterns with simplified escaping
    static const QRegularExpression nodeRegex(R"(^\s*(\w+)\s*\[([^\]]*)\])");
    static const QRegularExpression edgeRegex(R"(^\s*(\w+)\s*(->|--)\s*(\w+)(?:\s*\[([^\]]*)\])?)");
    static const QRegularExpression attrRegex(R"((\w+)\s*=\s*(?:"([^"]*)"|([^\s;,]+)))");
