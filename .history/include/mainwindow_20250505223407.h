...
public:
    ...
    std::string generateInteractiveDot(std::shared_ptr<GraphGenerator::CFGGraph> graph);
    std::string generateProgressiveDot(std::shared_ptr<GraphGenerator::CFGGraph> graph, int startNode);
...
private:
    ...
    std::unordered_map<int, bool> m_expandedNodes;
    std::unordered_map<int, bool> m_visibleNodes;
    int m_currentRootNode = -1;
    ...
    QMap<int, bool> m_expandedNodes; // Tracks which nodes are expanded
